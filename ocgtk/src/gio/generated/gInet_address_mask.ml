...
class inet_address_mask (obj : Inet_address_mask.t) : inet_address_mask_t =
  object (self)
    ...
    method get_family : unit -> Gio_enums.socketfamily =
      fun () -> Inet_address_mask.get_family obj

    method get_length : unit -> int = fun () -> Inet_address_mask.get_length obj
    ...
    method to_string : unit -> string =
      fun () -> Inet_address_mask.to_string obj

    method as_inet_address_mask = obj
  end

let new_ (addr : GInet_address.inet_address_t) (length : int) :
    (inet_address_mask_t, GError.t) result =
...
let new_from_string (mask_string : string) :
    (inet_address_mask_t, GError.t) result =
