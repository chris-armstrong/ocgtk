class type menu_item_t = object
  method get_attribute_value :
    string -> Gvariant_type.t option -> Gvariant.t option

  method get_link :
    string -> GMenu_link_iter_and__menu_model.menu_model_t option

  method set_action_and_target_value :
    string option -> Gvariant.t option -> unit
  ...
  method set_link :
    string -> GMenu_link_iter_and__menu_model.menu_model_t option -> unit

  method set_section :
    GMenu_link_iter_and__menu_model.menu_model_t option -> unit
...
class menu_item (obj : Menu_item.t) : menu_item_t =
  object (self)
    method get_attribute_value :
        string -> Gvariant_type.t option -> Gvariant.t option =
    ...
    method get_link :
      ...
      fun link ->
        Option.map
          (fun ret -> new GMenu_link_iter_and__menu_model.menu_model ret)
          (Menu_item.get_link obj link)
    ...
    method set_attribute_value : string -> Gvariant.t option -> unit =
      fun attribute value -> Menu_item.set_attribute_value obj attribute value

    method set_detailed_action : string -> unit =
      fun detailed_action -> Menu_item.set_detailed_action obj detailed_action
    ...
    method set_label : string option -> unit =
      fun label -> Menu_item.set_label obj label
...
let new_from_model (model : GMenu_link_iter_and__menu_model.menu_model_t)
    (item_index : int) : menu_item_t =
  let model = model#as_menu_model in
  let obj_ = Menu_item.new_from_model model item_index in
  new menu_item obj_

let new_section (label : string option)
    (section : GMenu_link_iter_and__menu_model.menu_model_t) : menu_item_t =
  let section = section#as_menu_model in
  let obj_ = Menu_item.new_section label section in
  new menu_item obj_

let new_submenu (label : string option)
    (submenu : GMenu_link_iter_and__menu_model.menu_model_t) : menu_item_t =
