class type proxy_t = object
  method connect :
    ...
    GProxy_address.proxy_address_t ->
    GCancellable.cancellable_t option ->
    (GIo_stream.io_stream_t, GError.t) result

  method connect_finish :
    GAsync_result.async_result_t -> (GIo_stream.io_stream_t, GError.t) result
...
class proxy (obj : Proxy.t) : proxy_t =
  object (self)
    method connect :
        GIo_stream.io_stream_t ->
        GProxy_address.proxy_address_t ->
      ...
      fun connection proxy_address cancellable ->
        ...
        Result.map
          (fun ret -> new GIo_stream.io_stream ret)
          (Proxy.connect obj connection proxy_address cancellable)
    ...
    method connect_finish :
      ...
      fun result ->
        ...
        Result.map
          (fun ret -> new GIo_stream.io_stream ret)
          (Proxy.connect_finish obj result)

    method supports_hostname : unit -> bool =
      fun () -> Proxy.supports_hostname obj
