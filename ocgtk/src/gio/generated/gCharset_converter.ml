...
class charset_converter (obj : Charset_converter.t) : charset_converter_t =
  object (self)
    inherit GConverter.converter (Converter.from_gobject obj)
    inherit GInitable.initable (Initable.from_gobject obj)

    method get_num_fallbacks : unit -> int =
      fun () -> Charset_converter.get_num_fallbacks obj

    method get_use_fallback : unit -> bool =
      fun () -> Charset_converter.get_use_fallback obj

    method set_use_fallback : bool -> unit =
      fun use_fallback -> Charset_converter.set_use_fallback obj use_fallback
...
let new_ (to_charset : string) (from_charset : string) :
    (charset_converter_t, GError.t) result =
