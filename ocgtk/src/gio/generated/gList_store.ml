...
class list_store (obj : List_store.t) : list_store_t =
  object (self)
    ...
    method append : [ `object_ ] Gobject.obj -> unit =
      fun item -> List_store.append obj item

    method insert : int -> [ `object_ ] Gobject.obj -> unit =
      fun position item -> List_store.insert obj position item

    method remove : int -> unit = fun position -> List_store.remove obj position
    method remove_all : unit -> unit = fun () -> List_store.remove_all obj
    ...
    method item_type = List_store.get_item_type obj
    method n_items = List_store.get_n_items obj
    method as_list_store = obj
  end

let new_ (item_type : Gobject.Type.t) : list_store_t =
  let obj_ = List_store.new_ item_type in
  new list_store obj_
