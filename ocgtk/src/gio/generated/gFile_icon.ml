...
class file_icon (obj : File_icon.t) : file_icon_t =
  object (self)
    inherit GIcon.icon (Icon.from_gobject obj)
    inherit GLoadable_icon.loadable_icon (Loadable_icon.from_gobject obj)

    method get_file : unit -> GApp_info_cycle_64c425a0.file_t =
      fun () -> new GApp_info_cycle_64c425a0.file (File_icon.get_file obj)

    method as_file_icon = obj
  end

let new_ (file : GApp_info_cycle_64c425a0.file_t) : file_icon_t =
