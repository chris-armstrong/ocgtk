...
class unix_mount_monitor (obj : Unix_mount_monitor.t) : unix_mount_monitor_t =
  object (self)
    method on_mountpoints_changed ?(after = false) ~callback () =
      Unix_mount_monitor.on_mountpoints_changed ~after
        self#as_unix_mount_monitor ~callback

    method on_mounts_changed ?(after = false) ~callback () =
      Unix_mount_monitor.on_mounts_changed ~after self#as_unix_mount_monitor
        ~callback
