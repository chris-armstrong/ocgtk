...
class simple_io_stream (obj : Simple_io_stream.t) : simple_io_stream_t =
  object (self)
    inherit GIo_stream.io_stream (obj :> Io_stream.t)
    method as_simple_io_stream = obj
  end

let new_ (input_stream : GInput_stream.input_stream_t)
    (output_stream : GOutput_stream.output_stream_t) : simple_io_stream_t =
