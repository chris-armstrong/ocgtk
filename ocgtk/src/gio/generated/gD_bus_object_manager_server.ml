class type d_bus_object_manager_server_t = object
  ...
  method export_uniquely :
    GD_bus_object_skeleton.d_bus_object_skeleton_t -> unit
...
class d_bus_object_manager_server (obj : D_bus_object_manager_server.t) :
  d_bus_object_manager_server_t =
  object (self)
    inherit
    ...
    method get_connection : unit -> GD_bus_connection.d_bus_connection_t option
      ...
      fun () ->
        Option.map
          ...
          (D_bus_object_manager_server.get_connection obj)
    ...
    method set_connection : GD_bus_connection.d_bus_connection_t option -> unit
      ...
      fun connection ->
        let connection =
          Option.map (fun c -> c#as_d_bus_connection) connection
        in
        D_bus_object_manager_server.set_connection obj connection

    method unexport : string -> bool =
      fun object_path -> D_bus_object_manager_server.unexport obj object_path
