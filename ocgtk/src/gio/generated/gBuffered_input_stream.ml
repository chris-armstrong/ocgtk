class type buffered_input_stream_t = object
  ...
  method fill :
    int -> GCancellable.cancellable_t option -> (int, GError.t) result
...
class buffered_input_stream (obj : Buffered_input_stream.t) :
  buffered_input_stream_t =
  object (self)
    inherit
      GFilter_input_stream.filter_input_stream (obj :> Filter_input_stream.t)
    ...
    method get_available : unit -> Gsize.t =
      fun () -> Buffered_input_stream.get_available obj

    method get_buffer_size : unit -> Gsize.t =
      fun () -> Buffered_input_stream.get_buffer_size obj
    ...
    method set_buffer_size : Gsize.t -> unit =
      fun size -> Buffered_input_stream.set_buffer_size obj size
...
let new_sized (base_stream : GInput_stream.input_stream_t) (size : Gsize.t) :
    buffered_input_stream_t =
  let base_stream = base_stream#as_input_stream in
