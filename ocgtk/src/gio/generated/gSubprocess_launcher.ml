...
class subprocess_launcher (obj : Subprocess_launcher.t) : subprocess_launcher_t
  =
  object (self)
    method close : unit -> unit = fun () -> Subprocess_launcher.close obj

    method getenv : string -> string option =
      fun variable -> Subprocess_launcher.getenv obj variable

    method set_cwd : string -> unit =
      fun cwd -> Subprocess_launcher.set_cwd obj cwd

    method set_environ : string array -> unit =
      fun env -> Subprocess_launcher.set_environ obj env

    method set_flags : Gio_enums.subprocessflags -> unit =
      fun flags -> Subprocess_launcher.set_flags obj flags

    method set_stderr_file_path : string option -> unit =
      fun path -> Subprocess_launcher.set_stderr_file_path obj path

    method set_stdin_file_path : string option -> unit =
      fun path -> Subprocess_launcher.set_stdin_file_path obj path

    method set_stdout_file_path : string option -> unit =
      fun path -> Subprocess_launcher.set_stdout_file_path obj path
    ...
    method spawnv : string array -> (GSubprocess.subprocess_t, GError.t) result
      ...
      fun argv ->
        Result.map
          (fun ret -> new GSubprocess.subprocess ret)
          (Subprocess_launcher.spawnv obj argv)
    ...
    method take_stderr_fd : int -> unit =
      fun fd -> Subprocess_launcher.take_stderr_fd obj fd

    method take_stdin_fd : int -> unit =
      fun fd -> Subprocess_launcher.take_stdin_fd obj fd

    method take_stdout_fd : int -> unit =
      fun fd -> Subprocess_launcher.take_stdout_fd obj fd

    method unsetenv : string -> unit =
      fun variable -> Subprocess_launcher.unsetenv obj variable

    method as_subprocess_launcher = obj
  end

let new_ (flags : Gio_enums.subprocessflags) : subprocess_launcher_t =
  let obj_ = Subprocess_launcher.new_ flags in
  new subprocess_launcher obj_
