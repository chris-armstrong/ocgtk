...
type bustype =
  [ (** An alias for the message bus that activated the process, if any. *)
    `STARTER
    (** Not a message bus. *)
  | `NONE  (** The system-wide message bus. *)
  | `SYSTEM  (** The login session message bus. *)
  | `SESSION ]
...
type converterresult =
  ...
  | `CONVERTED  (** The conversion is finished *)
  | `FINISHED  (** Flushing is finished *)
  | `FLUSHED ]
...
type credentialstype =
  ...
  | `LINUX_UCRED  (** The native credentials type is a `struct cmsgcred`. *)
  | `FREEBSD_CMSGCRED
...
type dbuserror =
  ...
  | `IO_ERROR  (** A D-Bus bus address was malformed. *)
  | `BAD_ADDRESS
    (** Requested operation isn't supported (like ENOSYS on UNIX). *)
  | `NOT_SUPPORTED  (** Some limited resource is exhausted. *)
  | `LIMITS_EXCEEDED
    (** Security restrictions don't allow doing what you're trying to do. *)
  | `ACCESS_DENIED  (** Authentication didn't work. *)
  | `AUTH_FAILED
  ...
  | `TIMEOUT  (** No network access (probably ENETUNREACH on a socket). *)
  | `NO_NETWORK
    (** Can't bind a socket since its address is in use (i.e. EADDRINUSE). *)
  | `ADDRESS_IN_USE
    (** The connection is disconnected and you're trying to use it. *)
  | `DISCONNECTED  (** Invalid arguments passed to a method call. *)
  | `INVALID_ARGS  (** Missing file. *)
  | `FILE_NOT_FOUND
  ...
  | `TIMED_OUT  (** Tried to remove or modify a match rule that didn't exist. *)
  | `MATCH_RULE_NOT_FOUND  (** The match rule isn't syntactically valid. *)
  | `MATCH_RULE_INVALID
  ...
  | `SPAWN_FAILED  (** We failed to setup the environment correctly. *)
  | `SPAWN_SETUP_FAILED  (** We failed to setup the config parser correctly. *)
  | `SPAWN_CONFIG_INVALID  (** Bus name was not valid. *)
  ...
  | `SPAWN_NO_MEMORY
    (** Tried to get a UNIX process ID and it wasn't available. *)
  | `UNIX_PROCESS_ID_UNKNOWN  (** A type signature is not valid. *)
  | `INVALID_SIGNATURE
  ...
  | `UNKNOWN_PROPERTY  (** Property you tried to set is read-only. Since 2.42 *)
  | `PROPERTY_READ_ONLY ]
...
type dbusmessageheaderfield =
  [ (** Not a valid header field. *)
    `INVALID
    (** The object path. *)
  | `PATH  (** The interface name. *)
  | `INTERFACE  (** The method or signal name. *)
  | `MEMBER  (** The name of the error that occurred. *)
  | `ERROR_NAME  (** The serial number the message is a reply to. *)
  | `REPLY_SERIAL  (** The name the message is intended for. *)
  | `DESTINATION
    (** Unique name of the sender of the message (filled in by the bus). *)
  | `SENDER  (** The signature of the message body. *)
  | `SIGNATURE
...
type dbusmessagetype =
  [ (** Message is of invalid type. *)
    `INVALID
    (** Method call. *)
  | `METHOD_CALL  (** Method reply. *)
  | `METHOD_RETURN  (** Error reply. *)
...
type datastreamnewlinetype =
  ...
  | `CR  (** Selects "CR, LF" line ending, common on Microsoft Windows. *)
  | `CR_LF  (** Automatically try to handle any line ending type. *)
  | `ANY ]
...
type emblemorigin =
  ...
  | `DEVICE  (** Emblem depicts live metadata, such as "readonly" *)
  | `LIVEMETADATA
...
type fileattributestatus =
  ...
  | `SET  (** Indicates an error in setting the value. *)
  | `ERROR_SETTING ]
...
type fileattributetype =
  ...
  | `STRING  (** a zero terminated string of non-zero bytes. *)
  | `BYTE_STRING  (** a boolean value. *)
  | `BOOLEAN  (** an unsigned 4-byte/32-bit integer. *)
  | `UINT32  (** a signed 4-byte/32-bit integer. *)
  | `INT32  (** an unsigned 8-byte/64-bit integer. *)
  | `UINT64  (** a signed 8-byte/64-bit integer. *)
  | `INT64  (** a #GObject. *)
  | `OBJECT  (** a %NULL terminated char **. Since 2.22 *)
  | `STRINGV ]
...
type filemonitorevent =
  ...
  | `CHANGES_DONE_HINT  (** a file was deleted. *)
  | `DELETED  (** a file was created. *)
  | `CREATED  (** a file attribute was changed. *)
  | `ATTRIBUTE_CHANGED  (** the file location will soon be unmounted. *)
  | `PRE_UNMOUNT  (** the file location was unmounted. *)
  | `UNMOUNTED
...
type filetype =
  ...
  | `REGULAR  (** File handle represents a directory. *)
  | `DIRECTORY  (** File handle represents a symbolic link (Unix systems). *)
  | `SYMBOLIC_LINK
    (** File is a "special" file, such as a socket, fifo, block device, or
        character device. *)
  | `SPECIAL  (** File is a shortcut (Windows systems). *)
  | `SHORTCUT  (** File is a mountable location. *)
  | `MOUNTABLE ]
...
type ioerrorenum =
  [ (** Generic error condition for when an operation fails and no more specific
    ...
    `FAILED
    (** File not found. *)
  | `NOT_FOUND  (** File already exists. *)
  | `EXISTS  (** File is a directory. *)
  | `IS_DIRECTORY  (** File is not a directory. *)
  | `NOT_DIRECTORY  (** File is a directory that isn't empty. *)
  | `NOT_EMPTY  (** File is not a regular file. *)
  | `NOT_REGULAR_FILE  (** File is not a symbolic link. *)
  | `NOT_SYMBOLIC_LINK  (** File cannot be mounted. *)
  | `NOT_MOUNTABLE_FILE  (** Filename is too many characters. *)
  | `FILENAME_TOO_LONG
    (** Filename is invalid or contains invalid characters. *)
  | `INVALID_FILENAME  (** File contains too many symbolic links. *)
  | `TOO_MANY_LINKS  (** No space left on drive. *)
  | `NO_SPACE  (** Invalid argument. *)
  | `INVALID_ARGUMENT  (** Permission denied. *)
  | `PERMISSION_DENIED
    (** Operation (or one of its parameters) not supported *)
  | `NOT_SUPPORTED  (** File isn't mounted. *)
  | `NOT_MOUNTED  (** File is already mounted. *)
  | `ALREADY_MOUNTED  (** File was closed. *)
  | `CLOSED  (** Operation was cancelled. See #GCancellable. *)
  | `CANCELLED  (** Operations are still pending. *)
  | `PENDING  (** File is read only. *)
  | `READ_ONLY  (** Backup couldn't be created. *)
  | `CANT_CREATE_BACKUP  (** File's Entity Tag was incorrect. *)
  | `WRONG_ETAG  (** Operation timed out. *)
  | `TIMED_OUT  (** Operation would be recursive. *)
  | `WOULD_RECURSE  (** File is busy. *)
  | `BUSY  (** Operation would block. *)
  | `WOULD_BLOCK  (** Host couldn't be found (remote operations). *)
  | `HOST_NOT_FOUND  (** Operation would merge files. *)
  | `WOULD_MERGE
  ...
  | `TOO_MANY_OPEN_FILES  (** The object has not been initialized. Since 2.22 *)
  | `NOT_INITIALIZED  (** The requested address is already in use. Since 2.22 *)
  | `ADDRESS_IN_USE  (** Need more input to finish operation. Since 2.24 *)
  | `PARTIAL_INPUT  (** The input data was invalid. Since 2.24 *)
  | `INVALID_DATA
    (** A remote object generated an error that doesn't correspond to a locally
        ...
        extract the D-Bus error name and g_dbus_error_strip_remote_error() to
        fix up the message so it matches what was received on the wire. Since
        2.26. *)
  | `DBUS_ERROR  (** Host unreachable. Since 2.26 *)
  | `HOST_UNREACHABLE  (** Network unreachable. Since 2.26 *)
  | `NETWORK_UNREACHABLE  (** Connection refused. Since 2.26 *)
  | `CONNECTION_REFUSED  (** Connection to proxy server failed. Since 2.26 *)
  | `PROXY_FAILED  (** Proxy authentication failed. Since 2.26 *)
  | `PROXY_AUTH_FAILED  (** Proxy server needs authentication. Since 2.26 *)
  | `PROXY_NEED_AUTH
    (** Proxy connection is not allowed by ruleset. Since 2.26 *)
  | `PROXY_NOT_ALLOWED  (** Broken pipe. Since 2.36 *)
  | `BROKEN_PIPE
    (** Connection closed by peer. Note that this is the same code as
        ...
        returned %G_IO_ERROR_BROKEN_PIPE, but others returned
        %G_IO_ERROR_FAILED. Now they should all return the same value, which has
        this more logical name. Since 2.44. *)
  | `CONNECTION_CLOSED  (** Transport endpoint is not connected. Since 2.44 *)
  | `NOT_CONNECTED  (** Message too large. Since 2.48. *)
  | `MESSAGE_TOO_LARGE  (** No such device found. Since 2.74 *)
  | `NO_SUCH_DEVICE  (** Destination address unset. Since 2.80 *)
  | `DESTINATION_UNSET ]
...
type mountoperationresult =
  ...
  | `ABORTED  (** The request was unhandled (i.e. not implemented) *)
  | `UNHANDLED ]
...
type notificationpriority =
  [ (** the default priority, to be used for the majority of notifications (for
    ...
    (** for notifications that do not require immediate attention - typically
        used for contextual background information, such as contact birthdays or
        local weather *)
  | `LOW
...
type passwordsave =
  ...
  | `FOR_SESSION  (** save a password permanently. *)
  | `PERMANENTLY ]
...
type resolvererror =
  [ (** the requested name/address/service was not found *)
    `NOT_FOUND
    (** the requested information could not be looked up due to a network error
        or similar problem *)
  | `TEMPORARY_FAILURE  (** unknown error *)
  | `INTERNAL ]
...
type resolverrecordtype =
  ...
  | `MX  (** look up DNS TXT records for a name *)
  | `TXT  (** look up DNS SOA records for a zone *)
  | `SOA  (** look up DNS NS records for a domain *)
  | `NS ]
...
type socketclientevent =
  ...
  | `RESOLVED
    (** The client is connecting to a remote host (either a proxy or the
        destination server). *)
  | `CONNECTING  (** The client has connected to a remote host. *)
  | `CONNECTED
    (** The client is negotiating with a proxy to connect to the destination
        server. *)
  | `PROXY_NEGOTIATING  (** The client has negotiated with the proxy server. *)
  | `PROXY_NEGOTIATED  (** The client is performing a TLS handshake. *)
  | `TLS_HANDSHAKING  (** The client has performed a TLS handshake. *)
  | `TLS_HANDSHAKED
...
type socketfamily =
  ...
  | `UNIX  (** the IPv4 family *)
  | `IPV4  (** the IPv6 family *)
  | `IPV6 ]
...
type socketlistenerevent =
  ...
  | `BOUND  (** The listener is about to start listening on this socket. *)
  | `LISTENING  (** The listener is now listening on this socket. *)
  | `LISTENED ]
...
type sockettype =
  ...
  | `STREAM  (** Connectionless, unreliable datagram passing. (e.g. UDP) *)
  | `DATAGRAM
...
type tlsauthenticationmode =
  ...
  | `REQUESTED  (** client authentication is required *)
  | `REQUIRED ]
...
type tlschannelbindingerror =
  ...
  | `NOT_AVAILABLE
    (** Binding type is not supported on the current connection. This error
        could be triggered when requesting `tls-server-end-point` binding data
        for a certificate which has no hash function or uses multiple hash
        functions. *)
  | `NOT_SUPPORTED
...
type tlserror =
  ...
  | `CERTIFICATE_REQUIRED
    (** The TLS connection was closed without proper notice, which may indicate
...
type tlsprotocolversion =
  ...
  | `SSL_3_0  (** TLS 1.0, which is insecure and should not be used *)
  | `TLS_1_0  (** TLS 1.1, which is insecure and should not be used *)
  | `TLS_1_1
    (** TLS 1.2, defined by
  ...
  | `TLS_1_3  (** DTLS 1.0, which is insecure and should not be used *)
  | `DTLS_1_0
    (** DTLS 1.2, defined by
...
type tlsrehandshakemode =
  ...
  | `SAFELY  (** Allow unsafe rehandshaking *)
  | `UNSAFELY ]
...
type unixsocketaddresstype =
  [ (** invalid *)
    `INVALID
    (** anonymous *)
  | `ANONYMOUS  (** a filesystem path *)
  | `PATH  (** an abstract name *)
  | `ABSTRACT
...
type zlibcompressorformat =
  [ (** deflate compression with zlib header *)
    `ZLIB
    (** gzip file format *)
  | `GZIP  (** deflate compression with no header *)
  | `RAW ]
...
type appinfocreateflags_flag =
  ...
  | `NEEDS_TERMINAL  (** Application supports URI arguments. *)
  | `SUPPORTS_URIS  (** Application supports startup notification. Since 2.26 *)
...
type applicationflags_flag =
  ...
  | `IS_SERVICE  (** Don't try to become the primary instance. *)
  | `IS_LAUNCHER
...
type askpasswordflags_flag =
  ...
  | `NEED_USERNAME  (** operation requires a domain. *)
  | `NEED_DOMAIN  (** operation supports saving settings. *)
  | `SAVING_SUPPORTED  (** operation supports anonymous users. *)
  | `ANONYMOUS_SUPPORTED  (** operation takes TCRYPT parameters (Since: 2.58) *)
  | `TCRYPT ]
...
type dbusconnectionflags_flag =
  ...
  | `AUTHENTICATION_CLIENT  (** Perform authentication against client. *)
...
type dbusserverflags_flag =
  ...
  | `RUN_IN_THREAD  (** Allow the anonymous authentication method. *)
  | `AUTHENTICATION_ALLOW_ANONYMOUS
...
type fileattributeinfoflags_flag =
  ...
  | `COPY_WITH_FILE  (** copy the attribute values when the file is moved. *)
  | `COPY_WHEN_MOVED ]
...
type filecopyflags_flag =
  ...
  | `OVERWRITE  (** Make a backup of any existing files. *)
  | `BACKUP  (** Don't follow symlinks. *)
  | `NOFOLLOW_SYMLINKS
...
type iostreamspliceflags_flag =
  ...
  | `CLOSE_STREAM1  (** Close the second stream after the splice. *)
  | `CLOSE_STREAM2
...
type outputstreamspliceflags_flag =
  ...
  | `CLOSE_SOURCE  (** Close the target stream after the splice. *)
  | `CLOSE_TARGET ]
...
type resolvernamelookupflags_flag =
  ...
  | `IPV4_ONLY  (** only resolve ipv6 addresses *)
  | `IPV6_ONLY ]
...
type socketmsgflags_flag =
  ...
  | `OOB  (** Read data from the socket without removing it from the queue. *)
  | `PEEK
...
type subprocessflags_flag =
  ...
  | `STDIN_PIPE  (** stdin is inherited from the calling process. *)
  | `STDIN_INHERIT
...
type tlscertificateflags_flag =
  ...
  | `NOT_ACTIVATED  (** The certificate has expired *)
  | `EXPIRED
    (** The certificate has been revoked according to the #GTlsConnection's
        certificate revocation list. *)
  | `REVOKED  (** The certificate's algorithm is considered insecure. *)
  | `INSECURE  (** Some other error occurred validating the certificate *)
  | `GENERIC_ERROR  (** the combination of all of the above flags *)
  | `VALIDATE_ALL ]
...
type tlspasswordflags_flag =
  ...
  | `MANY_TRIES
    ...
    *)
  | `FINAL_TRY  (** For PKCS #11, the user PIN is required. Since: 2.70. *)
  | `PKCS11_USER
