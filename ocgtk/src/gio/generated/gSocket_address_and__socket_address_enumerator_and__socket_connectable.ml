...
and socket_address_enumerator_t = object
  method next :
    ...
    (socket_address_t option, GError.t) result

  method next_finish :
    GAsync_result.async_result_t -> (socket_address_t option, GError.t) result

  method as_socket_address_enumerator :
    Socket_address_and__socket_address_enumerator_and__socket_connectable
  ...
  object (self)
    method next :
      ...
      fun cancellable ->
        ...
        Result.map
          (fun ret -> Option.map (fun ret -> new socket_address ret) ret)
          (Socket_address_and__socket_address_enumerator_and__socket_connectable
           .Socket_address_enumerator
           .next obj cancellable)
