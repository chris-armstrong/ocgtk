...
class d_bus_connection (obj : D_bus_connection.t) : d_bus_connection_t =
  object (self)
    ...
    method call_finish :
      ...
      fun res ->
        ...
        D_bus_connection.call_finish obj res
    ...
    method call_sync :
        ...
        GCancellable.cancellable_t option ->
        (Gvariant.t, GError.t) result =
      fun bus_name object_path interface_name method_name parameters reply_type
          flags timeout_msec cancellable ->
        let cancellable = Option.map (fun c -> c#as_cancellable) cancellable in
        D_bus_connection.call_sync obj bus_name object_path interface_name
    ...
    method close_finish :
      ...
      fun res ->
        let res = res#as_async_result in
        D_bus_connection.close_finish obj res
    ...
    method emit_signal :
        ...
        Gvariant.t option ->
        (bool, GError.t) result =
      fun destination_bus_name object_path interface_name signal_name
          parameters ->
        D_bus_connection.emit_signal obj destination_bus_name object_path
          interface_name signal_name parameters

    method export_action_group :
        string -> GAction_group.action_group_t -> (int, GError.t) result =
      fun object_path action_group ->
        let action_group = action_group#as_action_group in
        D_bus_connection.export_action_group obj object_path action_group
    ...
    method flush_finish :
      ...
      fun res ->
        let res = res#as_async_result in
        D_bus_connection.flush_finish obj res
    ...
    method get_capabilities : unit -> Gio_enums.dbuscapabilityflags =
      fun () -> D_bus_connection.get_capabilities obj

    method get_exit_on_close : unit -> bool =
      fun () -> D_bus_connection.get_exit_on_close obj

    method get_flags : unit -> Gio_enums.dbusconnectionflags =
      fun () -> D_bus_connection.get_flags obj

    method get_guid : unit -> string = fun () -> D_bus_connection.get_guid obj

    method get_last_serial : unit -> UInt32.t =
      fun () -> D_bus_connection.get_last_serial obj

    method get_peer_credentials : unit -> GCredentials.credentials_t option =
      fun () ->
        Option.map
          (fun ret -> new GCredentials.credentials ret)
          (D_bus_connection.get_peer_credentials obj)

    method get_stream : unit -> GIo_stream.io_stream_t =
      fun () -> new GIo_stream.io_stream (D_bus_connection.get_stream obj)

    method get_unique_name : unit -> string option =
      fun () -> D_bus_connection.get_unique_name obj

    method is_closed : unit -> bool = fun () -> D_bus_connection.is_closed obj

    method remove_filter : int -> unit =
      fun filter_id -> D_bus_connection.remove_filter obj filter_id
    ...
    method send_message_with_reply_finish :
      ...
      fun res ->
        let res = res#as_async_result in
        Result.map
          (fun ret -> new GD_bus_message.d_bus_message ret)
          (D_bus_connection.send_message_with_reply_finish obj res)

    method set_exit_on_close : bool -> unit =
      fun exit_on_close -> D_bus_connection.set_exit_on_close obj exit_on_close
    ...
    method start_message_processing : unit -> unit =
      fun () -> D_bus_connection.start_message_processing obj

    method unexport_action_group : int -> unit =
      fun export_id -> D_bus_connection.unexport_action_group obj export_id

    method unexport_menu_model : int -> unit =
      fun export_id -> D_bus_connection.unexport_menu_model obj export_id
...
let new_for_address_finish (res : GAsync_result.async_result_t) :
    (d_bus_connection_t, GError.t) result =
  let res = res#as_async_result in
  let obj_ = D_bus_connection.new_for_address_finish res in
  Result.map (fun obj_ -> new d_bus_connection obj_) obj_

let new_for_address_sync (address : string)
    (flags : Gio_enums.dbusconnectionflags)
    (observer : GD_bus_auth_observer.d_bus_auth_observer_t option)
    (cancellable : GCancellable.cancellable_t option) :
    (d_bus_connection_t, GError.t) result =
  let observer = Option.map (fun c -> c#as_d_bus_auth_observer) observer in
  let cancellable = Option.map (fun c -> c#as_cancellable) cancellable in
  let obj_ =
    D_bus_connection.new_for_address_sync address flags observer cancellable
  in
  Result.map (fun obj_ -> new d_bus_connection obj_) obj_

let new_sync (stream : GIo_stream.io_stream_t) (guid : string option)
    (flags : Gio_enums.dbusconnectionflags)
    (observer : GD_bus_auth_observer.d_bus_auth_observer_t option)
    (cancellable : GCancellable.cancellable_t option) :
    (d_bus_connection_t, GError.t) result =
