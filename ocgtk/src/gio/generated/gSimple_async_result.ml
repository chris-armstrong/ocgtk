...
class simple_async_result (obj : Simple_async_result.t) : simple_async_result_t
  =
  object (self)
    inherit GAsync_result.async_result (Async_result.from_gobject obj)
    method complete : unit -> unit = fun () -> Simple_async_result.complete obj

    method complete_in_idle : unit -> unit =
      fun () -> Simple_async_result.complete_in_idle obj

    method get_op_res_gboolean : unit -> bool =
      fun () -> Simple_async_result.get_op_res_gboolean obj

    method get_op_res_gssize : unit -> int =
      fun () -> Simple_async_result.get_op_res_gssize obj

    method propagate_error : unit -> (bool, GError.t) result =
      fun () -> Simple_async_result.propagate_error obj

    method set_check_cancellable : GCancellable.cancellable_t option -> unit =
      fun check_cancellable ->
        let check_cancellable =
          Option.map (fun c -> c#as_cancellable) check_cancellable
        in
        Simple_async_result.set_check_cancellable obj check_cancellable

    method set_from_error : GError.t -> unit =
      fun error -> Simple_async_result.set_from_error obj error
    ...
    method set_op_res_gboolean : bool -> unit =
      fun op_res -> Simple_async_result.set_op_res_gboolean obj op_res

    method set_op_res_gssize : int -> unit =
      fun op_res -> Simple_async_result.set_op_res_gssize obj op_res
