...
class buffered_output_stream (obj : Buffered_output_stream.t) :
  buffered_output_stream_t =
  object (self)
    inherit
    ...
    method get_auto_grow : unit -> bool =
      fun () -> Buffered_output_stream.get_auto_grow obj

    method get_buffer_size : unit -> Gsize.t =
      fun () -> Buffered_output_stream.get_buffer_size obj

    method set_auto_grow : bool -> unit =
      fun auto_grow -> Buffered_output_stream.set_auto_grow obj auto_grow

    method set_buffer_size : Gsize.t -> unit =
      fun size -> Buffered_output_stream.set_buffer_size obj size

    method as_buffered_output_stream = obj
  end

let new_ (base_stream : GOutput_stream.output_stream_t) :
    buffered_output_stream_t =
...
let new_sized (base_stream : GOutput_stream.output_stream_t) (size : Gsize.t) :
    buffered_output_stream_t =
  let base_stream = base_stream#as_output_stream in
