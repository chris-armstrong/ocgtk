class type menu_t = object
  ...
  method append_section :
    string option -> GMenu_link_iter_and__menu_model.menu_model_t -> unit

  method append_submenu :
    string option -> GMenu_link_iter_and__menu_model.menu_model_t -> unit
  ...
  method insert_section :
    int -> string option -> GMenu_link_iter_and__menu_model.menu_model_t -> unit
  ...
  method prepend_section :
    string option -> GMenu_link_iter_and__menu_model.menu_model_t -> unit

  method prepend_submenu :
    string option -> GMenu_link_iter_and__menu_model.menu_model_t -> unit
...
class menu (obj : Menu.t) : menu_t =
  object (self)
    inherit
      GMenu_link_iter_and__menu_model.menu_model
        (obj :> Menu_link_iter_and__menu_model.Menu_model.t)

    method append : string option -> string option -> unit =
      fun label detailed_action -> Menu.append obj label detailed_action
    ...
    method freeze : unit -> unit = fun () -> Menu.freeze obj
    ...
    method prepend : string option -> string option -> unit =
      fun label detailed_action -> Menu.prepend obj label detailed_action
    ...
    method remove : int -> unit = fun position -> Menu.remove obj position
    method remove_all : unit -> unit = fun () -> Menu.remove_all obj
    method as_menu = obj
  end

let new_ () : menu_t = new menu (Menu.new_ ())
