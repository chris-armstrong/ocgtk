...
class d_bus_interface_skeleton (obj : D_bus_interface_skeleton.t) :
  d_bus_interface_skeleton_t =
  object (self)
    inherit
      GD_bus_interface_and__d_bus_object.d_bus_interface
        (D_bus_interface_and__d_bus_object.D_bus_interface.from_gobject obj)

    method on_g_authorize_method ~callback =
      D_bus_interface_skeleton.on_g_authorize_method
        self#as_d_bus_interface_skeleton ~callback
    ...
    method flush : unit -> unit = fun () -> D_bus_interface_skeleton.flush obj
    ...
    method get_connection : unit -> GD_bus_connection.d_bus_connection_t option
      ...
      fun () ->
        Option.map
          ...
          (D_bus_interface_skeleton.get_connection obj)

    method get_connections : unit -> GD_bus_connection.d_bus_connection_t list =
      fun () ->
        (List.map (fun ret -> new GD_bus_connection.d_bus_connection ret))
          (D_bus_interface_skeleton.get_connections obj)

    method get_flags : unit -> Gio_enums.dbusinterfaceskeletonflags =
      fun () -> D_bus_interface_skeleton.get_flags obj

    method get_object_path : unit -> string option =
      fun () -> D_bus_interface_skeleton.get_object_path obj

    method get_properties : unit -> Gvariant.t =
      fun () -> D_bus_interface_skeleton.get_properties obj

    method get_vtable : unit -> D_bus_interface_v_table.t =
      fun () -> D_bus_interface_skeleton.get_vtable obj
    ...
    method set_flags : Gio_enums.dbusinterfaceskeletonflags -> unit =
      fun flags -> D_bus_interface_skeleton.set_flags obj flags

    method unexport : unit -> unit =
      fun () -> D_bus_interface_skeleton.unexport obj

    method unexport_from_connection :
        GD_bus_connection.d_bus_connection_t -> unit =
      fun connection ->
        let connection = connection#as_d_bus_connection in
