...
class button (obj : Button.t) : button_t = object (self)
  ...
  method get_child : unit -> GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.widget_t option =
    fun () ->
      Option.map (fun ret -> new GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.widget ret) (Button.get_child obj)

  method get_has_frame : unit -> bool =
    fun () ->
      (Button.get_has_frame obj)

  method get_icon_name : unit -> string option =
    fun () ->
      (Button.get_icon_name obj)

  method get_label : unit -> string option =
    fun () ->
      (Button.get_label obj)

  method get_use_underline : unit -> bool =
    fun () ->
      (Button.get_use_underline obj)
  ...
  method set_has_frame : bool -> unit =
    fun has_frame ->
      (Button.set_has_frame obj has_frame)
