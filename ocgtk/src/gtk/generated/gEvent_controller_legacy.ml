class type event_controller_legacy_t = object
    ...
    method on_event : ?after:bool -> callback:(event:Ocgtk_gdk.Gdk.Event.event_t -> bool) -> unit -> Gobject.Signal.handler_id
...
class event_controller_legacy (obj : Event_controller_legacy.t) : event_controller_legacy_t = object (self)
  ...
  method on_event ?(after = false) ~callback () =
    Event_controller_legacy.on_event ~after self#as_event_controller_legacy
      ~callback:(fun ~event -> callback ~event:(new Ocgtk_gdk.Gdk.Event.event event))
