...
external get_offset_value : t -> string option -> bool * float = "ml_gtk_level_bar_get_offset_value"
...
let on_offset_changed ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let name = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
    callback ~name)
 in
