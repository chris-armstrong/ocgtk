...
let on_begin_print ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let context = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    callback ~context)
 in
...
let on_custom_widget_apply ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let widget = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    callback ~widget)
 in
...
let on_done_ ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let result = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.printoperationresult_of_int (Gobject.Value.get_enum_int v)) in
    callback ~result)
 in
...
let on_draw_page ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let context = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    let page_nr = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
...
let on_preview ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let preview = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    let context = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_object_exn v) in
    let parent = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_object v) in
...
let on_request_page_setup ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let context = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    let page_nr = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
    let setup = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_object_exn v) in
...
let on_update_custom_widget ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let widget = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    let setup = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_object_exn v) in
    let settings = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_object_exn v) in
