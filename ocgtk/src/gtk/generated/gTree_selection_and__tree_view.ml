...
and tree_view_t = object
    ...
    method on_columns_changed : ?after:bool -> callback:(unit -> unit) -> unit -> Gobject.Signal.handler_id
    method on_cursor_changed : ?after:bool -> callback:(unit -> unit) -> unit -> Gobject.Signal.handler_id
    method on_expand_collapse_cursor_row : ?after:bool -> callback:(object_:bool -> p0:bool -> p1:bool -> bool) -> unit -> Gobject.Signal.handler_id
    method on_move_cursor : ?after:bool -> callback:(step:Gtk_enums.movementstep -> direction:int -> extend:bool -> modify:bool -> bool) -> unit -> Gobject.Signal.handler_id
    method on_select_all : ?after:bool -> callback:(unit -> bool) -> unit -> Gobject.Signal.handler_id
    method on_select_cursor_parent : ?after:bool -> callback:(unit -> bool) -> unit -> Gobject.Signal.handler_id
    method on_select_cursor_row : ?after:bool -> callback:(object_:bool -> bool) -> unit -> Gobject.Signal.handler_id
    method on_start_interactive_search : ?after:bool -> callback:(unit -> bool) -> unit -> Gobject.Signal.handler_id
    method on_toggle_cursor_row : ?after:bool -> callback:(unit -> bool) -> unit -> Gobject.Signal.handler_id
    method on_unselect_all : ?after:bool -> callback:(unit -> bool) -> unit -> Gobject.Signal.handler_id
    ...
    method set_tooltip_cell : GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.tooltip_t -> Tree_path.t option -> GTree_view_column.tree_view_column_t option -> GCell_renderer.cell_renderer_t option -> unit
    method set_tooltip_column : int -> unit
    method set_tooltip_row : GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.tooltip_t -> Tree_path.t -> unit
...
class tree_selection (obj : Tree_selection_and__tree_view.Tree_selection.t) : tree_selection_t = object (self)
  method on_changed ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_selection.on_changed ~after self#as_tree_selection ~callback
...
and tree_view (obj : Tree_selection_and__tree_view.Tree_view.t) : tree_view_t = object (self)
  ...
  method on_columns_changed ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_columns_changed ~after self#as_tree_view ~callback

  method on_cursor_changed ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_cursor_changed ~after self#as_tree_view ~callback

  method on_expand_collapse_cursor_row ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_expand_collapse_cursor_row ~after self#as_tree_view ~callback

  method on_move_cursor ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_move_cursor ~after self#as_tree_view ~callback

  method on_select_all ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_select_all ~after self#as_tree_view ~callback

  method on_select_cursor_parent ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_select_cursor_parent ~after self#as_tree_view ~callback

  method on_select_cursor_row ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_select_cursor_row ~after self#as_tree_view ~callback

  method on_start_interactive_search ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_start_interactive_search ~after self#as_tree_view ~callback

  method on_toggle_cursor_row ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_toggle_cursor_row ~after self#as_tree_view ~callback

  method on_unselect_all ?(after = false) ~callback () =
    Tree_selection_and__tree_view.Tree_view.on_unselect_all ~after self#as_tree_view ~callback
