...
class check_button (obj : Check_button.t) : check_button_t =
  object (self)
    inherit
      GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget
      .widget
        ...
        Option.map
          (fun ret ->
            new
              ...
              .widget
              ret)
          (Check_button.get_child obj)

    method get_inconsistent : unit -> bool =
      fun () -> Check_button.get_inconsistent obj

    method get_label : unit -> string option =
      fun () -> Check_button.get_label obj

    method get_use_underline : unit -> bool =
      fun () -> Check_button.get_use_underline obj

    method set_active : bool -> unit =
      fun setting -> Check_button.set_active obj setting
    ...
    method set_inconsistent : bool -> unit =
      fun inconsistent -> Check_button.set_inconsistent obj inconsistent

    method set_label : string option -> unit =
      fun label -> Check_button.set_label obj label

    method set_use_underline : bool -> unit =
      fun setting -> Check_button.set_use_underline obj setting

    method as_check_button = obj
  end

let new_ () : check_button_t = new check_button (Check_button.new_ ())
