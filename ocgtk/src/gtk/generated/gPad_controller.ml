class type pad_controller_t = object
  ...
  method set_action :
    Gtk_enums.padactiontype -> int -> int -> string -> string -> unit
...
class pad_controller (obj : Pad_controller.t) : pad_controller_t =
  object (self)
    inherit
      GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget
      .event_controller
    ...
    method action_group =
      new Ocgtk_gio.Gio.Action_group.action_group
        (Pad_controller.get_action_group obj)

    method pad = new Ocgtk_gdk.Gdk.Device.device (Pad_controller.get_pad obj)
    method as_pad_controller = obj
  end

let new_ (group : Ocgtk_gio.Gio.Action_group.action_group_t)
    (pad : Ocgtk_gdk.Gdk.Device.device_t option) : pad_controller_t =
