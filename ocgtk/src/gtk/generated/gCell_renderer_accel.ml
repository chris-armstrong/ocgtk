class type cell_renderer_accel_t = object
    ...
    method on_accel_cleared : ?after:bool -> callback:(path_string:string -> unit) -> unit -> Gobject.Signal.handler_id
    method on_accel_edited : ?after:bool -> callback:(path_string:string -> accel_key:int -> accel_mods:Ocgtk_gdk.Gdk_enums.modifiertype -> hardware_keycode:int -> unit) -> unit -> Gobject.Signal.handler_id
...
class cell_renderer_accel (obj : Cell_renderer_accel.t) : cell_renderer_accel_t = object (self)
  ...
  method on_accel_cleared ?(after = false) ~callback () =
    Cell_renderer_accel.on_accel_cleared ~after self#as_cell_renderer_accel ~callback

  method on_accel_edited ?(after = false) ~callback () =
    Cell_renderer_accel.on_accel_edited ~after self#as_cell_renderer_accel ~callback
