class type text_tag_table_t = object
    ...
    method on_tag_added : ?after:bool -> callback:(tag:GText_tag.text_tag_t -> unit) -> unit -> Gobject.Signal.handler_id
    method on_tag_changed : ?after:bool -> callback:(tag:GText_tag.text_tag_t -> size_changed:bool -> unit) -> unit -> Gobject.Signal.handler_id
    method on_tag_removed : ?after:bool -> callback:(tag:GText_tag.text_tag_t -> unit) -> unit -> Gobject.Signal.handler_id
...
class text_tag_table (obj : Text_tag_table.t) : text_tag_table_t = object (self)
  ...
  method on_tag_changed ?(after = false) ~callback () =
    Text_tag_table.on_tag_changed ~after self#as_text_tag_table
      ~callback:(fun ~tag ~size_changed -> callback ~tag:(new GText_tag.text_tag tag) ~size_changed)
