...
class slice_list_model (obj : Slice_list_model.t) : slice_list_model_t =
  object (self)
    inherit
    ...
    method get_model : unit -> Ocgtk_gio.Gio.List_model.list_model_t option =
      fun () ->
        Option.map
          ...
          (Slice_list_model.get_model obj)
    ...
    method set_offset : int -> unit =
      fun offset -> Slice_list_model.set_offset obj offset

    method set_size : int -> unit =
      fun size -> Slice_list_model.set_size obj size
...
let new_ (model : Ocgtk_gio.Gio.List_model.list_model_t option) (offset : int)
    (size : int) : slice_list_model_t =
  let model = Option.map (fun c -> c#as_list_model) model in
