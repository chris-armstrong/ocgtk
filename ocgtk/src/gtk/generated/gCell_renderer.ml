...
class type cell_renderer_t = object
  ...
  method start_editing :
    ...
    Ocgtk_gdk.Gdk.Rectangle.rectangle_t ->
    Gtk_enums.cellrendererstate ->
    GCell_editable.cell_editable_t option
  ...
  object (self)
    ...
    method activate :
      ...
      fun event widget path background_area cell_area flags ->
        ...
        Cell_renderer.activate obj event widget path background_area cell_area
          flags

    method get_is_expanded : unit -> bool =
      fun () -> Cell_renderer.get_is_expanded obj

    method get_is_expander : unit -> bool =
      fun () -> Cell_renderer.get_is_expander obj

    method get_request_mode : unit -> Gtk_enums.sizerequestmode =
      fun () -> Cell_renderer.get_request_mode obj
    ...
    method is_activatable : unit -> bool =
      fun () -> Cell_renderer.is_activatable obj

    method set_alignment : float -> float -> unit =
      fun xalign yalign -> Cell_renderer.set_alignment obj xalign yalign

    method set_fixed_size : int -> int -> unit =
      fun width height -> Cell_renderer.set_fixed_size obj width height

    method set_is_expanded : bool -> unit =
      fun is_expanded -> Cell_renderer.set_is_expanded obj is_expanded

    method set_is_expander : bool -> unit =
      fun is_expander -> Cell_renderer.set_is_expander obj is_expander

    method set_padding : int -> int -> unit =
      fun xpad ypad -> Cell_renderer.set_padding obj xpad ypad

    method set_sensitive : bool -> unit =
      fun sensitive -> Cell_renderer.set_sensitive obj sensitive

    method set_visible : bool -> unit =
      fun visible -> Cell_renderer.set_visible obj visible
    ...
    method snapshot :
      ...
      fun snapshot widget background_area cell_area flags ->
        ...
        let background_area = background_area#as_rectangle in
        let cell_area = cell_area#as_rectangle in
        Cell_renderer.snapshot obj snapshot widget background_area cell_area
          flags
    ...
    method start_editing :
      ...
      fun event widget path background_area cell_area flags ->
        ...
        Option.map
          ...
          (Cell_renderer.start_editing obj event widget path background_area
             cell_area flags)

    method stop_editing : bool -> unit =
      fun canceled -> Cell_renderer.stop_editing obj canceled

    method set_cell_background v = Cell_renderer.set_cell_background obj v

    method cell_background_rgba =
      new Ocgtk_gdk.Gdk.Rgb_a.rgb_a (Cell_renderer.get_cell_background_rgba obj)

    method set_cell_background_rgba : Ocgtk_gdk.Gdk.Rgb_a.rgb_a_t -> unit =
      fun v -> Cell_renderer.set_cell_background_rgba obj v#as_rgb_a

    method cell_background_set = Cell_renderer.get_cell_background_set obj

    method set_cell_background_set v =
      Cell_renderer.set_cell_background_set obj v
