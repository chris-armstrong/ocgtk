...
external get_entry : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t = "ml_gtk_entry_completion_get_entry"
...
external compute_prefix : t -> string -> string option = "ml_gtk_entry_completion_compute_prefix"
...
let on_insert_prefix ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let prefix = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
