...
class drop_down (obj : Drop_down.t) : drop_down_t =
  object (self)
    ...
    method on_activate ~callback =
      Drop_down.on_activate self#as_drop_down ~callback
    ...
    method get_expression : unit -> GExpression.expression_t option =
      fun () ->
        Option.map
          ...
          (Drop_down.get_expression obj)

    method get_factory : unit -> GList_item_factory.list_item_factory_t option =
      fun () ->
        Option.map
          (fun ret -> new GList_item_factory.list_item_factory ret)
          (Drop_down.get_factory obj)
    ...
    method get_header_factory :
      ...
      fun () ->
        Option.map
          (fun ret -> new GList_item_factory.list_item_factory ret)
          (Drop_down.get_header_factory obj)
    ...
    method get_list_factory :
      ...
      fun () ->
        Option.map
          (fun ret -> new GList_item_factory.list_item_factory ret)
          (Drop_down.get_list_factory obj)

    method get_model : unit -> Ocgtk_gio.Gio.List_model.list_model_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gio.Gio.List_model.list_model ret)
          (Drop_down.get_model obj)

    method get_search_match_mode : unit -> Gtk_enums.stringfiltermatchmode =
      fun () -> Drop_down.get_search_match_mode obj

    method get_selected : unit -> int = fun () -> Drop_down.get_selected obj

    method get_selected_item : unit -> [ `object_ ] Gobject.obj option =
      fun () -> Drop_down.get_selected_item obj

    method get_show_arrow : unit -> bool =
      fun () -> Drop_down.get_show_arrow obj

    method set_enable_search : bool -> unit =
      fun enable_search -> Drop_down.set_enable_search obj enable_search
    ...
    method set_selected : int -> unit =
      fun position -> Drop_down.set_selected obj position

    method set_show_arrow : bool -> unit =
      fun show_arrow -> Drop_down.set_show_arrow obj show_arrow

    method as_drop_down = obj
  end

let new_ (model : Ocgtk_gio.Gio.List_model.list_model_t option)
    (expression : GExpression.expression_t option) : drop_down_t =
  let model = Option.map (fun c -> c#as_list_model) model in
  let expression = Option.map (fun c -> c#as_expression) expression in
