...
class color_dialog_button (obj : Color_dialog_button.t) : color_dialog_button_t
  =
  object (self)
    inherit
      GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget
    ...
    method on_activate ~callback =
      Color_dialog_button.on_activate self#as_color_dialog_button ~callback

    method get_dialog : unit -> GColor_dialog.color_dialog_t option =
      fun () ->
        Option.map
          (fun ret -> new GColor_dialog.color_dialog ret)
          (Color_dialog_button.get_dialog obj)

    method get_rgba : unit -> Ocgtk_gdk.Gdk.Rgb_a.rgb_a_t =
      fun () -> new Ocgtk_gdk.Gdk.Rgb_a.rgb_a (Color_dialog_button.get_rgba obj)
