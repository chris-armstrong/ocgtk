...
external set_mnemonic_widget : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_label_set_mnemonic_widget"
...
external get_mnemonic_widget : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option = "ml_gtk_label_get_mnemonic_widget"
...
let on_activate_link ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let uri = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
