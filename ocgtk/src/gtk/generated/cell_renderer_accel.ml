...
let on_accel_cleared ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let path_string = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
...
let on_accel_edited ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let path_string = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
    let accel_key = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_uint v) in
    let accel_mods = (let v = (Gobject.Closure.nth argv ~pos:3) in Ocgtk_gdk.Gdk_enums.modifiertype_of_int (Gobject.Value.get_flags_int v)) in
    let hardware_keycode = (let v = (Gobject.Closure.nth argv ~pos:4) in Gobject.Value.get_uint v) in
