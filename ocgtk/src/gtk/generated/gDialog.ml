...
class dialog (obj : Dialog.t) : dialog_t =
  object (self)
    inherit
    ...
    method get_content_area : unit -> GBox.box_t =
      fun () -> new GBox.box (Dialog.get_content_area obj)
    ...
    method response : int -> unit =
      fun response_id -> Dialog.response obj response_id

    method set_default_response : int -> unit =
      fun response_id -> Dialog.set_default_response obj response_id
...
let new_ () : dialog_t = new dialog (Dialog.new_ ())
