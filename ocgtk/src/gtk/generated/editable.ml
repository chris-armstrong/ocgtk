...
let on_delete_text ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let start_pos = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_int v) in
    let end_pos = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
