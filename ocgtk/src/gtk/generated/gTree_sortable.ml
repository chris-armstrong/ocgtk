class type tree_sortable_t = object
    method on_sort_column_changed : ?after:bool -> callback:(unit -> unit) -> unit -> Gobject.Signal.handler_id
...
class tree_sortable (obj : Tree_sortable.t) : tree_sortable_t = object (self)
  method on_sort_column_changed ?(after = false) ~callback () =
    Tree_sortable.on_sort_column_changed ~after self#as_tree_sortable ~callback
