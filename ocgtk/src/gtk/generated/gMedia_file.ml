...
class media_file (obj : Media_file.t) : media_file_t =
  object (self)
    ...
    method get_file : unit -> Ocgtk_gio.Gio.File.file_t option =
      fun () ->
        Option.map
          ...
          (Media_file.get_file obj)

    method get_input_stream :
        unit -> Ocgtk_gio.Gio.Input_stream.input_stream_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gio.Gio.Input_stream.input_stream ret)
          (Media_file.get_input_stream obj)
    ...
    method set_filename : string option -> unit =
      fun filename -> Media_file.set_filename obj filename
    ...
    method set_resource : string option -> unit =
      fun resource_path -> Media_file.set_resource obj resource_path

    method as_media_file = obj
  end

let new_ () : media_file_t = new media_file (Media_file.new_ ())
...
let new_for_input_stream (stream : Ocgtk_gio.Gio.Input_stream.input_stream_t) :
    media_file_t =
