...
class single_selection (obj : Single_selection.t) : single_selection_t =
  object (self)
    inherit
    ...
    method get_autoselect : unit -> bool =
      fun () -> Single_selection.get_autoselect obj

    method get_can_unselect : unit -> bool =
      fun () -> Single_selection.get_can_unselect obj

    method get_model : unit -> Ocgtk_gio.Gio.List_model.list_model_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gio.Gio.List_model.list_model ret)
          (Single_selection.get_model obj)

    method get_selected : unit -> int =
      fun () -> Single_selection.get_selected obj

    method get_selected_item : unit -> [ `object_ ] Gobject.obj option =
      fun () -> Single_selection.get_selected_item obj

    method set_autoselect : bool -> unit =
      fun autoselect -> Single_selection.set_autoselect obj autoselect

    method set_can_unselect : bool -> unit =
      fun can_unselect -> Single_selection.set_can_unselect obj can_unselect
    ...
    method set_selected : int -> unit =
      fun position -> Single_selection.set_selected obj position
