...
external set_client_widget : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_im_context_set_client_widget"
...
let on_commit ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let str = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
    callback ~str)
 in
...
let on_delete_surrounding ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let offset = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_int v) in
    let n_chars = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
