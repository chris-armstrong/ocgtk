class type app_chooser_button_t = object
    ...
    method on_custom_item_activated : ?after:bool -> callback:(item_name:string -> unit) -> unit -> Gobject.Signal.handler_id
...
class app_chooser_button (obj : App_chooser_button.t) : app_chooser_button_t = object (self)
  ...
  method on_custom_item_activated ?(after = false) ~callback () =
    App_chooser_button.on_custom_item_activated ~after self#as_app_chooser_button ~callback
