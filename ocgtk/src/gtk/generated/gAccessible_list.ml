...
class accessible_list (obj : Accessible_list.t) : accessible_list_t =
  object (self)
    method get_objects : unit -> GAt_context_and__accessible.accessible_t list =
      fun () ->
        (List.map (fun ret -> new GAt_context_and__accessible.accessible ret))
          (Accessible_list.get_objects obj)
...
let new_from_list (list : At_context_and__accessible.Accessible.t list) :
    accessible_list_t =
