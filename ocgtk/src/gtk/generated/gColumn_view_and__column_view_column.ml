...
class column_view (obj : Column_view_and__column_view_column.Column_view.t) :
  ...
  object (self)
    ...
    method get_columns : unit -> Ocgtk_gio.Gio.List_model.list_model_t =
      fun () ->
        new Ocgtk_gio.Gio.List_model.list_model
          (Column_view_and__column_view_column.Column_view.get_columns obj)

    method get_enable_rubberband : unit -> bool =
      fun () ->
        Column_view_and__column_view_column.Column_view.get_enable_rubberband
          obj
    ...
    method get_header_factory :
      ...
      fun () ->
        Option.map
          ...
          (Column_view_and__column_view_column.Column_view.get_header_factory
             obj)
    ...
    method get_model : unit -> GSelection_model.selection_model_t option =
      fun () ->
        Option.map
          ...
          (Column_view_and__column_view_column.Column_view.get_model obj)

    method get_reorderable : unit -> bool =
      fun () ->
        Column_view_and__column_view_column.Column_view.get_reorderable obj

    method get_row_factory :
        unit -> GList_item_factory.list_item_factory_t option =
      fun () ->
        Option.map
          (fun ret -> new GList_item_factory.list_item_factory ret)
          (Column_view_and__column_view_column.Column_view.get_row_factory obj)

    method get_show_column_separators : unit -> bool =
      fun () ->
        Column_view_and__column_view_column.Column_view
        .get_show_column_separators obj

    method get_show_row_separators : unit -> bool =
      fun () ->
        Column_view_and__column_view_column.Column_view.get_show_row_separators
          obj

    method get_single_click_activate : unit -> bool =
      fun () ->
        Column_view_and__column_view_column.Column_view
        .get_single_click_activate obj

    method get_sorter : unit -> GSorter.sorter_t option =
      fun () ->
        Option.map
          (fun ret -> new GSorter.sorter ret)
          (Column_view_and__column_view_column.Column_view.get_sorter obj)
    ...
    method insert_column : int -> column_view_column_t -> unit =
      fun position column ->
        let column = column#as_column_view_column in
        Column_view_and__column_view_column.Column_view.insert_column obj
          position column
    ...
    method scroll_to :
      ...
      fun pos column flags scroll ->
        ...
        Column_view_and__column_view_column.Column_view.scroll_to obj pos column
          flags scroll

    method set_enable_rubberband : bool -> unit =
      fun enable_rubberband ->
        Column_view_and__column_view_column.Column_view.set_enable_rubberband
          obj enable_rubberband

    method set_header_factory :
        GList_item_factory.list_item_factory_t option -> unit =
      fun factory ->
        let factory = Option.map (fun c -> c#as_list_item_factory) factory in
        Column_view_and__column_view_column.Column_view.set_header_factory obj
          factory

    method set_model : GSelection_model.selection_model_t option -> unit =
      fun model ->
        let model = Option.map (fun c -> c#as_selection_model) model in
        Column_view_and__column_view_column.Column_view.set_model obj model

    method set_reorderable : bool -> unit =
      fun reorderable ->
        Column_view_and__column_view_column.Column_view.set_reorderable obj
          reorderable
    ...
    method set_row_factory :
      ...
      fun factory ->
        let factory = Option.map (fun c -> c#as_list_item_factory) factory in
        Column_view_and__column_view_column.Column_view.set_row_factory obj
          factory

    method set_show_column_separators : bool -> unit =
      fun show_column_separators ->
        Column_view_and__column_view_column.Column_view
        .set_show_column_separators obj show_column_separators

    method set_show_row_separators : bool -> unit =
      fun show_row_separators ->
        Column_view_and__column_view_column.Column_view.set_show_row_separators
          obj show_row_separators

    method set_single_click_activate : bool -> unit =
      fun single_click_activate ->
        Column_view_and__column_view_column.Column_view
        .set_single_click_activate obj single_click_activate

    method set_tab_behavior : Gtk_enums.listtabbehavior -> unit =
      fun tab_behavior ->
        Column_view_and__column_view_column.Column_view.set_tab_behavior obj
          tab_behavior
    ...
    method sort_by_column :
      ...
      fun column direction ->
        let column = Option.map (fun c -> c#as_column_view_column) column in
        Column_view_and__column_view_column.Column_view.sort_by_column obj
          column direction
...
and column_view_column
  (obj : Column_view_and__column_view_column.Column_view_column.t) :
  column_view_column_t =
  object (self)
    method get_column_view : unit -> column_view_t option =
      fun () ->
        Option.map
          (fun ret -> new column_view ret)
    ...
    method get_factory : unit -> GList_item_factory.list_item_factory_t option =
      fun () ->
        Option.map
          (fun ret -> new GList_item_factory.list_item_factory ret)
          (Column_view_and__column_view_column.Column_view_column.get_factory
             obj)

    method get_fixed_width : unit -> int =
      fun () ->
        Column_view_and__column_view_column.Column_view_column.get_fixed_width
          obj

    method get_header_menu :
        unit -> Ocgtk_gio.Gio.Menu_model.menu_model_t option =
      fun () ->
        Option.map
    ...
    method get_sorter : unit -> GSorter.sorter_t option =
      fun () ->
        Option.map
          (fun ret -> new GSorter.sorter ret)
          (Column_view_and__column_view_column.Column_view_column.get_sorter obj)
    ...
    method set_expand : bool -> unit =
      fun expand ->
        Column_view_and__column_view_column.Column_view_column.set_expand obj
          expand

    method set_factory : GList_item_factory.list_item_factory_t option -> unit =
      fun factory ->
        let factory = Option.map (fun c -> c#as_list_item_factory) factory in
        Column_view_and__column_view_column.Column_view_column.set_factory obj
          factory

    method set_fixed_width : int -> unit =
      fun fixed_width ->
        Column_view_and__column_view_column.Column_view_column.set_fixed_width
          obj fixed_width

    method set_header_menu :
        Ocgtk_gio.Gio.Menu_model.menu_model_t option -> unit =
      fun menu ->
        let menu = Option.map (fun c -> c#as_menu_model) menu in
        Column_view_and__column_view_column.Column_view_column.set_header_menu
          obj menu
    ...
    method set_resizable : bool -> unit =
      fun resizable ->
        Column_view_and__column_view_column.Column_view_column.set_resizable obj
          resizable

    method set_sorter : GSorter.sorter_t option -> unit =
      fun sorter ->
        let sorter = Option.map (fun c -> c#as_sorter) sorter in
        Column_view_and__column_view_column.Column_view_column.set_sorter obj
          sorter

    method set_title : string option -> unit =
      fun title ->
        Column_view_and__column_view_column.Column_view_column.set_title obj
          title

    method set_visible : bool -> unit =
      fun visible ->
        Column_view_and__column_view_column.Column_view_column.set_visible obj
          visible
...
let new_ (title : string option)
    (factory : GList_item_factory.list_item_factory_t option) :
    column_view_column_t =
  let factory = Option.map (fun c -> c#as_list_item_factory) factory in
  let obj_ =
