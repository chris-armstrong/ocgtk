...
class sort_list_model (obj : Sort_list_model.t) : sort_list_model_t =
  object (self)
    inherit
    ...
    method get_incremental : unit -> bool =
      fun () -> Sort_list_model.get_incremental obj

    method get_model : unit -> Ocgtk_gio.Gio.List_model.list_model_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gio.Gio.List_model.list_model ret)
          (Sort_list_model.get_model obj)
    ...
    method get_section_sorter : unit -> GSorter.sorter_t option =
      fun () ->
        Option.map
          ...
          (Sort_list_model.get_section_sorter obj)

    method get_sorter : unit -> GSorter.sorter_t option =
      fun () ->
        Option.map
          (fun ret -> new GSorter.sorter ret)
          (Sort_list_model.get_sorter obj)

    method set_incremental : bool -> unit =
      fun incremental -> Sort_list_model.set_incremental obj incremental
...
let new_ (model : Ocgtk_gio.Gio.List_model.list_model_t option)
    (sorter : GSorter.sorter_t option) : sort_list_model_t =
  let model = Option.map (fun c -> c#as_list_model) model in
  let sorter = Option.map (fun c -> c#as_sorter) sorter in
