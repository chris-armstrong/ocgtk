...
class grid_view (obj : Grid_view.t) : grid_view_t =
  object (self)
    ...
    method get_factory : unit -> GList_item_factory.list_item_factory_t option =
      fun () ->
        Option.map
          ...
          (Grid_view.get_factory obj)

    method get_max_columns : unit -> int =
      fun () -> Grid_view.get_max_columns obj

    method get_min_columns : unit -> int =
      fun () -> Grid_view.get_min_columns obj

    method get_model : unit -> GSelection_model.selection_model_t option =
      fun () ->
        Option.map
          (fun ret -> new GSelection_model.selection_model ret)
          (Grid_view.get_model obj)

    method get_single_click_activate : unit -> bool =
      fun () -> Grid_view.get_single_click_activate obj

    method get_tab_behavior : unit -> Gtk_enums.listtabbehavior =
      fun () -> Grid_view.get_tab_behavior obj

    method scroll_to :
        int -> Gtk_enums.listscrollflags -> Scroll_info.t option -> unit =
      fun pos flags scroll -> Grid_view.scroll_to obj pos flags scroll
    ...
    method set_max_columns : int -> unit =
      fun max_columns -> Grid_view.set_max_columns obj max_columns

    method set_min_columns : int -> unit =
      fun min_columns -> Grid_view.set_min_columns obj min_columns

    method set_model : GSelection_model.selection_model_t option -> unit =
      fun model ->
        let model = Option.map (fun c -> c#as_selection_model) model in
        Grid_view.set_model obj model
    ...
    method set_tab_behavior : Gtk_enums.listtabbehavior -> unit =
      fun tab_behavior -> Grid_view.set_tab_behavior obj tab_behavior

    method as_grid_view = obj
  end

let new_ (model : GSelection_model.selection_model_t option)
    (factory : GList_item_factory.list_item_factory_t option) : grid_view_t =
  let model = Option.map (fun c -> c#as_selection_model) model in
  let factory = Option.map (fun c -> c#as_list_item_factory) factory in
