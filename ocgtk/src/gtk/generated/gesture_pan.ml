...
let on_pan ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let direction = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.pandirection_of_int (Gobject.Value.get_enum_int v)) in
    let offset = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_double v) in
