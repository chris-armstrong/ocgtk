...
external get_unit : t -> Ocgtk_gdk.Gdk.scrollunit = "ml_gtk_event_controller_scroll_get_unit"
...
let on_decelerate ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let vel_x = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_double v) in
    let vel_y = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_double v) in
...
let on_scroll ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let dx = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_double v) in
    let dy = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_double v) in
