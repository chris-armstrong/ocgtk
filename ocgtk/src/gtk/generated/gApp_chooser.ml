...
class app_chooser (obj : App_chooser.t) : app_chooser_t =
  object (self)
    method get_app_info : unit -> Ocgtk_gio.Gio.App_info.app_info_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gio.Gio.App_info.app_info ret)
          (App_chooser.get_app_info obj)

    method get_content_type : unit -> string =
      fun () -> App_chooser.get_content_type obj
