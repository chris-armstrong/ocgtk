...
external forward : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> bool = "ml_gtk_event_controller_key_forward"
...
let on_key_pressed ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let keyval = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_uint v) in
    let keycode = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_uint v) in
    let state = (let v = (Gobject.Closure.nth argv ~pos:3) in Ocgtk_gdk.Gdk_enums.modifiertype_of_int (Gobject.Value.get_flags_int v)) in
...
let on_modifiers ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let state = (let v = (Gobject.Closure.nth argv ~pos:1) in Ocgtk_gdk.Gdk_enums.modifiertype_of_int (Gobject.Value.get_flags_int v)) in
