...
class gesture_single (obj : Gesture_single.t) : gesture_single_t = object (self)
  ...
  method get_current_sequence : unit -> Ocgtk_gdk.Gdk.Event_sequence.event_sequence_t option =
    fun () ->
      Option.map (fun ret -> new Ocgtk_gdk.Gdk.Event_sequence.event_sequence ret) (Gesture_single.get_current_sequence obj)

  method get_exclusive : unit -> bool =
    fun () ->
      (Gesture_single.get_exclusive obj)

  method get_touch_only : unit -> bool =
    fun () ->
      (Gesture_single.get_touch_only obj)

  method set_button : int -> unit =
    fun button ->
      (Gesture_single.set_button obj button)

  method set_exclusive : bool -> unit =
    fun exclusive ->
      (Gesture_single.set_exclusive obj exclusive)
