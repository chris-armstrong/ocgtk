...
class image (obj : Image.t) : image_t =
  object (self)
    inherit
      GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget
      .widget
    ...
    method get_icon_name : unit -> string option =
      fun () -> Image.get_icon_name obj

    method get_icon_size : unit -> Gtk_enums.iconsize =
      fun () -> Image.get_icon_size obj

    method get_paintable : unit -> Ocgtk_gdk.Gdk.Paintable.paintable_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gdk.Gdk.Paintable.paintable ret)
          (Image.get_paintable obj)

    method get_pixel_size : unit -> int = fun () -> Image.get_pixel_size obj

    method get_storage_type : unit -> Gtk_enums.imagetype =
      fun () -> Image.get_storage_type obj

    method set_from_file : string option -> unit =
      fun filename -> Image.set_from_file obj filename
    ...
    method set_from_icon_name : string option -> unit =
      fun icon_name -> Image.set_from_icon_name obj icon_name

    method set_from_paintable :
        Ocgtk_gdk.Gdk.Paintable.paintable_t option -> unit =
      fun paintable ->
    ...
    method set_from_resource : string option -> unit =
      fun resource_path -> Image.set_from_resource obj resource_path

    method set_icon_size : Gtk_enums.iconsize -> unit =
      fun icon_size -> Image.set_icon_size obj icon_size
...
let new_ () : image_t = new image (Image.new_ ())
...
let new_from_paintable (paintable : Ocgtk_gdk.Gdk.Paintable.paintable_t option)
    : image_t =
  let paintable = Option.map (fun c -> c#as_paintable) paintable in
  let obj_ = Image.new_from_paintable paintable in
  new image obj_

let new_from_pixbuf (pixbuf : Ocgtk_gdkpixbuf.GdkPixbuf.Pixbuf.pixbuf_t option)
    : image_t =
  let pixbuf = Option.map (fun c -> c#as_pixbuf) pixbuf in
