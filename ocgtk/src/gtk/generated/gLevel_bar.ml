...
class level_bar (obj : Level_bar.t) : level_bar_t =
  object (self)
    ...
    method get_mode : unit -> Gtk_enums.levelbarmode =
      fun () -> Level_bar.get_mode obj

    method get_value : unit -> float = fun () -> Level_bar.get_value obj

    method remove_offset_value : string option -> unit =
      fun name -> Level_bar.remove_offset_value obj name

    method set_inverted : bool -> unit =
      fun inverted -> Level_bar.set_inverted obj inverted

    method set_max_value : float -> unit =
      fun value -> Level_bar.set_max_value obj value

    method set_min_value : float -> unit =
      fun value -> Level_bar.set_min_value obj value

    method set_mode : Gtk_enums.levelbarmode -> unit =
      fun mode -> Level_bar.set_mode obj mode

    method set_value : float -> unit =
      fun value -> Level_bar.set_value obj value

    method as_level_bar = obj
  end

let new_ () : level_bar_t = new level_bar (Level_bar.new_ ())
