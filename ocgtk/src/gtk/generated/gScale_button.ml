...
class scale_button (obj : Scale_button.t) : scale_button_t =
  object (self)
    ...
    method get_adjustment : unit -> GAdjustment.adjustment_t =
      fun () -> new GAdjustment.adjustment (Scale_button.get_adjustment obj)

    method get_has_frame : unit -> bool =
      fun () -> Scale_button.get_has_frame obj

    method get_minus_button : unit -> GButton.button_t =
      fun () -> new GButton.button (Scale_button.get_minus_button obj)

    method get_plus_button : unit -> GButton.button_t =
      fun () -> new GButton.button (Scale_button.get_plus_button obj)
    ...
    method get_value : unit -> float = fun () -> Scale_button.get_value obj
    ...
    method set_has_frame : bool -> unit =
      fun has_frame -> Scale_button.set_has_frame obj has_frame

    method set_icons : string array -> unit =
      fun icons -> Scale_button.set_icons obj icons

    method set_value : float -> unit =
      fun value -> Scale_button.set_value obj value

    method as_scale_button = obj
  end

let new_ (min : float) (max : float) (step : float)
    (icons : string array option) : scale_button_t =
