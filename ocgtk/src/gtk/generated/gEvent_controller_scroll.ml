...
class event_controller_scroll (obj : Event_controller_scroll.t) :
  ...
  object (self)
    ...
    method on_decelerate ?(after = false) ~callback () =
      Event_controller_scroll.on_decelerate ~after
        self#as_event_controller_scroll ~callback

    method on_scroll ?(after = false) ~callback () =
      Event_controller_scroll.on_scroll ~after self#as_event_controller_scroll
        ~callback

    method on_scroll_begin ?(after = false) ~callback () =
      Event_controller_scroll.on_scroll_begin ~after
        self#as_event_controller_scroll ~callback

    method on_scroll_end ?(after = false) ~callback () =
      Event_controller_scroll.on_scroll_end ~after
        self#as_event_controller_scroll ~callback
