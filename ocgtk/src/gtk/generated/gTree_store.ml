...
class tree_store (obj : Tree_store.t) : tree_store_t =
  object (self)
    ...
    method is_ancestor : Tree_iter.t -> Tree_iter.t -> bool =
      fun iter descendant -> Tree_store.is_ancestor obj iter descendant

    method iter_depth : Tree_iter.t -> int =
      fun iter -> Tree_store.iter_depth obj iter

    method iter_is_valid : Tree_iter.t -> bool =
      fun iter -> Tree_store.iter_is_valid obj iter

    method move_after : Tree_iter.t -> Tree_iter.t option -> unit =
      fun iter position -> Tree_store.move_after obj iter position

    method move_before : Tree_iter.t -> Tree_iter.t option -> unit =
      fun iter position -> Tree_store.move_before obj iter position

    method remove : Tree_iter.t -> bool = fun iter -> Tree_store.remove obj iter

    method set_column_types : int -> Gobject.Type.t array -> unit =
      fun n_columns types -> Tree_store.set_column_types obj n_columns types

    method set_value : Tree_iter.t -> int -> Gobject.Value.t -> unit =
      fun iter column value -> Tree_store.set_value obj iter column value

    method set_valuesv :
        Tree_iter.t -> int array -> Gobject.Value.t array -> int -> unit =
      fun iter columns values n_values ->
        Tree_store.set_valuesv obj iter columns values n_values

    method swap : Tree_iter.t -> Tree_iter.t -> unit =
      fun a b -> Tree_store.swap obj a b

    method as_tree_store = obj
  end

let newv (n_columns : int) (types : Gobject.Type.t array) : tree_store_t =
