...
class file_chooser_widget (obj : File_chooser_widget.t) : file_chooser_widget_t = object (self)
  ...
  method on_desktop_folder ?(after = false) ~callback () =
    File_chooser_widget.on_desktop_folder ~after self#as_file_chooser_widget ~callback

  method on_down_folder ?(after = false) ~callback () =
    File_chooser_widget.on_down_folder ~after self#as_file_chooser_widget ~callback

  method on_home_folder ?(after = false) ~callback () =
    File_chooser_widget.on_home_folder ~after self#as_file_chooser_widget ~callback

  method on_location_popup ?(after = false) ~callback () =
    File_chooser_widget.on_location_popup ~after self#as_file_chooser_widget ~callback

  method on_location_popup_on_paste ?(after = false) ~callback () =
    File_chooser_widget.on_location_popup_on_paste ~after self#as_file_chooser_widget ~callback

  method on_location_toggle_popup ?(after = false) ~callback () =
    File_chooser_widget.on_location_toggle_popup ~after self#as_file_chooser_widget ~callback

  method on_places_shortcut ?(after = false) ~callback () =
    File_chooser_widget.on_places_shortcut ~after self#as_file_chooser_widget ~callback

  method on_quick_bookmark ?(after = false) ~callback () =
    File_chooser_widget.on_quick_bookmark ~after self#as_file_chooser_widget ~callback

  method on_recent_shortcut ?(after = false) ~callback () =
    File_chooser_widget.on_recent_shortcut ~after self#as_file_chooser_widget ~callback

  method on_search_shortcut ?(after = false) ~callback () =
    File_chooser_widget.on_search_shortcut ~after self#as_file_chooser_widget ~callback

  method on_show_hidden ?(after = false) ~callback () =
    File_chooser_widget.on_show_hidden ~after self#as_file_chooser_widget ~callback

  method on_up_folder ?(after = false) ~callback () =
    File_chooser_widget.on_up_folder ~after self#as_file_chooser_widget ~callback
