...
class adjustment (obj : Adjustment.t) : adjustment_t =
  object (self)
    method on_changed ?(after = false) ~callback () =
      Adjustment.on_changed ~after self#as_adjustment ~callback

    method on_value_changed ?(after = false) ~callback () =
      Adjustment.on_value_changed ~after self#as_adjustment ~callback
    ...
    method configure :
      ...
      fun value lower upper step_increment page_increment page_size ->
        Adjustment.configure obj value lower upper step_increment page_increment
          page_size

    method get_lower : unit -> float = fun () -> Adjustment.get_lower obj

    method get_minimum_increment : unit -> float =
      fun () -> Adjustment.get_minimum_increment obj

    method get_page_increment : unit -> float =
      fun () -> Adjustment.get_page_increment obj

    method get_page_size : unit -> float =
      fun () -> Adjustment.get_page_size obj

    method get_step_increment : unit -> float =
      fun () -> Adjustment.get_step_increment obj

    method get_upper : unit -> float = fun () -> Adjustment.get_upper obj
    method get_value : unit -> float = fun () -> Adjustment.get_value obj

    method set_lower : float -> unit =
      fun lower -> Adjustment.set_lower obj lower

    method set_page_increment : float -> unit =
      fun page_increment -> Adjustment.set_page_increment obj page_increment

    method set_page_size : float -> unit =
      fun page_size -> Adjustment.set_page_size obj page_size

    method set_step_increment : float -> unit =
      fun step_increment -> Adjustment.set_step_increment obj step_increment

    method set_upper : float -> unit =
      fun upper -> Adjustment.set_upper obj upper

    method set_value : float -> unit =
      fun value -> Adjustment.set_value obj value

    method as_adjustment = obj
  end

let new_ (value : float) (lower : float) (upper : float)
    (step_increment : float) (page_increment : float) (page_size : float) :
    adjustment_t =
  let obj_ =
    Adjustment.new_ value lower upper step_increment page_increment page_size
  in
  new adjustment obj_
