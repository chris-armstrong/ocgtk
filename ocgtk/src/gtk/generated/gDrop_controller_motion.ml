...
class drop_controller_motion (obj : Drop_controller_motion.t) : drop_controller_motion_t = object (self)
  ...
  method on_enter ?(after = false) ~callback () =
    Drop_controller_motion.on_enter ~after self#as_drop_controller_motion ~callback

  method on_leave ?(after = false) ~callback () =
    Drop_controller_motion.on_leave ~after self#as_drop_controller_motion ~callback

  method on_motion ?(after = false) ~callback () =
    Drop_controller_motion.on_motion ~after self#as_drop_controller_motion ~callback
  ...
  method get_drop : unit -> Ocgtk_gdk.Gdk.Drop.drop_t option =
    fun () ->
      Option.map (fun ret -> new Ocgtk_gdk.Gdk.Drop.drop ret) (Drop_controller_motion.get_drop obj)
