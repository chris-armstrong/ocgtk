...
and Tree_view
 : sig
  ...
  external set_tooltip_row : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Tooltip.t -> Tree_path.t -> unit = "ml_gtk_tree_view_set_tooltip_row"
  ...
  See also gtk_tree_view_set_tooltip_column() for a simpler alternative. *)
  external set_tooltip_cell : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Tooltip.t -> Tree_path.t option -> Tree_view_column.t option -> Cell_renderer.t option -> unit = "ml_gtk_tree_view_set_tooltip_cell"
  ...
  external convert_widget_to_tree_coords : t -> int -> int -> int * int = "ml_gtk_tree_view_convert_widget_to_tree_coords"

  (** Converts widget coordinates to coordinates for the bin_window. *)
  external convert_widget_to_bin_window_coords : t -> int -> int -> int * int = "ml_gtk_tree_view_convert_widget_to_bin_window_coords"

  (** Converts tree coordinates (coordinates in full scrollable area of the tree)
  to widget coordinates. *)
  external convert_tree_to_widget_coords : t -> int -> int -> int * int = "ml_gtk_tree_view_convert_tree_to_widget_coords"

  (** Converts tree coordinates (coordinates in full scrollable area of the tree)
  to bin_window coordinates. *)
  external convert_tree_to_bin_window_coords : t -> int -> int -> int * int = "ml_gtk_tree_view_convert_tree_to_bin_window_coords"

  (** Converts bin_window coordinates to widget relative coordinates. *)
  external convert_bin_window_to_widget_coords : t -> int -> int -> int * int = "ml_gtk_tree_view_convert_bin_window_to_widget_coords"

  (** Converts bin_window coordinates to coordinates for the
  tree (the full scrollable area of the tree). *)
  external convert_bin_window_to_tree_coords : t -> int -> int -> int * int = "ml_gtk_tree_view_convert_bin_window_to_tree_coords"
  ...
  external get_enable_grid_lines : t -> Gtk_enums.treeviewgridlines = "ml_gtk_tree_view_get_enable_grid_lines"

  (** Set property: enable-grid-lines *)
  external set_enable_grid_lines : t -> Gtk_enums.treeviewgridlines -> unit = "ml_gtk_tree_view_set_enable_grid_lines"
  ...
  val on_start_interactive_search : ?after:bool -> t -> callback:(unit -> bool) -> Gobject.Signal.handler_id
  ...
  let on_expand_collapse_cursor_row ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      ...
      let p1 = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_boolean v) in
      let result = callback ~object_ ~p0 ~p1 in
      let v = Gobject.Closure.result argv in
      let x = result in
      Gobject.Value.set_boolean v x) in
    Gobject.Signal.connect obj ~name:"expand-collapse-cursor-row" ~callback:closure
      ~after:(Option.value after ~default:false)
  ...
  let on_move_cursor ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      ...
      let direction = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
      let extend = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_boolean v) in
      let modify = (let v = (Gobject.Closure.nth argv ~pos:4) in Gobject.Value.get_boolean v) in
  ...
  let on_start_interactive_search ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      ...
      let x = result in
      Gobject.Value.set_boolean v x) in
    Gobject.Signal.connect obj ~name:"start-interactive-search" ~callback:closure
      ~after:(Option.value after ~default:false)
