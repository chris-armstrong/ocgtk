...
external set_placeholder : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_list_box_set_placeholder"
...
external remove : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> unit = "ml_gtk_list_box_remove"

(** Prepend a widget to the list.

If a sort function is set, the widget will
actually be inserted at the calculated position. *)
external prepend : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> unit = "ml_gtk_list_box_prepend"
...
external insert : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> int -> unit = "ml_gtk_list_box_insert"
...
external get_selected_rows : t -> List_box_row.t list = "ml_gtk_list_box_get_selected_rows"
...
If a sort function is set, the widget will
actually be inserted at the calculated position. *)
external append : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> unit = "ml_gtk_list_box_append"
...
let on_move_cursor ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let object_ = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.movementstep_of_int (Gobject.Value.get_enum_int v)) in
    let p0 = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
    let p1 = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_boolean v) in
    let p2 = (let v = (Gobject.Closure.nth argv ~pos:4) in Gobject.Value.get_boolean v) in
...
let on_row_activated ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let row = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
    callback ~row)
 in
...
let on_row_selected ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let row = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object v) in
    callback ~row)
 in
