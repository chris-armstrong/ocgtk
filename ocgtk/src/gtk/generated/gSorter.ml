class type sorter_t = object
    method on_changed : ?after:bool -> callback:(change:Gtk_enums.sorterchange -> unit) -> unit -> Gobject.Signal.handler_id
...
class sorter (obj : Sorter.t) : sorter_t = object (self)
  method on_changed ?(after = false) ~callback () =
    Sorter.on_changed ~after self#as_sorter ~callback
