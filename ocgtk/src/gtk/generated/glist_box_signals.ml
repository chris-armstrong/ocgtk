...
class list_box_signals (obj : List_box.t) = object
  method on_activate_cursor_row ~callback =
    Gobject.Signal.connect_simple obj ~name:"activate-cursor-row" ~callback ~after:false
...
The default binding for this signal is <kbd>Ctrl</kbd>-<kbd>a</kbd>. *)
  ...
  method on_selected_rows_changed ~callback =
    Gobject.Signal.connect_simple obj ~name:"selected-rows-changed" ~callback ~after:false

  method on_toggle_cursor_row ~callback =
    Gobject.Signal.connect_simple obj ~name:"toggle-cursor-row" ~callback ~after:false
