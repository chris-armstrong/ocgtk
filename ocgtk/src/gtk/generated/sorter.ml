...
external compare : t -> [`object_] Gobject.obj -> [`object_] Gobject.obj -> Gtk_enums.ordering = "ml_gtk_sorter_compare"
...
let on_changed ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let change = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.sorterchange_of_int (Gobject.Value.get_enum_int v)) in
    callback ~change)
 in
