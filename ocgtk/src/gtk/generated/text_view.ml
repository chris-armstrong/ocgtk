...
external set_gutter : t -> Gtk_enums.textwindowtype -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_text_view_set_gutter"
...
external remove : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> unit = "ml_gtk_text_view_remove"
...
external move_overlay : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> int -> int -> unit = "ml_gtk_text_view_move_overlay"
...
external get_line_yrange : t -> Text_buffer_and__text_iter_and__text_mark.Text_iter.t -> int * int = "ml_gtk_text_view_get_line_yrange"
...
of the line. *)
external get_line_at_y : t -> int -> Text_buffer_and__text_iter_and__text_mark.Text_iter.t * int = "ml_gtk_text_view_get_line_at_y"
...
external get_iter_at_position : t -> int -> int -> bool * Text_buffer_and__text_iter_and__text_mark.Text_iter.t * int = "ml_gtk_text_view_get_iter_at_position"
...
@win must be one of %GTK_TEXT_WINDOW_LEFT, %GTK_TEXT_WINDOW_RIGHT,
%GTK_TEXT_WINDOW_TOP, or %GTK_TEXT_WINDOW_BOTTOM. *)
external get_gutter : t -> Gtk_enums.textwindowtype -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option = "ml_gtk_text_view_get_gutter"
...
external add_overlay : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> int -> int -> unit = "ml_gtk_text_view_add_overlay"

(** Adds a child widget in the text buffer, at the given @anchor. *)
external add_child_at_anchor : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t -> Text_child_anchor.t -> unit = "ml_gtk_text_view_add_child_at_anchor"
...
let on_delete_from_cursor ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let type_ = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.deletetype_of_int (Gobject.Value.get_enum_int v)) in
    let count = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
...
let on_insert_at_cursor ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let string = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
    callback ~string)
 in
...
let on_move_cursor ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let step = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.movementstep_of_int (Gobject.Value.get_enum_int v)) in
    let count = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
    let extend_selection = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_boolean v) in
...
let on_move_viewport ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let step = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.scrollstep_of_int (Gobject.Value.get_enum_int v)) in
    let count = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
    callback ~step ~count)
 in
...
let on_preedit_changed ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let preedit = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
    callback ~preedit)
 in
...
let on_select_all ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let select = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_boolean v) in
    callback ~select)
 in
