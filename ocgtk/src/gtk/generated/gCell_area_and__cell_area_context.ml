...
class type cell_area_t = object
  ...
  method add_focus_sibling :
    GCell_renderer.cell_renderer_t -> GCell_renderer.cell_renderer_t -> unit

  method attribute_connect :
    GCell_renderer.cell_renderer_t -> string -> int -> unit
  ...
  method get_focus_from_sibling :
    GCell_renderer.cell_renderer_t -> GCell_renderer.cell_renderer_t option

  method get_focus_siblings :
    GCell_renderer.cell_renderer_t -> Cell_renderer.t list
  ...
  method is_focus_sibling :
    GCell_renderer.cell_renderer_t -> GCell_renderer.cell_renderer_t -> bool
...
and cell_area_context_t = object
  ...
  method as_cell_area_context :
    Cell_area_and__cell_area_context.Cell_area_context.t
end

(* Signal class defined in gcell_area_signals.ml *)

class cell_area (obj : Cell_area_and__cell_area_context.Cell_area.t) :
  cell_area_t =
  object (self)
    ...
    method activate :
      ...
      fun context widget cell_area flags edit_only ->
        ...
        Cell_area_and__cell_area_context.Cell_area.activate obj context widget
          cell_area flags edit_only
    ...
    method activate_cell :
      ...
      fun widget renderer event cell_area flags ->
        ...
        let cell_area = cell_area#as_rectangle in
        Cell_area_and__cell_area_context.Cell_area.activate_cell obj widget
          renderer event cell_area flags
    ...
    method add_focus_sibling :
      ...
      fun renderer sibling ->
        ...
        Cell_area_and__cell_area_context.Cell_area.add_focus_sibling obj
          renderer sibling
    ...
    method attribute_connect :
      ...
      fun renderer attribute column ->
        let renderer = renderer#as_cell_renderer in
        Cell_area_and__cell_area_context.Cell_area.attribute_connect obj
          renderer attribute column

    method attribute_disconnect :
        GCell_renderer.cell_renderer_t -> string -> unit =
      fun renderer attribute ->
        let renderer = renderer#as_cell_renderer in
        Cell_area_and__cell_area_context.Cell_area.attribute_disconnect obj
          renderer attribute

    method attribute_get_column :
        GCell_renderer.cell_renderer_t -> string -> int =
      fun renderer attribute ->
        let renderer = renderer#as_cell_renderer in
        Cell_area_and__cell_area_context.Cell_area.attribute_get_column obj
          renderer attribute

    method copy_context : cell_area_context_t -> cell_area_context_t =
      fun context ->
        let context = context#as_cell_area_context in
        new cell_area_context
          (Cell_area_and__cell_area_context.Cell_area.copy_context obj context)

    method create_context : unit -> cell_area_context_t =
      fun () ->
        new cell_area_context
          (Cell_area_and__cell_area_context.Cell_area.create_context obj)
    ...
    method event :
      ...
      fun context widget event cell_area flags ->
        ...
        let event = event#as_event in
        let cell_area = cell_area#as_rectangle in
        Cell_area_and__cell_area_context.Cell_area.event obj context widget
    ...
    method get_edit_widget : unit -> GCell_editable.cell_editable_t option =
      fun () ->
        Option.map
          (fun ret -> new GCell_editable.cell_editable ret)
          (Cell_area_and__cell_area_context.Cell_area.get_edit_widget obj)

    method get_edited_cell : unit -> GCell_renderer.cell_renderer_t option =
      fun () ->
        Option.map
          (fun ret -> new GCell_renderer.cell_renderer ret)
          (Cell_area_and__cell_area_context.Cell_area.get_edited_cell obj)

    method get_focus_cell : unit -> GCell_renderer.cell_renderer_t option =
      fun () ->
        Option.map
          (fun ret -> new GCell_renderer.cell_renderer ret)
          (Cell_area_and__cell_area_context.Cell_area.get_focus_cell obj)
    ...
    method get_focus_from_sibling :
      ...
      fun renderer ->
        ...
        Option.map
          (fun ret -> new GCell_renderer.cell_renderer ret)
          (Cell_area_and__cell_area_context.Cell_area.get_focus_from_sibling obj
             renderer)
    ...
    method get_focus_siblings :
      ...
      fun renderer ->
        let renderer = renderer#as_cell_renderer in
        Cell_area_and__cell_area_context.Cell_area.get_focus_siblings obj
          renderer

    method get_request_mode : unit -> Gtk_enums.sizerequestmode =
      fun () -> Cell_area_and__cell_area_context.Cell_area.get_request_mode obj
    ...
    method is_activatable : unit -> bool =
      fun () -> Cell_area_and__cell_area_context.Cell_area.is_activatable obj
    ...
    method is_focus_sibling :
      ...
      fun renderer sibling ->
        let renderer = renderer#as_cell_renderer in
        let sibling = sibling#as_cell_renderer in
        Cell_area_and__cell_area_context.Cell_area.is_focus_sibling obj renderer
          sibling
    ...
    method remove_focus_sibling :
      ...
      fun renderer sibling ->
        let renderer = renderer#as_cell_renderer in
        let sibling = sibling#as_cell_renderer in
        Cell_area_and__cell_area_context.Cell_area.remove_focus_sibling obj
          renderer sibling
    ...
    method snapshot :
      ...
      fun context widget snapshot background_area cell_area flags paint_focus ->
        ...
        let cell_area = cell_area#as_rectangle in
        Cell_area_and__cell_area_context.Cell_area.snapshot obj context widget
          snapshot background_area cell_area flags paint_focus
...
and cell_area_context
  (obj : Cell_area_and__cell_area_context.Cell_area_context.t) :
  cell_area_context_t =
  object (self)
    method allocate : int -> int -> unit =
      fun width height ->
        Cell_area_and__cell_area_context.Cell_area_context.allocate obj width
          height

    method get_area : unit -> cell_area_t =
      fun () ->
        new cell_area
          (Cell_area_and__cell_area_context.Cell_area_context.get_area obj)

    method push_preferred_height : int -> int -> unit =
      fun minimum_height natural_height ->
        Cell_area_and__cell_area_context.Cell_area_context.push_preferred_height
          obj minimum_height natural_height

    method push_preferred_width : int -> int -> unit =
      fun minimum_width natural_width ->
        Cell_area_and__cell_area_context.Cell_area_context.push_preferred_width
          obj minimum_width natural_width

    method reset : unit -> unit =
      fun () -> Cell_area_and__cell_area_context.Cell_area_context.reset obj

    method minimum_height =
      Cell_area_and__cell_area_context.Cell_area_context.get_minimum_height obj

    method minimum_width =
      Cell_area_and__cell_area_context.Cell_area_context.get_minimum_width obj

    method natural_height =
      Cell_area_and__cell_area_context.Cell_area_context.get_natural_height obj

    method natural_width =
      Cell_area_and__cell_area_context.Cell_area_context.get_natural_width obj
