...
let on_adjust_bounds ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let value = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_double v) in
    callback ~value)
 in
...
let on_change_value ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    ...
    let value = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_double v) in
...
let on_move_slider ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let step = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.scrolltype_of_int (Gobject.Value.get_enum_int v)) in
    callback ~step)
 in
