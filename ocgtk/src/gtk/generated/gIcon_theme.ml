...
class icon_theme (obj : Icon_theme.t) : icon_theme_t = object (self)
  method on_changed ?(after = false) ~callback () =
    Icon_theme.on_changed ~after self#as_icon_theme ~callback
  ...
  method get_display : unit -> Ocgtk_gdk.Gdk.Display.display_t option =
    fun () ->
      Option.map (fun ret -> new Ocgtk_gdk.Gdk.Display.display ret) (Icon_theme.get_display obj)
...
let new_ () : icon_theme_t =
  new icon_theme (Icon_theme.new_ ())
