...
CAMLexport CAMLprim value ml_gtk_slice_list_model_get_item_type(value self)
{
    CAMLparam1(self);
    CAMLlocal1(result);
...
if (pspec == NULL) caml_failwith("ml_gtk_slice_list_model_get_item_type: property 'item-type' not found");
...
CAMLparam1(self);
(void)self;
caml_failwith("SliceListModel requires GTK >= 4.8");
return Val_unit;
}
#endif
