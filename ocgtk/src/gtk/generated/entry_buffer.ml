...
let on_deleted_text ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let position = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_uint v) in
    let n_chars = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_uint v) in
...
let on_inserted_text ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let position = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_uint v) in
    let chars = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_string v) in
    let n_chars = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_uint v) in
