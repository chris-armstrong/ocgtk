class type drop_target_async_t = object
    ...
    method on_drag_enter : ?after:bool -> callback:(drop:Ocgtk_gdk.Gdk.Drop.drop_t -> x:float -> y:float -> Ocgtk_gdk.Gdk_enums.dragaction) -> unit -> Gobject.Signal.handler_id
    method on_drag_leave : ?after:bool -> callback:(drop:Ocgtk_gdk.Gdk.Drop.drop_t -> unit) -> unit -> Gobject.Signal.handler_id
    method on_drag_motion : ?after:bool -> callback:(drop:Ocgtk_gdk.Gdk.Drop.drop_t -> x:float -> y:float -> Ocgtk_gdk.Gdk_enums.dragaction) -> unit -> Gobject.Signal.handler_id
    method on_drop : ?after:bool -> callback:(drop:Ocgtk_gdk.Gdk.Drop.drop_t -> x:float -> y:float -> bool) -> unit -> Gobject.Signal.handler_id
    ...
    method set_formats : Ocgtk_gdk.Gdk.Content_formats.content_formats_t option -> unit
...
class drop_target_async (obj : Drop_target_async.t) : drop_target_async_t = object (self)
  ...
  method on_accept ?(after = false) ~callback () =
    Drop_target_async.on_accept ~after self#as_drop_target_async
      ~callback:(fun ~drop -> callback ~drop:(new Ocgtk_gdk.Gdk.Drop.drop drop))

  method on_drag_enter ?(after = false) ~callback () =
    Drop_target_async.on_drag_enter ~after self#as_drop_target_async
      ~callback:(fun ~drop ~x ~y -> callback ~drop:(new Ocgtk_gdk.Gdk.Drop.drop drop) ~x ~y)

  method on_drag_leave ?(after = false) ~callback () =
    Drop_target_async.on_drag_leave ~after self#as_drop_target_async
      ~callback:(fun ~drop -> callback ~drop:(new Ocgtk_gdk.Gdk.Drop.drop drop))

  method on_drag_motion ?(after = false) ~callback () =
    Drop_target_async.on_drag_motion ~after self#as_drop_target_async
      ~callback:(fun ~drop ~x ~y -> callback ~drop:(new Ocgtk_gdk.Gdk.Drop.drop drop) ~x ~y)

  method on_drop ?(after = false) ~callback () =
    Drop_target_async.on_drop ~after self#as_drop_target_async
      ~callback:(fun ~drop ~x ~y -> callback ~drop:(new Ocgtk_gdk.Gdk.Drop.drop drop) ~x ~y)
  ...
  method get_formats : unit -> Ocgtk_gdk.Gdk.Content_formats.content_formats_t option =
    fun () ->
      Option.map (fun ret -> new Ocgtk_gdk.Gdk.Content_formats.content_formats ret) (Drop_target_async.get_formats obj)
