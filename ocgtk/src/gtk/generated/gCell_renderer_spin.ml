...
class cell_renderer_spin (obj : Cell_renderer_spin.t) : cell_renderer_spin_t =
  object (self)
    inherit GCell_renderer_text.cell_renderer_text (obj :> Cell_renderer_text.t)

    method adjustment =
      new GAdjustment.adjustment (Cell_renderer_spin.get_adjustment obj)

    method set_adjustment : GAdjustment.adjustment_t -> unit =
      fun v -> Cell_renderer_spin.set_adjustment obj v#as_adjustment
...
let new_ () : cell_renderer_spin_t =
  new cell_renderer_spin (Cell_renderer_spin.new_ ())
