class type entry_buffer_t = object
    method on_deleted_text : ?after:bool -> callback:(position:int -> n_chars:int -> unit) -> unit -> Gobject.Signal.handler_id
    method on_inserted_text : ?after:bool -> callback:(position:int -> chars:string -> n_chars:int -> unit) -> unit -> Gobject.Signal.handler_id
...
class entry_buffer (obj : Entry_buffer.t) : entry_buffer_t = object (self)
  method on_deleted_text ?(after = false) ~callback () =
    Entry_buffer.on_deleted_text ~after self#as_entry_buffer ~callback

  method on_inserted_text ?(after = false) ~callback () =
    Entry_buffer.on_inserted_text ~after self#as_entry_buffer ~callback
