...
module rec At_context : sig
  ...
  val on_state_change : ?after:bool -> t -> callback:(unit -> unit) -> Gobject.Signal.handler_id
  ...
  let on_state_change ?after obj ~callback =
...
and Accessible
 : sig
  ...
  child widget. *)
  external get_bounds : t -> bool * int * int * int * int = "ml_gtk_accessible_get_bounds"
