...
class font_dialog (obj : Font_dialog.t) : font_dialog_t =
  object (self)
    method choose_face_finish :
        Ocgtk_gio.Gio.Async_result.async_result_t ->
        (Ocgtk_pango.Pango.Font_face.font_face_t option, GError.t) result =
    ...
    method get_filter : unit -> GFilter.filter_t option =
      fun () ->
        Option.map
          ...
          (Font_dialog.get_filter obj)

    method get_font_map : unit -> Ocgtk_pango.Pango.Font_map.font_map_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_pango.Pango.Font_map.font_map ret)
          (Font_dialog.get_font_map obj)

    method get_language : unit -> Ocgtk_pango.Pango.Language.language_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_pango.Pango.Language.language ret)
          (Font_dialog.get_language obj)

    method get_modal : unit -> bool = fun () -> Font_dialog.get_modal obj
    method get_title : unit -> string = fun () -> Font_dialog.get_title obj
    ...
    method set_modal : bool -> unit =
      fun modal -> Font_dialog.set_modal obj modal

    method set_title : string -> unit =
      fun title -> Font_dialog.set_title obj title

    method as_font_dialog = obj
  end

let new_ () : font_dialog_t = new font_dialog (Font_dialog.new_ ())
