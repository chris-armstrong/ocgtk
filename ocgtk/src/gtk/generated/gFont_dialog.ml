...
class font_dialog (obj : Font_dialog.t) : font_dialog_t = object (self)
  ...
  method choose_face_finish : Ocgtk_gio.Gio.Async_result.async_result_t -> (Ocgtk_pango.Pango.Font_face.font_face_t option, GError.t) result =
    fun result ->
      ...
      Result.map (fun ret -> Option.map (fun ret -> new Ocgtk_pango.Pango.Font_face.font_face ret) ret)(Font_dialog.choose_face_finish obj result)

  method choose_family_finish : Ocgtk_gio.Gio.Async_result.async_result_t -> (Ocgtk_pango.Pango.Font_family.font_family_t option, GError.t) result =
    fun result ->
      let result = result#as_async_result in
      Result.map (fun ret -> Option.map (fun ret -> new Ocgtk_pango.Pango.Font_family.font_family ret) ret)(Font_dialog.choose_family_finish obj result)

  method choose_font_finish : Ocgtk_gio.Gio.Async_result.async_result_t -> (Ocgtk_pango.Pango.Font_description.font_description_t option, GError.t) result =
    fun result ->
      let result = result#as_async_result in
      Result.map (fun ret -> Option.map (fun ret -> new Ocgtk_pango.Pango.Font_description.font_description ret) ret)(Font_dialog.choose_font_finish obj result)
  ...
  method get_font_map : unit -> Ocgtk_pango.Pango.Font_map.font_map_t option =
    fun () ->
      Option.map (fun ret -> new Ocgtk_pango.Pango.Font_map.font_map ret) (Font_dialog.get_font_map obj)

  method get_language : unit -> Ocgtk_pango.Pango.Language.language_t option =
    fun () ->
      Option.map (fun ret -> new Ocgtk_pango.Pango.Language.language ret) (Font_dialog.get_language obj)
...
let new_ () : font_dialog_t =
  new font_dialog (Font_dialog.new_ ())
