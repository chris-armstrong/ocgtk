class type event_controller_key_t = object
    ...
    method on_im_update : ?after:bool -> callback:(unit -> unit) -> unit -> Gobject.Signal.handler_id
    method on_key_pressed : ?after:bool -> callback:(keyval:int -> keycode:int -> state:Ocgtk_gdk.Gdk_enums.modifiertype -> bool) -> unit -> Gobject.Signal.handler_id
    method on_key_released : ?after:bool -> callback:(keyval:int -> keycode:int -> state:Ocgtk_gdk.Gdk_enums.modifiertype -> unit) -> unit -> Gobject.Signal.handler_id
    method on_modifiers : ?after:bool -> callback:(state:Ocgtk_gdk.Gdk_enums.modifiertype -> bool) -> unit -> Gobject.Signal.handler_id
    method forward : GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.widget_t -> bool
...
class event_controller_key (obj : Event_controller_key.t) : event_controller_key_t = object (self)
  ...
  method on_im_update ?(after = false) ~callback () =
    Event_controller_key.on_im_update ~after self#as_event_controller_key ~callback

  method on_key_pressed ?(after = false) ~callback () =
    Event_controller_key.on_key_pressed ~after self#as_event_controller_key ~callback

  method on_key_released ?(after = false) ~callback () =
    Event_controller_key.on_key_released ~after self#as_event_controller_key ~callback

  method on_modifiers ?(after = false) ~callback () =
    Event_controller_key.on_modifiers ~after self#as_event_controller_key ~callback
