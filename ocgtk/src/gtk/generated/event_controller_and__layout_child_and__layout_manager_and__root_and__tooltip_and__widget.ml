...
and Tooltip
 : sig
  ...
  external set_custom : t -> Widget.t option -> unit = "ml_gtk_tooltip_set_custom"
...
and Widget
 : sig
  ...
  external get_template_child : t -> Gobject.Type.t -> string -> [`object_] Gobject.obj = "ml_gtk_widget_get_template_child"
  ...
  external get_ancestor : t -> Gobject.Type.t -> t option = "ml_gtk_widget_get_ancestor"
  ...
  ``` *)
  external dispose_template : t -> Gobject.Type.t -> unit = "ml_gtk_widget_dispose_template"
  ...
  val on_query_tooltip : ?after:bool -> t -> callback:(x:int -> y:int -> keyboard_mode:bool -> tooltip:Tooltip.t -> bool) -> Gobject.Signal.handler_id
  ...
  let on_direction_changed ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let previous_direction = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.textdirection_of_int (Gobject.Value.get_enum_int v)) in
  ...
  let on_keynav_failed ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let direction = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.directiontype_of_int (Gobject.Value.get_enum_int v)) in
  ...
  let on_mnemonic_activate ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let group_cycling = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_boolean v) in
  ...
  let on_query_tooltip ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let x = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_int v) in
      let y = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
      let keyboard_mode = (let v = (Gobject.Closure.nth argv ~pos:3) in Gobject.Value.get_boolean v) in
      let tooltip = (let v = (Gobject.Closure.nth argv ~pos:4) in Gobject.Value.get_object_exn v) in
      let result = callback ~x ~y ~keyboard_mode ~tooltip in
      let v = Gobject.Closure.result argv in
      let x = result in
      Gobject.Value.set_boolean v x) in
    Gobject.Signal.connect obj ~name:"query-tooltip" ~callback:closure
      ~after:(Option.value after ~default:false)
  ...
  let on_state_flags_changed ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let flags = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.stateflags_of_int (Gobject.Value.get_flags_int v)) in
      callback ~flags)
   in
