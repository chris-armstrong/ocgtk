...
class shortcut (obj : Shortcut.t) : shortcut_t =
  object (self)
    method get_action : unit -> GShortcut_action.shortcut_action_t option =
      fun () ->
        Option.map
          (fun ret -> new GShortcut_action.shortcut_action ret)
          (Shortcut.get_action obj)

    method get_arguments : unit -> Gvariant.t option =
      fun () -> Shortcut.get_arguments obj

    method get_trigger : unit -> GShortcut_trigger.shortcut_trigger_t option =
      fun () ->
        Option.map
          (fun ret -> new GShortcut_trigger.shortcut_trigger ret)
          (Shortcut.get_trigger obj)
    ...
    method set_arguments : Gvariant.t option -> unit =
      fun args -> Shortcut.set_arguments obj args
...
let new_ (trigger : GShortcut_trigger.shortcut_trigger_t option)
    (action : GShortcut_action.shortcut_action_t option) : shortcut_t =
  let trigger = Option.map (fun c -> c#as_shortcut_trigger) trigger in
  let action = Option.map (fun c -> c#as_shortcut_action) action in
