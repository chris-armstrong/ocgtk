...
class cell_renderer_combo (obj : Cell_renderer_combo.t) : cell_renderer_combo_t
  =
  object (self)
    inherit GCell_renderer_text.cell_renderer_text (obj :> Cell_renderer_text.t)

    method on_changed ?(after = false) ~callback () =
      Cell_renderer_combo.on_changed ~after self#as_cell_renderer_combo
        ~callback
    ...
    method model =
      new GTree_model.tree_model (Cell_renderer_combo.get_model obj)

    method set_model : GTree_model.tree_model_t -> unit =
      fun v -> Cell_renderer_combo.set_model obj v#as_tree_model
