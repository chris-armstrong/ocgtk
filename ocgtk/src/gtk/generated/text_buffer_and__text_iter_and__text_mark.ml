...
module rec Text_buffer : sig
  ...
  val on_mark_deleted : ?after:bool -> t -> callback:(mark:Text_mark.t -> unit) -> Gobject.Signal.handler_id
  ...
  let on_mark_deleted ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let mark = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
      callback ~mark)
   in
  ...
  let on_paste_done ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let clipboard = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
      callback ~clipboard)
   in
