...
class application_window (obj : Application_window.t) : application_window_t =
  object (self)
    inherit
    ...
    inherit
      Ocgtk_gio.Gio.Action_group.action_group
        (Ocgtk_gio.Gio.Wrappers.Action_group.from_gobject obj)

    inherit
      Ocgtk_gio.Gio.Action_map.action_map
        (Ocgtk_gio.Gio.Wrappers.Action_map.from_gobject obj)

    method get_help_overlay :
        unit -> GShortcuts_window.shortcuts_window_t option =
      fun () ->
        Option.map
          (fun ret -> new GShortcuts_window.shortcuts_window ret)
          (Application_window.get_help_overlay obj)

    method get_id : unit -> int = fun () -> Application_window.get_id obj

    method get_show_menubar : unit -> bool =
      fun () -> Application_window.get_show_menubar obj

    method set_help_overlay :
        GShortcuts_window.shortcuts_window_t option -> unit =
      fun help_overlay ->
        let help_overlay =
          Option.map (fun c -> c#as_shortcuts_window) help_overlay
        in
        Application_window.set_help_overlay obj help_overlay

    method set_show_menubar : bool -> unit =
      fun show_menubar -> Application_window.set_show_menubar obj show_menubar

    method as_application_window = obj
  end

let new_
    (application : GApplication_and__window_and__window_group.application_t) :
