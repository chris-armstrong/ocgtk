...
external set_child : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_combo_box_set_child"
...
external get_child : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option = "ml_gtk_combo_box_get_child"
...
let on_format_entry_text ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    ...
    let x = result in
    Gobject.Value.set_string v x) in
