class type cell_renderer_toggle_t = object
    ...
    method on_toggled : ?after:bool -> callback:(path:string -> unit) -> unit -> Gobject.Signal.handler_id
...
class cell_renderer_toggle (obj : Cell_renderer_toggle.t) : cell_renderer_toggle_t = object (self)
  ...
  method on_toggled ?(after = false) ~callback () =
    Cell_renderer_toggle.on_toggled ~after self#as_cell_renderer_toggle ~callback
