...
let on_swipe ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let velocity_x = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_double v) in
    let velocity_y = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_double v) in
