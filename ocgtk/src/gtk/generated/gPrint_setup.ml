...
class print_setup (obj : Print_setup.t) : print_setup_t = object (self)
  ...
  method get_page_setup : unit -> GPage_setup.page_setup_t option =
    fun () ->
      Option.map (fun ret -> new GPage_setup.page_setup ret) (Print_setup.get_page_setup obj)

  method get_print_settings : unit -> GPrint_settings.print_settings_t option =
    fun () ->
      Option.map (fun ret -> new GPrint_settings.print_settings ret) (Print_setup.get_print_settings obj)
