...
class popover_menu (obj : Popover_menu.t) : popover_menu_t =
  object (self)
    ...
    method get_flags : unit -> Gtk_enums.popovermenuflags =
      fun () -> Popover_menu.get_flags obj
    ...
    method get_menu_model : unit -> Ocgtk_gio.Gio.Menu_model.menu_model_t option
      ...
      fun () ->
        Option.map
          ...
          (Popover_menu.get_menu_model obj)
    ...
    method set_flags : Gtk_enums.popovermenuflags -> unit =
      fun flags -> Popover_menu.set_flags obj flags
...
let new_from_model (model : Ocgtk_gio.Gio.Menu_model.menu_model_t option) :
    popover_menu_t =
  let model = Option.map (fun c -> c#as_menu_model) model in
  let obj_ = Popover_menu.new_from_model model in
  new popover_menu obj_

let new_from_model_full (model : Ocgtk_gio.Gio.Menu_model.menu_model_t)
    (flags : Gtk_enums.popovermenuflags) : popover_menu_t =
