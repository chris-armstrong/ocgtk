...
class file_chooser_native (obj : File_chooser_native.t) : file_chooser_native_t
  =
  object (self)
    ...
    method get_accept_label : unit -> string option =
      fun () -> File_chooser_native.get_accept_label obj

    method get_cancel_label : unit -> string option =
      fun () -> File_chooser_native.get_cancel_label obj

    method set_accept_label : string option -> unit =
      fun accept_label -> File_chooser_native.set_accept_label obj accept_label

    method set_cancel_label : string option -> unit =
      fun cancel_label -> File_chooser_native.set_cancel_label obj cancel_label
...
let new_ (title : string option)
    ...
    (action : Gtk_enums.filechooseraction) (accept_label : string option)
    (cancel_label : string option) : file_chooser_native_t =
  let parent = Option.map (fun c -> c#as_window) parent in
  let obj_ =
    File_chooser_native.new_ title parent action accept_label cancel_label
  in
