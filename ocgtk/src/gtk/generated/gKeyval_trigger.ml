...
class keyval_trigger (obj : Keyval_trigger.t) : keyval_trigger_t =
  object (self)
    ...
    method get_modifiers : unit -> Ocgtk_gdk.Gdk.modifiertype =
      fun () -> Keyval_trigger.get_modifiers obj

    method as_keyval_trigger = obj
  end

let new_ (keyval : int) (modifiers : Ocgtk_gdk.Gdk.modifiertype) :
    keyval_trigger_t =
