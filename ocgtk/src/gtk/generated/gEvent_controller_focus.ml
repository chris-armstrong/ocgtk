...
class event_controller_focus (obj : Event_controller_focus.t) : event_controller_focus_t = object (self)
  ...
  method on_enter ?(after = false) ~callback () =
    Event_controller_focus.on_enter ~after self#as_event_controller_focus ~callback

  method on_leave ?(after = false) ~callback () =
    Event_controller_focus.on_leave ~after self#as_event_controller_focus ~callback
