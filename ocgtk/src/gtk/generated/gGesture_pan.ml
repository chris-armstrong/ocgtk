class type gesture_pan_t = object
    ...
    method on_pan : ?after:bool -> callback:(direction:Gtk_enums.pandirection -> offset:float -> unit) -> unit -> Gobject.Signal.handler_id
...
class gesture_pan (obj : Gesture_pan.t) : gesture_pan_t = object (self)
  ...
  method on_pan ?(after = false) ~callback () =
    Gesture_pan.on_pan ~after self#as_gesture_pan ~callback
