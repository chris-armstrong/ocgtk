...
class icon_view_signals (obj : Icon_view.t) =
  object
    method on_select_all ~callback =
      Gobject.Signal.connect_simple obj ~name:"select-all" ~callback
        ~after:false
    (** A [keybinding signal][class@Gtk.SignalAction] which gets emitted when
    ...
    method on_select_cursor_item ~callback =
      Gobject.Signal.connect_simple obj ~name:"select-cursor-item" ~callback
        ~after:false
    (** A [keybinding signal][class@Gtk.SignalAction] which gets emitted when
        the user selects the item that is currently focused.
    ...
    method on_toggle_cursor_item ~callback =
      Gobject.Signal.connect_simple obj ~name:"toggle-cursor-item" ~callback
        ~after:false
    (** A [keybinding signal][class@Gtk.SignalAction] which gets emitted when
        the user toggles whether the currently focused item is selected or not.
