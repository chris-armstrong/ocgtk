class type combo_box_t = object
    ...
    method on_move_active : ?after:bool -> callback:(scroll_type:Gtk_enums.scrolltype -> unit) -> unit -> Gobject.Signal.handler_id
...
class combo_box (obj : Combo_box.t) : combo_box_t = object (self)
  ...
  method get_child : unit -> GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.widget_t option =
    fun () ->
      Option.map (fun ret -> new GEvent_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.widget ret) (Combo_box.get_child obj)
