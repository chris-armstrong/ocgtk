...
class string_list (obj : String_list.t) : string_list_t =
  object (self)
    inherit
    ...
    method remove : int -> unit =
      fun position -> String_list.remove obj position

    method splice : int -> int -> string array option -> unit =
      fun position n_removals additions ->
        String_list.splice obj position n_removals additions

    method take : string -> unit = fun string -> String_list.take obj string
