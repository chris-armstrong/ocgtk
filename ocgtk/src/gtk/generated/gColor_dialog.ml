class type color_dialog_t = object
    method choose_rgba_finish : Ocgtk_gio.Gio.Async_result.async_result_t -> (Ocgtk_gdk.Gdk.Rgb_a.rgb_a_t option, GError.t) result
...
class color_dialog (obj : Color_dialog.t) : color_dialog_t = object (self)
  ...
  method choose_rgba_finish : Ocgtk_gio.Gio.Async_result.async_result_t -> (Ocgtk_gdk.Gdk.Rgb_a.rgb_a_t option, GError.t) result =
    fun result ->
      ...
      Result.map (fun ret -> Option.map (fun ret -> new Ocgtk_gdk.Gdk.Rgb_a.rgb_a ret) ret)(Color_dialog.choose_rgba_finish obj result)
...
let new_ () : color_dialog_t =
  new color_dialog (Color_dialog.new_ ())
