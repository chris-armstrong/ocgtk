...
let on_angle_changed ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let angle = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_double v) in
    let angle_delta = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_double v) in
