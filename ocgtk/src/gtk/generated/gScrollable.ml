...
class scrollable (obj : Scrollable.t) : scrollable_t =
  object (self)
    method get_hadjustment : unit -> GAdjustment.adjustment_t option =
      fun () ->
        Option.map
          (fun ret -> new GAdjustment.adjustment ret)
          (Scrollable.get_hadjustment obj)

    method get_hscroll_policy : unit -> Gtk_enums.scrollablepolicy =
      fun () -> Scrollable.get_hscroll_policy obj

    method get_vadjustment : unit -> GAdjustment.adjustment_t option =
      fun () ->
        Option.map
          (fun ret -> new GAdjustment.adjustment ret)
          (Scrollable.get_vadjustment obj)

    method get_vscroll_policy : unit -> Gtk_enums.scrollablepolicy =
      fun () -> Scrollable.get_vscroll_policy obj
    ...
    method set_hscroll_policy : Gtk_enums.scrollablepolicy -> unit =
      fun policy -> Scrollable.set_hscroll_policy obj policy
    ...
    method set_vscroll_policy : Gtk_enums.scrollablepolicy -> unit =
      fun policy -> Scrollable.set_vscroll_policy obj policy
