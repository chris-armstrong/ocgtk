...
class snapshot (obj : Snapshot.t) : snapshot_t =
  object (self)
    inherit
      Ocgtk_gdk.Gdk.Snapshot.snapshot (obj :> Ocgtk_gdk.Gdk.Wrappers.Snapshot.t)
    ...
    method append_cairo :
        Ocgtk_graphene.Graphene.Rect.rect_t ->
        Ocgtk_cairo.Cairo.Context.context_t =
      fun bounds ->
    ...
    method append_linear_gradient :
      ...
      fun bounds start_point end_point stops n_stops ->
        ...
        Snapshot.append_linear_gradient obj bounds start_point end_point stops
          n_stops
    ...
    method append_radial_gradient :
      ...
      fun bounds center hradius vradius start end_ stops n_stops ->
        let bounds = bounds#as_rect in
        let center = center#as_point in
        Snapshot.append_radial_gradient obj bounds center hradius vradius start
          end_ stops n_stops
    ...
    method append_repeating_linear_gradient :
      ...
      fun bounds start_point end_point stops n_stops ->
        ...
        let start_point = start_point#as_point in
        let end_point = end_point#as_point in
        Snapshot.append_repeating_linear_gradient obj bounds start_point
          end_point stops n_stops
    ...
    method append_repeating_radial_gradient :
      ...
      fun bounds center hradius vradius start end_ stops n_stops ->
        let bounds = bounds#as_rect in
        let center = center#as_point in
        Snapshot.append_repeating_radial_gradient obj bounds center hradius
          vradius start end_ stops n_stops
    ...
    method gl_shader_pop_texture : unit -> unit =
      fun () -> Snapshot.gl_shader_pop_texture obj

    method perspective : float -> unit =
      fun depth -> Snapshot.perspective obj depth

    method pop : unit -> unit = fun () -> Snapshot.pop obj

    method push_blend : Ocgtk_gsk.Gsk.blendmode -> unit =
      fun blend_mode -> Snapshot.push_blend obj blend_mode

    method push_blur : float -> unit =
      fun radius -> Snapshot.push_blur obj radius
    ...
    method push_fill :
        Ocgtk_gsk.Gsk.Path.path_t -> Ocgtk_gsk.Gsk.fillrule -> unit =
      fun path fill_rule ->
        let path = path#as_path in
    ...
    method push_mask : Ocgtk_gsk.Gsk.maskmode -> unit =
      fun mask_mode -> Snapshot.push_mask obj mask_mode

    method push_opacity : float -> unit =
      fun opacity -> Snapshot.push_opacity obj opacity
    ...
    method push_shadow :
        Ocgtk_gsk.Gsk.Wrappers.Shadow.t array -> Gsize.t -> unit =
      fun shadow n_shadows -> Snapshot.push_shadow obj shadow n_shadows
    ...
    method scale : float -> float -> unit =
      fun factor_x factor_y -> Snapshot.scale obj factor_x factor_y
    ...
    method to_node : unit -> Ocgtk_gsk.Gsk.Render_node.render_node_t option =
      fun () ->
        Option.map
          (fun ret -> new Ocgtk_gsk.Gsk.Render_node.render_node ret)
          (Snapshot.to_node obj)
    ...
    method to_paintable :
      ...
      fun size ->
        ...
        Option.map
          ...
          (Snapshot.to_paintable obj size)
...
let new_ () : snapshot_t = new snapshot (Snapshot.new_ ())
