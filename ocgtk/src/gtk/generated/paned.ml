...
external set_start_child : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_paned_set_start_child"
...
If @child is `NULL`, the existing child will be removed. *)
external set_end_child : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option -> unit = "ml_gtk_paned_set_end_child"

(** Gets whether the separator should be wide. *)
external get_wide_handle : t -> bool = "ml_gtk_paned_get_wide_handle"

(** Retrieves the start child of the given `GtkPaned`. *)
external get_start_child : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option = "ml_gtk_paned_get_start_child"
...
external get_end_child : t -> Event_controller_and__layout_child_and__layout_manager_and__root_and__tooltip_and__widget.Widget.t option = "ml_gtk_paned_get_end_child"
...
let on_cycle_child_focus ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let reversed = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_boolean v) in
...
let on_move_handle ?after obj ~callback =
  let closure = Gobject.Closure.create (fun argv ->
    let scroll_type = (let v = (Gobject.Closure.nth argv ~pos:1) in Gtk_enums.scrolltype_of_int (Gobject.Value.get_enum_int v)) in
