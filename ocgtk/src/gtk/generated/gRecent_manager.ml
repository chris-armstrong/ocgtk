...
class recent_manager (obj : Recent_manager.t) : recent_manager_t = object (self)
  inherit Grecent_manager_signals.recent_manager_signals obj

  method add_full : string -> Recent_data.t -> bool =
    fun uri recent_data ->
      (Recent_manager.add_full obj uri recent_data)
  ...
  method has_item : string -> bool =
    fun uri ->
      (Recent_manager.has_item obj uri)

  method lookup_item : string -> (Recent_info.t option, GError.t) result =
    fun uri ->
      (Recent_manager.lookup_item obj uri)

  method move_item : string -> string option -> (bool, GError.t) result =
    fun uri new_uri ->
      (Recent_manager.move_item obj uri new_uri)

  method purge_items : unit -> (int, GError.t) result =
    fun () ->
      (Recent_manager.purge_items obj)

  method remove_item : string -> (bool, GError.t) result =
    fun uri ->
      (Recent_manager.remove_item obj uri)
