...
class type at_context_t = object
    method on_state_change : ?after:bool -> callback:(unit -> unit) -> unit -> Gobject.Signal.handler_id
...
class at_context (obj : At_context_and__accessible.At_context.t) : at_context_t = object (self)
  method on_state_change ?(after = false) ~callback () =
    At_context_and__accessible.At_context.on_state_change ~after self#as_at_context ~callback
