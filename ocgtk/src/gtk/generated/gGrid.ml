...
class grid (obj : Grid.t) : grid_t =
  object (self)
        ...
        Option.map
          (fun ret ->
            new
              ...
              .widget
              ret)
          (Grid.get_child_at obj column row)

    method get_column_homogeneous : unit -> bool =
      fun () -> Grid.get_column_homogeneous obj

    method get_column_spacing : unit -> int =
      fun () -> Grid.get_column_spacing obj

    method get_row_baseline_position : int -> Gtk_enums.baselineposition =
      fun row -> Grid.get_row_baseline_position obj row

    method get_row_homogeneous : unit -> bool =
      fun () -> Grid.get_row_homogeneous obj

    method get_row_spacing : unit -> int = fun () -> Grid.get_row_spacing obj

    method insert_column : int -> unit =
      fun position -> Grid.insert_column obj position
    ...
    method insert_row : int -> unit =
      fun position -> Grid.insert_row obj position
    ...
    method remove_column : int -> unit =
      fun position -> Grid.remove_column obj position

    method remove_row : int -> unit =
      fun position -> Grid.remove_row obj position

    method set_baseline_row : int -> unit =
      fun row -> Grid.set_baseline_row obj row

    method set_column_homogeneous : bool -> unit =
      fun homogeneous -> Grid.set_column_homogeneous obj homogeneous

    method set_column_spacing : int -> unit =
      fun spacing -> Grid.set_column_spacing obj spacing

    method set_row_baseline_position : int -> Gtk_enums.baselineposition -> unit
        =
      fun row pos -> Grid.set_row_baseline_position obj row pos

    method set_row_homogeneous : bool -> unit =
      fun homogeneous -> Grid.set_row_homogeneous obj homogeneous

    method set_row_spacing : int -> unit =
      fun spacing -> Grid.set_row_spacing obj spacing

    method as_grid = obj
  end

let new_ () : grid_t = new grid (Grid.new_ ())
