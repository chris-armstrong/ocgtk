...
class pixbuf (obj : Pixbuf.t) : pixbuf_t =
  object (self)
    inherit
      Ocgtk_gio.Gio.Icon.icon (Ocgtk_gio.Gio.Wrappers.Icon.from_gobject obj)

    inherit
      Ocgtk_gio.Gio.Loadable_icon.loadable_icon
        (Ocgtk_gio.Gio.Wrappers.Loadable_icon.from_gobject obj)

    method apply_embedded_orientation : unit -> pixbuf_t option =
      fun () ->
        Option.map
          (fun ret -> new pixbuf ret)
          (Pixbuf.apply_embedded_orientation obj)
    ...
    method composite :
        ...
        int ->
        unit =
      fun dest dest_x dest_y dest_width dest_height offset_x offset_y scale_x
          scale_y interp_type overall_alpha ->
        let dest = dest#as_pixbuf in
        Pixbuf.composite obj dest dest_x dest_y dest_width dest_height offset_x
          offset_y scale_x scale_y interp_type overall_alpha
    ...
    method composite_color :
        ...
        unit =
      fun dest dest_x dest_y dest_width dest_height offset_x offset_y scale_x
          scale_y interp_type overall_alpha check_x check_y check_size color1
          color2 ->
        let dest = dest#as_pixbuf in
        Pixbuf.composite_color obj dest dest_x dest_y dest_width dest_height
          offset_x offset_y scale_x scale_y interp_type overall_alpha check_x
    ...
    method composite_color_simple :
        ...
        UInt32.t ->
        pixbuf_t option =
      fun dest_width dest_height interp_type overall_alpha check_size color1
          color2 ->
        Option.map
          (fun ret -> new pixbuf ret)
          (Pixbuf.composite_color_simple obj dest_width dest_height interp_type
             overall_alpha check_size color1 color2)

    method copy : unit -> pixbuf_t option =
      fun () -> Option.map (fun ret -> new pixbuf ret) (Pixbuf.copy obj)

    method copy_area :
        int -> int -> int -> int -> pixbuf_t -> int -> int -> unit =
    ...
    method fill : UInt32.t -> unit = fun pixel -> Pixbuf.fill obj pixel
    ...
    method get_bits_per_sample : unit -> int =
      fun () -> Pixbuf.get_bits_per_sample obj

    method get_byte_length : unit -> Gsize.t =
      fun () -> Pixbuf.get_byte_length obj

    method get_colorspace : unit -> Gdkpixbuf_enums.colorspace =
      fun () -> Pixbuf.get_colorspace obj

    method get_has_alpha : unit -> bool = fun () -> Pixbuf.get_has_alpha obj
    method get_height : unit -> int = fun () -> Pixbuf.get_height obj
    method get_n_channels : unit -> int = fun () -> Pixbuf.get_n_channels obj

    method get_option : string -> string option =
      fun key -> Pixbuf.get_option obj key

    method get_rowstride : unit -> int = fun () -> Pixbuf.get_rowstride obj
    method get_width : unit -> int = fun () -> Pixbuf.get_width obj
    ...
    method read_pixel_bytes : unit -> Glib_bytes.t =
      fun () -> Pixbuf.read_pixel_bytes obj

    method remove_option : string -> bool =
      fun key -> Pixbuf.remove_option obj key
    ...
    method save_to_streamv :
      ...
      fun stream type_ option_keys option_values cancellable ->
        ...
        Pixbuf.save_to_streamv obj stream type_ option_keys option_values
          cancellable
    ...
    method scale :
      ...
      fun dest dest_x dest_y dest_width dest_height offset_x offset_y scale_x
          scale_y interp_type ->
        let dest = dest#as_pixbuf in
        Pixbuf.scale obj dest dest_x dest_y dest_width dest_height offset_x
    ...
    method scale_simple :
      ...
      fun dest_width dest_height interp_type ->
        Option.map
          (fun ret -> new pixbuf ret)
          (Pixbuf.scale_simple obj dest_width dest_height interp_type)

    method set_option : string -> string -> bool =
      fun key value -> Pixbuf.set_option obj key value

    method pixel_bytes = Pixbuf.get_pixel_bytes obj
    method as_pixbuf = obj
  end

let new_ (colorspace : Gdkpixbuf_enums.colorspace) (has_alpha : bool)
    (bits_per_sample : int) (width : int) (height : int) : pixbuf_t =
  let obj_ = Pixbuf.new_ colorspace has_alpha bits_per_sample width height in
  new pixbuf obj_

let new_from_bytes (data : Glib_bytes.t)
    (colorspace : Gdkpixbuf_enums.colorspace) (has_alpha : bool)
    (bits_per_sample : int) (width : int) (height : int) (rowstride : int) :
    pixbuf_t =
  let obj_ =
    Pixbuf.new_from_bytes data colorspace has_alpha bits_per_sample width height
      rowstride
  in
  new pixbuf obj_
...
let new_from_file_at_scale (filename : string) (width : int) (height : int)
    (preserve_aspect_ratio : bool) : (pixbuf_t, GError.t) result =
  let obj_ =
    Pixbuf.new_from_file_at_scale filename width height preserve_aspect_ratio
  in
  Result.map (fun obj_ -> new pixbuf obj_) obj_

let new_from_file_at_size (filename : string) (width : int) (height : int) :
...
let new_from_resource_at_scale (resource_path : string) (width : int)
    (height : int) (preserve_aspect_ratio : bool) : (pixbuf_t, GError.t) result
    =
  let obj_ =
    Pixbuf.new_from_resource_at_scale resource_path width height
      preserve_aspect_ratio
  in
  Result.map (fun obj_ -> new pixbuf obj_) obj_

let new_from_stream (stream : Ocgtk_gio.Gio.Input_stream.input_stream_t)
    (cancellable : Ocgtk_gio.Gio.Cancellable.cancellable_t option) :
    (pixbuf_t, GError.t) result =
...
let new_from_stream_at_scale
    (stream : Ocgtk_gio.Gio.Input_stream.input_stream_t) (width : int)
    (height : int) (preserve_aspect_ratio : bool)
    (cancellable : Ocgtk_gio.Gio.Cancellable.cancellable_t option) :
    (pixbuf_t, GError.t) result =
  let stream = stream#as_input_stream in
  let cancellable = Option.map (fun c -> c#as_cancellable) cancellable in
  let obj_ =
    Pixbuf.new_from_stream_at_scale stream width height preserve_aspect_ratio
      cancellable
  in
  Result.map (fun obj_ -> new pixbuf obj_) obj_

let new_from_stream_finish
    (async_result : Ocgtk_gio.Gio.Async_result.async_result_t) :
    (pixbuf_t, GError.t) result =
