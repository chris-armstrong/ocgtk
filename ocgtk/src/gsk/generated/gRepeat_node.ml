...
class repeat_node (obj : Repeat_node.t) : repeat_node_t =
  object (self)
    inherit GRender_node.render_node (obj :> Render_node.t)

    method get_child : unit -> GRender_node.render_node_t =
      fun () -> new GRender_node.render_node (Repeat_node.get_child obj)
...
let new_ (bounds : Ocgtk_graphene.Graphene.Rect.rect_t)
    (child : GRender_node.render_node_t)
    (child_bounds : Ocgtk_graphene.Graphene.Rect.rect_t option) : repeat_node_t
    =
