...
class shader_args_builder (obj : Shader_args_builder.t) : shader_args_builder_t = object (self)
  ...
  method set_int : int -> int32 -> unit =
    fun idx value ->
      (Shader_args_builder.set_int obj idx value)

  method set_uint : int -> int -> unit =
    fun idx value ->
      (Shader_args_builder.set_uint obj idx value)
  ...
  method to_args : unit -> Glib_bytes.t =
    fun () ->
      (Shader_args_builder.to_args obj)
...
let new_ (shader : GGl_shader.gl_shader_t) (initial_values : Glib_bytes.t option) : shader_args_builder_t =
