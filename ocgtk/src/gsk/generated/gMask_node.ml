...
class mask_node (obj : Mask_node.t) : mask_node_t =
  object (self)
    inherit GRender_node.render_node (obj :> Render_node.t)

    method get_mask : unit -> GRender_node.render_node_t =
      fun () -> new GRender_node.render_node (Mask_node.get_mask obj)

    method get_mask_mode : unit -> Gsk_enums.maskmode =
      fun () -> Mask_node.get_mask_mode obj

    method get_source : unit -> GRender_node.render_node_t =
      fun () -> new GRender_node.render_node (Mask_node.get_source obj)

    method as_mask_node = obj
  end

let new_ (source : GRender_node.render_node_t)
    (mask : GRender_node.render_node_t) (mask_mode : Gsk_enums.maskmode) :
    mask_node_t =
