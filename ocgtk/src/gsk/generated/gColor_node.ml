...
class color_node (obj : Color_node.t) : color_node_t =
  object (self)
    inherit GRender_node.render_node (obj :> Render_node.t)

    method get_color : unit -> Ocgtk_gdk.Gdk.Rgb_a.rgb_a_t =
      fun () -> new Ocgtk_gdk.Gdk.Rgb_a.rgb_a (Color_node.get_color obj)

    method as_color_node = obj
  end

let new_ (rgba : Ocgtk_gdk.Gdk.Rgb_a.rgb_a_t)
    (bounds : Ocgtk_graphene.Graphene.Rect.rect_t) : color_node_t =
