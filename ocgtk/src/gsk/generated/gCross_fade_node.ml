...
class cross_fade_node (obj : Cross_fade_node.t) : cross_fade_node_t =
  object (self)
    inherit GRender_node.render_node (obj :> Render_node.t)

    method get_end_child : unit -> GRender_node.render_node_t =
      fun () -> new GRender_node.render_node (Cross_fade_node.get_end_child obj)

    method get_progress : unit -> float =
      fun () -> Cross_fade_node.get_progress obj
...
let new_ (start : GRender_node.render_node_t)
    (end_ : GRender_node.render_node_t) (progress : float) : cross_fade_node_t =
