...
class texture_node (obj : Texture_node.t) : texture_node_t =
  object (self)
    inherit GRender_node.render_node (obj :> Render_node.t)

    method get_texture : unit -> Ocgtk_gdk.Gdk.Texture.texture_t =
      fun () -> new Ocgtk_gdk.Gdk.Texture.texture (Texture_node.get_texture obj)

    method as_texture_node = obj
  end

let new_ (texture : Ocgtk_gdk.Gdk.Texture.texture_t)
    (bounds : Ocgtk_graphene.Graphene.Rect.rect_t) : texture_node_t =
