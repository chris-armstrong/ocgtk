...
let generate_property_methods ~ctx ~module_name ~current_layer2_module ~seen ~same_cluster_classes:_ (prop : gir_property) =
  let generate_getter _prop prop_snake =
    let method_name = prop_snake |> Utils.sanitize_identifier in
    let impl = match Class_gen_type_resolution.resolve_layer2_class_name ~ctx ~current_layer2_module ~gir_type:prop.prop_type with
      | Some class_name ->
        if prop.prop_type.nullable then
          sprintf "(%s.get_%s obj) |> Option.map (fun x -> new %s x)" module_name prop_snake class_name
        else
