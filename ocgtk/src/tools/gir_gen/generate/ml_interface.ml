...
let generate_signal_bindings ~output_mode:_ ~module_name:_ ~has_widget_parent:_
    _signals =
...
let build_constructor_signature ~ctx ~class_name ctor =
  let param_types =
    List.map ~f:(map_constructor_param ~ctx ~class_name) ctor.ctor_parameters
  in
...
let build_method_signature ~ctx ~class_name (meth : gir_method) =
  let param_types =
    List.filter_map
      ~f:(convert_method_param_to_ocaml_type ~ctx ~class_name)
      meth.parameters
  in
  ...
  let out_types =
    List.filter_map
      ~f:(convert_out_param_to_ocaml_type ~ctx ~class_name)
      meth.parameters
  in
...
let generate_type_declaration ~output_mode ~is_record ~base_type buf =
  ...
  | true, Interface -> bprintf buf "type t\n\n"
...
let generate_hierarchy_accessor_section ~ctx ~output_mode ~class_name buf =
  ...
  | Some hier_info when hier_info.hierarchy <> MonomorphicType ->
      if should_generate_accessor ~class_name hier_info then begin
        ...
        in
        let declaration =
          format_accessor_declaration ~output_mode ~accessor ~base_type
        in
...
let generate_methods_section ~ctx ~class_name ~c_type ~c_symbol_prefix
  ...
  List.iter
    ~f:(fun (meth : gir_method) ->
      generate_method_decl ~ctx ~class_name ~c_type ~c_symbol_prefix ~is_record
        ~buf meth)
    (List.rev methods)
...
let generate_ml_interface ~ctx ~output_mode ~class_name ~class_doc ~c_type
  ...
  let class_type_name, base_type =
    detect_class_hierarchy_names ~ctx ~class_name ~parent_chain
...
let generate_module_signature ~ctx ~entity ~base_type buf =
  let signature_contents =
    ...
    generate_ml_interface_internal ~ctx ~output_mode:Interface
...
let generate_combined_module_entity ~ctx ~output_mode ~entity
  ...
  let is_impl =
    match output_mode with Implementation -> true | Interface -> false
  in
