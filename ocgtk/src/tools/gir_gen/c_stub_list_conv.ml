...
let is_glist_type (gir_type : gir_type) =
  String.equal gir_type.name "GLib.List"
...
let c_type_of_list_kind = function
  | `GList -> "GList*"
  | `GSList -> "GSList*"

(** Get the element converter name for a given element type.
    This returns the Val_* converter function name for converting
    C element data to OCaml values. *)
let element_converter_name ~(ctx : generation_context) (elem_type : gir_type) : string option =
...
(** Generate C code for converting a GList/GSList return value to OCaml list.
    ...
    1. Declares CAMLlocal3(result, item, cell) at function scope
    2. Calls the appropriate Val_GList_with or Val_GSList_with macro
    3. Handles transfer ownership cleanup *)
let generate_list_c_to_ml ~(ctx : generation_context) ~var ~(elem_type : gir_type) ~(kind : list_kind) ~(xfer : transfer_ownership) =
  let macro_name =
    match kind with
    | `GList -> "Val_GList_with"
    | `GSList -> "Val_GSList_with"
  in
      ...
      ( "CAMLlocal3(result, item, cell);",
        sprintf
          "%s(%s, result, item, cell, %s);\n\
           \    %s"
          macro_name var elem_conv cleanup,
        "CAMLreturn(result);" )

(** Generate C code for converting an OCaml list parameter to GList/GSList.
    
    This generates code that:
    1. Iterates over the OCaml list
    2. Converts each element to C
    3. Builds the GList/GSList *)
let generate_list_ml_to_c ~(ctx : generation_context) ~var ~(elem_type : gir_type) ~(kind : list_kind) ~xfer:(_xfer : transfer_ownership) =
  let macro_name =
    match kind with
    | `GList -> "GList_val_with"
    | `GSList -> "GSList_val_with"
  in
    ...
    | _ ->
        ...
        | Some (tm : type_mapping) ->
            if String.equal tm.c_to_ml "LIST_INLINE" then
              ...
              sprintf "(gpointer)%s(Field(_iter, 0))" tm.ml_to_c
        | None ->
            sprintf "/* TODO: No converter for %s */ NULL" elem_type.name
  in
...
(** Generate the full return statement for a method returning a GList/GSList.
    ...
    This is the main entry point called from c_stub_method.ml.
    It generates the complete C code block for converting the C list to OCaml.
    *)
let generate_return_list_conversion ~(ctx : generation_context) ~c_var ~(gir_type : gir_type) =
  ...
  | Some kind ->
      let elem_type : gir_type =
        ...
        | None -> { name = "gpointer"; c_type = Some "gpointer"; nullable = false; transfer_ownership = TransferNone; array = None }
      in
      Some (generate_list_c_to_ml ~ctx ~var:c_var ~elem_type ~kind ~xfer:gir_type.transfer_ownership)

(** Generate parameter conversion for a GList/GSList parameter.
    
    This is the main entry point for handling list parameters.
    Returns the variable name to use in the C function call.
    *)
let generate_param_list_conversion ~(ctx : generation_context) ~ocaml_var ~(gir_type : gir_type) =
  match list_kind_of_type gir_type with
  | None -> None  (* Not a list type *)
  | Some kind ->
      let elem_type : gir_type =
        ...
        | None -> { name = "gpointer"; c_type = Some "gpointer"; nullable = false; transfer_ownership = TransferNone; array = None }
      in
      let conversion_code = generate_list_ml_to_c ~ctx ~var:ocaml_var ~elem_type ~kind ~xfer:gir_type.transfer_ownership in
