...
This document provides comprehensive documentation on the testing strategy, infrastructure, and organization for the ocgtk GIR generator.
...
The ocgtk GIR generator produces OCaml bindings from GObject Introspection (GIR) XML files. The generator operates in multiple layers, each requiring its own testing strategy:
...
│  - C FFI stubs (*.c files)              │
│  - Type conversions (Val_GtkXxx macros) │
...
│
├── infrastructure/              # Shared testing infrastructure
│   ├── helpers.ml               # Shared test utilities
│   ├── c_parser.ml              # C code parser
│   ├── c_ast.ml                 # C AST types and inspection
│   ├── c_validation.ml          # C validation helpers
│   ├── ml_parser.ml             # ML code parser (TODO)
│   ├── ml_ast.ml                # ML AST types (TODO)
│   └── ml_validation.ml         # ML validation helpers (TODO)
│
├── c_stubs/                     # Layer 0: C Stub Generation Tests
│   ├── c_stubs_tests.ml         # Basic constructor/method generation
│   ├── nullable_tests.ml        # Nullable parameter handling
│   ├── error_handling_tests.ml  # GError handling (throws=true)
│   ├── out_params_tests.ml      # Out and InOut parameters
│   ├── type_conversion_tests.ml # Type conversions
│   └── edge_cases.ml            # Edge cases and boundaries
│
├── ml_generation/               # Layer 1: ML File Generation Tests (TODO)
│   ├── module_structure_tests.ml    # Module organization (TODO)
│   ├── type_definition_tests.ml     # Type definitions (TODO)
│   ├── external_decl_tests.ml       # External declarations (TODO)
│   ├── signature_tests.ml           # .mli signature generation (TODO)
│   └── edge_cases.ml                # ML-specific edge cases (TODO)
│
├── class_generation/            # Layer 2: Class Generation Tests (TODO)
│   ├── class_structure_tests.ml     # Class definitions (TODO)
│   ├── inheritance_tests.ml         # Inheritance hierarchy (TODO)
│   ├── method_dispatch_tests.ml     # Method dispatch (TODO)
│   ├── property_accessor_tests.ml   # Property accessors (TODO)
│   └── edge_cases.ml                # Class-specific edge cases (TODO)
│
└── integration/                 # Full pipeline integration tests
    ├── properties.ml            # Property getter/setter generation
    ├── signals.ml               # Signal handling generation
    ├── enums.ml                 # Enum and bitfield generation
    ├── records.ml               # Record type support
...
#### c_stubs_tests.ml - Basic Generation & Record Types ✓ (16/16)
...
**Record Type Tests:**
- [x] Non-opaque record return type ✓
- [x] Non-opaque record parameter ✓
- [x] Opaque record return type ✓
- [x] Opaque record parameter ✓
- [x] Nullable record return type ✓
- [x] Nullable record parameter ✓

#### nullable_tests.ml - Nullable Handling ✓ (5/5)
...
#### error_handling_tests.ml - Error Handling ✓ (6/6)
- [x] Method with throws=true declares GError ✓
- [x] Error handling uses Res_Ok/Res_Error ✓
- [x] Error is passed by reference (&error) ✓
- [x] Error is properly initialized to NULL ✓
- [x] Complete error handling infrastructure ✓
- [x] Constructor with throws ✓

#### out_params_tests.ml - Out/InOut Parameters ✓ (6/6)
...
#### type_conversion_tests.ml - Type Conversions ✓ (9/9)
...
- [x] Object type conversion (Val_GtkWidget) ✓
...
✅ **Completed:**
- [x] Property getters (GValue handling) ✓
- [x] Property setters (GValue handling) ✓
- [x] Enum and bitfield generation ✓
- [x] Signal handling ✓
- [x] Record types (opaque and non-opaque) ✓
- [x] Nullable record types ✓
- [x] Edge cases (empty classes, etc.) ✓
...
✅ Use library-based tests for unit testing individual features
✅ Use integration tests for end-to-end validation
✅ Test both success and failure cases
✅ Test edge cases (nullable, empty lists, etc.)
✅ Add descriptive test names and comments
✅ Log generated code when debugging
✅ Update this document when adding new tests
✅ Organize tests into appropriate subdirectories

### DON'T:

❌ Rely on parser for multi-line if/else (known limitation)
❌ Test full C compilation (out of scope)
❌ Create temporary files in library-based tests
❌ Hard-code file paths (use helper functions)
❌ Skip return value checks (`Option.get` is acceptable in tests)
