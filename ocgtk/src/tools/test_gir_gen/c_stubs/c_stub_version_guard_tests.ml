...
let assert_contains ~label ~expected actual =
  if not (String.length expected = 0 || (
...
let test_no_guard_when_same_version () =
  ...
  emit_with_member_guard ~ctx:gtk_ctx
    ~class_version:(Some "4.10") ~member_version:(Some "4.10")
    ~fallback:(fun _ -> "FALLBACK") ~stub buf;
...
let test_no_guard_when_no_versions () =
  ...
  let stub = "STUB_BODY" in
  emit_with_member_guard ~ctx:gtk_ctx
    ~class_version:None ~member_version:None
    ~fallback:(fun _ -> "FALLBACK") ~stub buf;
  let output = buf_to_string buf in
  Alcotest.(check string) "plain stub emitted" stub output
...
let test_member_guard_when_member_higher () =
  ...
  emit_with_member_guard ~ctx:gtk_ctx
    ~class_version:(Some "4.10") ~member_version:(Some "4.12")
    ~fallback:(fun _ -> fallback_text) ~stub buf;
  let output = buf_to_string buf in
  assert_contains ~label:"opening #if" ~expected:"#if GTK_CHECK_VERSION(4,12,0)" output;
...
let test_member_guard_when_class_has_no_version () =
  let buf = Buffer.create 256 in
  emit_with_member_guard ~ctx:gio_ctx
    ~class_version:None ~member_version:(Some "2.40")
    ~fallback:(fun _ -> "FALLBACK") ~stub:"REAL" buf;
  let output = buf_to_string buf in
  assert_contains ~label:"opening #if" ~expected:"#if GLIB_CHECK_VERSION(2,40,0)" output;
  assert_contains ~label:"#else" ~expected:"#else" output;
  assert_contains ~label:"#endif" ~expected:"#endif" output
...
let test_fallback_method_message_includes_version () =
  let meth = make_void_method ~method_name:"get_foo" ~c_identifier:"gtk_widget_get_foo" () in
  let v = { major = 4; minor = 12; micro = 0 } in
  let output =
    emit_fallback_method_stub ~ctx:gtk_ctx ~c_type:"GtkWidget" ~class_name:"Widget"
      ~ml_name:"ml_gtk_widget_get_foo" ~c_identifier:"gtk_widget_get_foo"
      ~version:v meth
  in
  assert_contains ~label:"version in message" ~expected:"requires GTK >= 4.12" output

let test_fallback_method_message_pango () =
  let meth = make_void_method ~method_name:"get_bar" ~c_identifier:"pango_font_map_get_bar" () in
  let v = { major = 1; minor = 46; micro = 0 } in
  let output =
    emit_fallback_method_stub ~ctx:pango_ctx ~c_type:"PangoFontMap" ~class_name:"FontMap"
      ~ml_name:"ml_pango_font_map_get_bar" ~c_identifier:"pango_font_map_get_bar"
      ~version:v meth
  in
  assert_contains ~label:"version in message" ~expected:"requires Pango >= 1.46" output
...
let test_fallback_method_suppresses_unused_params () =
  let meth =
    Type_factory.make_gir_method ~method_name:"set_foo" ~c_identifier:"gtk_w_set_foo"
      ~return_type:(Type_factory.make_return_type ~name:"none" ~c_type:(Some "void") ())
      ~parameters:[
        Type_factory.make_gir_param ~param_name:"value"
          ~param_type:(Type_factory.make_gir_type ~name:"gboolean" ~c_type:"gboolean" ()) ()
      ] ()
  in
  let v = { major = 4; minor = 12; micro = 0 } in
  let output =
    emit_fallback_method_stub ~ctx:gtk_ctx ~c_type:"GtkW" ~class_name:"W"
      ~ml_name:"ml_gtk_w_set_foo" ~c_identifier:"gtk_w_set_foo"
      ~version:v meth
  in
...
let test_suite =
  [
    ...
    ("member_guard_when_member_higher", `Quick, test_member_guard_when_member_higher);
    ("member_guard_when_class_has_no_version", `Quick, test_member_guard_when_class_has_no_version);
    ("guard_if_on_own_line", `Quick, test_guard_if_on_own_line);
    ("fallback_method_message_includes_version", `Quick, test_fallback_method_message_includes_version);
    ("fallback_method_message_pango", `Quick, test_fallback_method_message_pango);
    ("fallback_message_omits_micro_when_zero", `Quick, test_fallback_message_omits_micro_when_zero);
    ("fallback_message_includes_micro_when_nonzero", `Quick, test_fallback_message_includes_micro_when_nonzero);
    ("fallback_method_suppresses_unused_params", `Quick, test_fallback_method_suppresses_unused_params);
  ]
