...
let make_method ~name ~version =
  {
    ...
    return_type =
      { name = "void"; c_type = None; nullable = false;
...
let make_property ~name ~version =
  {
    prop_name = name;
    prop_type =
      { name = "gboolean"; c_type = Some "gboolean"; nullable = false;
        transfer_ownership = TransferNone; array = None };
    readable = true;
    writable = true;
...
let test_class_ignore () =
  let cls = make_class ~name:"Widget" ~version:None
    ~methods:[] ~constructors:[] ~properties:[] ~signals:[] in
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[cls] ~interfaces:[] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_class_ignore_preserves_surviving () =
  let cls1 = make_class ~name:"Widget" ~version:None
    ~methods:[] ~constructors:[] ~properties:[] ~signals:[] in
  let cls2 = make_class ~name:"Button" ~version:None
    ~methods:[] ~constructors:[] ~properties:[] ~signals:[] in
  let overrides = { (make_empty_overrides "Gtk") with
    classes = [{ class_name = "Widget"; class_action = Some Ignore;
      constructors = []; methods = []; properties = []; signals = [] }];
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[cls1; cls2] ~interfaces:[] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_method_ignore () =
  let cls = make_class ~name:"Widget" ~version:None
    ...
    ~constructors:[] ~properties:[] ~signals:[] in
  let overrides = { (make_empty_overrides "Gtk") with
  ...
  Alcotest.(check int) "one method remaining" 1 (List.length surviving_cls.methods);
...
let test_constructor_ignore () =
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[cls] ~interfaces:[] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
  let surviving_cls = hd_exn "Expected one class" result.classes in
  Alcotest.(check int) "no constructors" 0 (List.length surviving_cls.constructors)
...
let test_signal_ignore () =
  let cls = make_class ~name:"Widget" ~version:None
    ~methods:[] ~constructors:[]
    ~properties:[]
    ~signals:[make_signal ~name:"destroy" ~version:None] in
  let overrides = { (make_empty_overrides "Gtk") with
    classes = [{ class_name = "Widget"; class_action = None;
...
let test_interface_method_ignore () =
  let intf = make_interface ~name:"Actionable" ~version:None
    ~methods:[make_method ~name:"get_action_name" ~version:None;
              make_method ~name:"set_action_name" ~version:None]
    ~properties:[] ~signals:[] in
  let overrides = { (make_empty_overrides "Gtk") with
    interfaces = [{ interface_name = "Actionable"; interface_action = None;
      methods = [{ component_name = "get_action_name"; action = Ignore }];
      properties = []; signals = [] }];
  } in
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[] ~interfaces:[intf] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_interface_ignore () =
  let intf = make_interface ~name:"Actionable" ~version:None
    ~methods:[] ~properties:[] ~signals:[] in
  let overrides = { (make_empty_overrides "Gtk") with
    interfaces = [{ interface_name = "Actionable"; interface_action = Some Ignore;
      methods = []; properties = []; signals = [] }];
  } in
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[] ~interfaces:[intf] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_record_field_ignore () =
  let rec_ = make_record ~name:"TextIter" ~version:None
    ~fields:[make_record_field ~name:"user_data" ~version:None;
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[] ~interfaces:[] ~enums:[] ~bitfields:[] ~records:[rec_] ~functions:[] in
...
let test_record_ignore () =
  let rec_ = make_record ~name:"PrintBackend" ~version:None
    ~fields:[] ~constructors:[] ~methods:[] ~functions:[] in
  let overrides = { (make_empty_overrides "Gtk") with
    records = [{ record_name = "PrintBackend"; record_action = Some Ignore;
...
let test_enum_member_ignore () =
  let enm = make_enum ~name:"RGBA" ~version:None
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[] ~interfaces:[] ~enums:[enm] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_enum_ignore () =
  let enm = make_enum ~name:"License" ~version:None
    ~members:[] ~functions:[] in
  let overrides = { (make_empty_overrides "Gtk") with
    enums = [{ enum_name = "License"; enum_action = Some Ignore;
      members = []; functions = [] }];
  } in
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[] ~interfaces:[] ~enums:[enm] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_bitfield_member_ignore () =
  let bf = make_bitfield ~name:"StateFlags" ~version:None
    ~flags:[make_bitfield_member ~name:"ACTIVE" ~version:None;
            make_bitfield_member ~name:"NORMAL" ~version:None] in
  let overrides = { (make_empty_overrides "Gtk") with
...
let test_function_ignore () =
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[] ~interfaces:[] ~enums:[] ~bitfields:[] ~records:[] ~functions:[fn] in
...
let test_empty_overrides_no_changes () =
  ...
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[cls] ~interfaces:[intf] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_unknown_class_warning () =
  let overrides = { (make_empty_overrides "Gtk") with
    classes = [{ class_name = "NonExistent"; class_action = Some Ignore;
      constructors = []; methods = []; properties = []; signals = [] }];
  } in
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
  ...
  Alcotest.(check bool) "mentions unknown class" true
...
let test_unknown_component_warning () =
  ...
  let overrides = { (make_empty_overrides "Gtk") with
    classes = [{ class_name = "Widget"; class_action = None;
      constructors = [];
      methods = [{ component_name = "nonexistent_method"; action = Ignore }];
      properties = []; signals = [] }];
  } in
  let result = Gir_gen_lib.Override_apply.apply_overrides ~overrides
    ~classes:[cls] ~interfaces:[] ~enums:[] ~bitfields:[] ~records:[] ~functions:[] in
...
let test_combined_class_and_component () =
  let cls1 = make_class ~name:"Widget" ~version:None
    ...
    ~constructors:[] ~properties:[] ~signals:[] in
  let cls2 = make_class ~name:"Button" ~version:None
    ~methods:[make_method ~name:"clicked" ~version:None;
              make_method ~name:"activate" ~version:None]
    ~constructors:[] ~properties:[] ~signals:[] in
  let overrides = { (make_empty_overrides "Gtk") with
    classes = [
      { class_name = "Widget"; class_action = Some Ignore;
        constructors = []; methods = []; properties = []; signals = [] };
      { class_name = "Button"; class_action = None;
        constructors = [];
...
let test_suite =
  [
    ...
    ("class ignore preserves surviving", `Quick, test_class_ignore_preserves_surviving);
    ("method ignore", `Quick, test_method_ignore);
    ("method version override", `Quick, test_method_version_override);
    ("method version replaces existing", `Quick, test_method_version_replaces_existing);
    ...
    ("bitfield member version override", `Quick, test_bitfield_member_version_override);
    ...
    ("class ignore no component warnings", `Quick, test_class_ignore_no_component_warnings);
