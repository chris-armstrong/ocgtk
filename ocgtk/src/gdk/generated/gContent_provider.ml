...
class content_provider (obj : Content_provider.t) : content_provider_t = object (self)
  method on_content_changed ?(after = false) ~callback () =
    Content_provider.on_content_changed ~after self#as_content_provider ~callback
...
let new_union (providers : Content_provider.t array option) (n_providers : Gsize.t) : content_provider_t =
