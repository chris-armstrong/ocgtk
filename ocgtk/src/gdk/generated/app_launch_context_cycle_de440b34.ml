...
and Clipboard
 : sig
  ...
  external set_value : t -> Gobject.Value.t -> unit = "ml_gdk_clipboard_set_value"
  ...
  external read_value_finish : t -> Ocgtk_gio.Gio.Wrappers.Async_result.t -> (Gobject.Value.t, GError.t) result = "ml_gdk_clipboard_read_value_finish"
...
and Device
 : sig
  ...
  val on_changed : ?after:bool -> t -> callback:(unit -> unit) -> Gobject.Signal.handler_id
  val on_tool_changed : ?after:bool -> t -> callback:(tool:Device_tool.t -> unit) -> Gobject.Signal.handler_id
  ...
  let on_tool_changed ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let tool = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
      callback ~tool)
   in
...
and Display
 : sig
  ...
  for the @display. *)
  external get_setting : t -> string -> Gobject.Value.t -> bool = "ml_gdk_display_get_setting"
  ...
  val on_closed : ?after:bool -> t -> callback:(is_error:bool -> unit) -> Gobject.Signal.handler_id
  val on_opened : ?after:bool -> t -> callback:(unit -> unit) -> Gobject.Signal.handler_id
  val on_seat_added : ?after:bool -> t -> callback:(seat:Seat.t -> unit) -> Gobject.Signal.handler_id
  val on_seat_removed : ?after:bool -> t -> callback:(seat:Seat.t -> unit) -> Gobject.Signal.handler_id
  val on_setting_changed : ?after:bool -> t -> callback:(setting:string -> unit) -> Gobject.Signal.handler_id
  ...
  let on_closed ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let is_error = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_boolean v) in
      callback ~is_error)
   in
  ...
  let on_seat_added ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let seat = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
      callback ~seat)
   in
  ...
  let on_setting_changed ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let setting = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_string v) in
      callback ~setting)
   in
...
and Seat
 : sig
  ...
  val on_device_added : ?after:bool -> t -> callback:(device:Device.t -> unit) -> Gobject.Signal.handler_id
  val on_device_removed : ?after:bool -> t -> callback:(device:Device.t -> unit) -> Gobject.Signal.handler_id
  val on_tool_added : ?after:bool -> t -> callback:(tool:Device_tool.t -> unit) -> Gobject.Signal.handler_id
  val on_tool_removed : ?after:bool -> t -> callback:(tool:Device_tool.t -> unit) -> Gobject.Signal.handler_id
  ...
  let on_device_added ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let device = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
      callback ~device)
   in
...
and Surface
 : sig
  ...
  val on_enter_monitor : ?after:bool -> t -> callback:(monitor:Monitor.t -> unit) -> Gobject.Signal.handler_id
  val on_event : ?after:bool -> t -> callback:(event:Event.t -> bool) -> Gobject.Signal.handler_id
  val on_layout : ?after:bool -> t -> callback:(width:int -> height:int -> unit) -> Gobject.Signal.handler_id
  val on_leave_monitor : ?after:bool -> t -> callback:(monitor:Monitor.t -> unit) -> Gobject.Signal.handler_id
  ...
  let on_enter_monitor ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let monitor = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
      callback ~monitor)
   in
  ...
  let on_event ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let event = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_object_exn v) in
  ...
  let on_layout ?after obj ~callback =
    let closure = Gobject.Closure.create (fun argv ->
      let width = (let v = (Gobject.Closure.nth argv ~pos:1) in Gobject.Value.get_int v) in
      let height = (let v = (Gobject.Closure.nth argv ~pos:2) in Gobject.Value.get_int v) in
