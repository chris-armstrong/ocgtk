...
class texture (obj : Texture.t) : texture_t =
  object (self)
    ...
    method get_color_state : unit -> Cicp_params_and__color_state.Color_state.t
        =
      fun () -> Texture.get_color_state obj

    method get_format : unit -> Gdk_enums.memoryformat =
      fun () -> Texture.get_format obj

    method get_height : unit -> int = fun () -> Texture.get_height obj
    method get_width : unit -> int = fun () -> Texture.get_width obj

    method save_to_png : string -> bool =
      fun filename -> Texture.save_to_png obj filename

    method save_to_png_bytes : unit -> Glib_bytes.t =
      fun () -> Texture.save_to_png_bytes obj

    method save_to_tiff : string -> bool =
      fun filename -> Texture.save_to_tiff obj filename

    method save_to_tiff_bytes : unit -> Glib_bytes.t =
      fun () -> Texture.save_to_tiff_bytes obj

    method as_texture = obj
  end

let new_for_pixbuf (pixbuf : Ocgtk_gdkpixbuf.GdkPixbuf.Pixbuf.pixbuf_t) :
    texture_t =
...
let new_from_file (file : Ocgtk_gio.Gio.File.file_t) :
    (texture_t, GError.t) result =
