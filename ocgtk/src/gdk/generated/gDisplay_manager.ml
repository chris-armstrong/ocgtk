...
class display_manager (obj : Display_manager.t) : display_manager_t = object (self)
  method on_display_opened ?(after = false) ~callback () =
    Display_manager.on_display_opened ~after self#as_display_manager
      ~callback:(fun ~display -> callback ~display:(new GApp_launch_context_cycle_de440b34.display display))


  method get_default_display : unit -> GApp_launch_context_cycle_de440b34.display_t option =
    fun () ->
      Option.map (fun ret -> new GApp_launch_context_cycle_de440b34.display ret) (Display_manager.get_default_display obj)

  method list_displays : unit -> GApp_launch_context_cycle_de440b34.display_t list =
    fun () ->
      (List.map (fun ret -> new GApp_launch_context_cycle_de440b34.display ret))(Display_manager.list_displays obj)

  method open_display : string option -> GApp_launch_context_cycle_de440b34.display_t option =
    fun name ->
      Option.map (fun ret -> new GApp_launch_context_cycle_de440b34.display ret) (Display_manager.open_display obj name)
