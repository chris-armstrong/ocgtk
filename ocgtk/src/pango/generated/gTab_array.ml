...
class tab_array (obj : Tab_array.t) : tab_array_t =
  object (self)
    ...
    method get_positions_in_pixels : unit -> bool =
      fun () -> Tab_array.get_positions_in_pixels obj

    method get_size : unit -> int = fun () -> Tab_array.get_size obj
    method resize : int -> unit = fun new_size -> Tab_array.resize obj new_size
    ...
    method sort : unit -> unit = fun () -> Tab_array.sort obj
    method to_string : unit -> string = fun () -> Tab_array.to_string obj
    method as_tab_array = obj
  end

let new_ (initial_size : int) (positions_in_pixels : bool) : tab_array_t =
  let obj_ = Tab_array.new_ initial_size positions_in_pixels in
  new tab_array obj_
