...
class renderer (obj : Renderer.t) : renderer_t =
  object (self)
    method activate : unit -> unit = fun () -> Renderer.activate obj
    method deactivate : unit -> unit = fun () -> Renderer.deactivate obj
    ...
    method get_alpha : Pango_enums.renderpart -> UInt16.t =
      fun part -> Renderer.get_alpha obj part
    ...
    method get_matrix : unit -> Matrix.t option =
      fun () -> Renderer.get_matrix obj

    method part_changed : Pango_enums.renderpart -> unit =
      fun part -> Renderer.part_changed obj part

    method set_alpha : Pango_enums.renderpart -> UInt16.t -> unit =
      fun part alpha -> Renderer.set_alpha obj part alpha

    method set_color : Pango_enums.renderpart -> Color.t option -> unit =
      fun part color -> Renderer.set_color obj part color

    method set_matrix : Matrix.t option -> unit =
      fun matrix -> Renderer.set_matrix obj matrix
