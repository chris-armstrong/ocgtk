...
class rect (obj : Rect.t) : rect_t = object (self)

  method contains_point : Point.t -> bool =
    fun p ->
      (Rect.contains_point obj p)

  method contains_rect : Rect.t -> bool =
    fun b ->
      (Rect.contains_rect obj b)
  ...
  method init : float -> float -> float -> float -> Rect.t =
    fun x y width height ->
      (Rect.init obj x y width height)

  method init_from_rect : Rect.t -> Rect.t =
    fun src ->
      (Rect.init_from_rect obj src)

  method inset : float -> float -> Rect.t =
    fun d_x d_y ->
      (Rect.inset obj d_x d_y)
  ...
  method offset : float -> float -> Rect.t =
    fun d_x d_y ->
      (Rect.offset obj d_x d_y)

  method round_to_pixel : unit -> Rect.t =
    fun () ->
      (Rect.round_to_pixel obj)
