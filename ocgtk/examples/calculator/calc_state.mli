...
(** Abstract calculator state.
    ...
    - After {!evaluate}, expression is set to the result value on success so
      that chained operations (e.g. [2+3=] then [*4=] yields [20]) work *)

val create : unit -> t
(** Initial state: empty expression, result "0", no error. *)

val append_char : t -> char -> t
(** Append a digit, operator, decimal point, or parenthesis. Returns state
    unchanged if:
    - In error state
    - Character is invalid
    - Leading operator other than minus Consecutive operators are replaced (last
      wins). *)
...
val evaluate : t -> t
(** Evaluate the current expression. Returns state unchanged if expression is
    incomplete (ends with operator) or already in error state. On success, sets
    both expression and result to the computed value. On failure, sets error
    flag. *)
