(** Expression evaluator for calculator
    ...
    Supports +, -, *, / operators with correct precedence, parentheses, and
    automatic closing of unclosed parentheses when valid. *)
...
(** Evaluate an expression string.
    ...
    Returns [Ok result] on success, [Error msg] on failure. Error cases include:
    division by zero, malformed expressions, invalid auto-close positions. *)
...
(** Check if an expression ends with an incomplete operator.
    ...
    Returns [true] if the expression is empty, whitespace-only, or ends with +,
    -, *, or /. *)
