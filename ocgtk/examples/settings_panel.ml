...
let () =
  ...
  let email_check = new Check_button.check_button (Wrappers.Check_button.new_with_label (Some "Enable email notifications")) in
  vbox#append (email_check :> Widget.widget_t);

  let sound_check = new Check_button.check_button (Wrappers.Check_button.new_with_label (Some "Enable sounds")) in
  sound_check#set_active true;
  vbox#append (sound_check :> Widget.widget_t);

  let auto_save_check = new Check_button.check_button (Wrappers.Check_button.new_with_label (Some "Auto-save documents")) in
  vbox#append (auto_save_check :> Widget.widget_t);

  (* Section 2: Radio buttons using CheckButton grouping (GTK4 style) *)
  let lbl2 = new Label.label (Wrappers.Label.new_ (Some "\nTheme Selection:")) in
  ...
  let advanced_toggle = new Toggle_button.toggle_button (Wrappers.Toggle_button.new_with_label "Advanced settings") in
  ...
  let font_spin = new Spin_button.spin_button (Wrappers.Spin_button.new_with_range 8.0 72.0 1.0) in
  font_spin#set_value 12.0;
  font_spin#set_digits 0;
  vbox#append (font_spin :> Widget.widget_t);

  (* Apply button to show current settings *)
  let apply_btn = new Button.button (Wrappers.Button.new_with_label "Show Current Settings") in
