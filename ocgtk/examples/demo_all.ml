...
let () =
  ...
  let check1 = Wrappers.Check_button.new_with_label (Some "Enable notifications") in
  Printf.printf "✓ CheckButton created with label: '%s'\n"
    (Option.value ~default:"" (Wrappers.Check_button.get_label check1));

  let check2 = Wrappers.Check_button.new_with_mnemonic (Some "_Advanced mode") in
  ...
  let gcheck_obj = Wrappers.Check_button.new_with_label (Some "Feature enabled") in
