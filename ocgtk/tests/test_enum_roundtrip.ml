(** Test round-trip conversions for GTK4 enums
    ...
    This test ensures that converting OCaml enum values to C integers and back
    produces the same value (bijection property). *)
...
let () =
  Alcotest.run "Enum Round-trip Conversions"
    [
      ...
      ( "GtkOrientation",
        [
          Alcotest.test_case "round-trip all values" `Quick
            test_orientation_roundtrip;
        ] );
      ( "GtkPackType",
        [
          Alcotest.test_case "round-trip all values" `Quick
            test_pack_type_roundtrip;
        ] );
    ]
