...
let test_convert_utf8_to_utf8 () =
  ...
  let converted =
    Glib.Convert.convert original ~to_codeset:"UTF-8" ~from_codeset:"UTF-8"
  in
...
let test_convert_invalid_codeset () =
  try
    let _result =
      Glib.Convert.convert "test" ~to_codeset:"INVALID_CHARSET_XYZ"
        ~from_codeset:"UTF-8"
    in
    Alcotest.fail "Should have raised GError"
  with Glib.GError msg ->
    Alcotest.(check bool)
      "GError raised for invalid codeset" true
      (String.length msg > 0)
...
let () =
  Alcotest.run "GLib Module Tests (Phase 2.1)"
    [
      ( "Enums (Phase 1.3)",
        [
          Alcotest.test_case "enum modules accessible" `Quick
            test_glib_enums_accessible;
      ...
      ( "Main Event Loop",
        [
          Alcotest.test_case "create and destroy" `Quick
            test_main_loop_create_destroy;
      ...
      ( "Timeout Callbacks",
        [
          ...
          Alcotest.test_case "timeout with priority" `Quick
            test_timeout_priority;
      ...
      ( "UTF-8 Validation",
        [
          ...
          Alcotest.test_case "validate Unicode" `Quick
            test_utf8_validate_unicode;
          Alcotest.test_case "validate empty" `Quick test_utf8_validate_empty;
          Alcotest.test_case "detect invalid UTF-8" `Quick
            test_utf8_validate_invalid;
        ] );
      ( "Character Set Conversion",
        [
          Alcotest.test_case "get charset" `Quick test_charset_get;
          Alcotest.test_case "locale UTF-8 roundtrip" `Quick
            test_locale_utf8_roundtrip;
          Alcotest.test_case "filename UTF-8 roundtrip" `Quick
            test_filename_utf8_roundtrip;
          Alcotest.test_case "UTF-8 to UTF-8" `Quick test_convert_utf8_to_utf8;
        ] );
      ( "GError Handling",
        [
          Alcotest.test_case "invalid codeset" `Quick
            test_convert_invalid_codeset;
        ] );
    ]
