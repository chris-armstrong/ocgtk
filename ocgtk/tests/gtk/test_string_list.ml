...
let test_take_string_copy_independence () =
  ...
  (match String_list.get_string list 0 with
   | Some retrieved -> check string "take: retrieved equals original" "mutable string" retrieved
...
let test_take_copies_ownership () =
  ...
  let list = String_list.new_ () in
  let owned = String.make 100 'x' in  (* Create larger string *)
  ...
  | Some retrieved ->
      ...
      if String.length retrieved = 100 && retrieved.[0] = 'x'
      then check bool "take: ownership transfer works" true true
      else fail "Retrieved string corrupted"
  | None -> fail "String lost after GC pressure"

let test_take_special_chars () =
  let list = String_list.new_ () in
  let special = "Hello\x00World\t\n\r\"\\" in  (* Null, tab, newline, quotes, backslash *)
  String_list.take list special;

  match String_list.get_string list 0 with
  | Some retrieved -> check string "take: special chars preserved" special retrieved
  | None -> fail "Special characters not preserved"

let test_take_unicode () =
  let list = String_list.new_ () in
  let unicode = "Unicode: \xE2\x9D\xA4\xE2\x9D\xA4" in  (* Hearts: ❤❤ *)
...
let test_take_long_string () =
  let list = String_list.new_ () in
  let long = String.make 10000 'z' in  (* 10KB string *)
...
let test_remove_invalid_index () =
  ...
  with e ->
    fail ("Invalid remove threw: " ^ Printexc.to_string e)

(* ========== Get N Items Tests ========== *)

let test_n_items_empty () =
  check int "Empty n_items" 0 (String_list.get_n_items (String_list.new_ ()))
...
let test_string_list_survives_gc () =
  ...
  match String_list.get_string list 0 with
  | Some s ->
      if s <> String.make 1000 'x'
      then fail "String corrupted after GC"
...
let () =
  run "Gtk.StringList Tests"
    [
      "Module", [
        test_case "module_accessible" `Quick test_string_list_module_accessible;
        test_case "type" `Quick test_string_list_type;
      ];
      "Creation", [
        test_case "empty_list" `Quick (require_gtk test_creation_empty_list);
        test_case "empty_string" `Quick (require_gtk test_creation_with_empty_string);
      ];
      "Take Method (String_copy)", [
        test_case "basic" `Quick (require_gtk test_take_basic);
        test_case "independence" `Quick (require_gtk test_take_string_copy_independence);
      ...
      "Get String (Extraction)", [
        test_case "valid_index" `Quick (require_gtk test_get_string_valid_index);
        test_case "negative_index" `Quick (require_gtk test_get_string_negative_index);
        test_case "out_of_bounds" `Quick (require_gtk test_get_string_out_of_bounds);
        test_case "empty_list" `Quick (require_gtk test_get_string_empty_list);
        test_case "returns_copy" `Quick (require_gtk test_get_string_returns_copy);
      ];
      "Remove", [
      ...
      "N Items", [
        ...
        test_case "operations" `Quick (require_gtk test_n_items_after_operations);
      ];
      "Wrapper", [
        test_case "gstring_list" `Quick (require_gtk test_gstring_list_wrapper);
      ];
      "Integration", [
        test_case "mixed_operations" `Quick (require_gtk test_mixed_operations);
        test_case "gc_survival" `Quick (require_gtk test_string_list_survives_gc);
      ];
    ]
