(** Comprehensive variance test suite for Gobject.obj
    ...
    - Button.t (with [`button | `widget | `initially_unowned]) can upcast to
      Widget.t (with [`widget | `initially_unowned])
...
(** Test 1: Button.t upcasts to Widget.t
    ...
    Since Button's variant includes all of Widget's tags plus `button, Button.t
    is a subtype of Widget.t with contravariant obj, allowing safe upcast. *)
...
(** Test 2: Box.t upcasts to Widget.t
    ...
    Box has polymorphic variant [`box | `widget | `initially_unowned] Widget has
    polymorphic variant [`widget | `initially_unowned]

    Box's variant includes all of Widget's tags plus `box, so Box.t is a subtype
    of Widget.t, allowing safe upcast. *)
...
let test_upcast_preserves_functionality () =
  ...
  Widget.hide widget;
  check bool "upcasted widget responds to methods" false
    (Widget.get_visible widget);
  Widget.show widget;
  check bool "upcasted widget can be shown again" true
    (Widget.get_visible widget)
...
let test_multiple_upcasts () =
  ...
  check bool "modifications visible through all references" false
    (Widget.get_visible widget2)
...
(** Test 5: Downcast should fail at compile time
    ...
    let test_downcast_widget_to_button () = let w = Widget.new_ () in let btn =
    (w : Widget.t :> Button.t) in (* ERROR: Widget is not a subtype of Button *)
    ()

    Widget.t has [`widget | `initially_unowned] Button.t has
    [`button | `widget | `initially_unowned]

    Widget's variant does NOT include all of Button's tags (missing `button), so
    Widget.t is NOT a subtype of Button.t, and the downcast should fail. *)
...
let () =
  run "Gobject.obj Variance Tests"
    [
      ( "upcast",
        [
          test_case "button_to_widget" `Quick
            (require_gtk test_upcast_button_to_widget);
          test_case "box_to_widget" `Quick
            (require_gtk test_upcast_box_to_widget);
          test_case "upcast_preserves_functionality" `Quick
            (require_gtk test_upcast_preserves_functionality);
          test_case "multiple_upcasts" `Quick
            (require_gtk test_multiple_upcasts);
        ] );
    ]
