 ...
 *
 * Also provides ml_gdk_rectangle_create for the boxed GValue round-trip test.
 * ml_gdk_rectangle_create wraps a freshly-heap-allocated GdkRectangle via
 * ml_gir_record_val_ptr_with_type (CAMLexport from wrappers.c, part of
 * ocgtk.common) which attaches the gdk_rectangle_get_type() GType so the
 * gir_record finalizer calls g_boxed_free correctly.
 *
 * Note: ml_gdk_rectangle_get_type is generated by gir_gen into
 * ml_rectangle_gen.c — no longer defined here.
 */
