...
let test_application_name_get_set () =
  ...
  Glib.set_application_name "ocgtk4 Test Suite";

  (* Verify it was set *)
  let result = Glib.get_application_name () in
  Alcotest.(check (option string))
    "application name set correctly" (Some "ocgtk4 Test Suite") result;
...
let test_program_name_get_set () =
  ...
  Glib.set_prgname "ocgtk4_test";

  (* Verify it was set *)
  let result = Glib.get_prgname () in
  Alcotest.(check (option string))
    "program name set correctly" (Some "ocgtk4_test") result
...
let test_gtype_operations () =
  ...
  Alcotest.(check bool) "String type has name" true (String.length type_name > 0);

  (* Should be able to check type relationships *)
  let is_string = Gobject.Type.is_a string_type_id string_type_id in
  Alcotest.(check bool) "type is_a itself" true is_string
...
let test_gvalue_int () =
  ...
  Alcotest.(check int) "GValue int roundtrip" 42 result;
...
let test_gvalue_string () =
  ...
  Alcotest.(check string) "GValue string roundtrip" "Hello, GTK4!" result;
...
let test_string_utilities () =
  ...
  Alcotest.(check bool) "utf8 string has content" true (len > 0)
  ...
  Alcotest.run "FFI Integration Tests"
    [
      ( "GLib/ApplicationName",
        [
          Alcotest.test_case "get_set application name" `Quick
            test_application_name_get_set;
          Alcotest.test_case "get_set program name" `Quick
            test_program_name_get_set;
        ] );
      ( "GObject/Types",
