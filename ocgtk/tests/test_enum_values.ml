(** Test that all enum values map to distinct C integers
    ...
    This test ensures that: 1. Each enum value maps to a unique C integer
    (injective) 2. The C integers are valid GTK4 constants 3. No two different
    OCaml values map to the same C value *)
...
let test_align_distinct () =
  ...
  Alcotest.(check int)
    "all GtkAlign values map to distinct integers" (List.length values)
    (List.length unique)
...
let test_orientation_distinct () =
  ...
  let unique = List.sort_uniq compare integers in
  Alcotest.(check int)
    "all GtkOrientation values map to distinct integers" (List.length values)
    (List.length unique)
...
let test_pack_type_distinct () =
  ...
  let unique = List.sort_uniq compare integers in
  Alcotest.(check int)
    "all GtkPackType values map to distinct integers" (List.length values)
    (List.length unique)
...
let () =
  Alcotest.run "Enum Value Coverage"
    [
      ...
      ( "GtkOrientation",
        [
          Alcotest.test_case "all values exist" `Quick
            test_orientation_values_exist;
          Alcotest.test_case "all values distinct" `Quick
            test_orientation_distinct;
        ] );
      ( "GtkPackType",
        [
          Alcotest.test_case "all values exist" `Quick
            test_pack_type_values_exist;
          Alcotest.test_case "all values distinct" `Quick
            test_pack_type_distinct;
        ] );
    ]
