...
let test_create_basic () =
  ...
  check int "rowstride" 300 (GdkPixbuf.get_rowstride pb)
(* 100 * 3 *)
...
let test_create_with_alpha () =
  ...
  check int "rowstride with alpha" 400 (GdkPixbuf.get_rowstride pb)
(* 100 * 4 *)
...
let test_create_large () =
  ...
  check int "large rowstride" 7680 (GdkPixbuf.get_rowstride pb)
(* 1920 * 4 *)
...
let test_copy_basic () =
  ...
  check int "copy channels"
    (GdkPixbuf.get_n_channels pb1)
    (GdkPixbuf.get_n_channels pb2);
  check bool "copy has_alpha"
    (GdkPixbuf.get_has_alpha pb1)
    (GdkPixbuf.get_has_alpha pb2)
...
let test_subpixbuf () =
  ...
  check int "subpixbuf channels"
    (GdkPixbuf.get_n_channels pb)
    (GdkPixbuf.get_n_channels sub)
...
let test_add_alpha_basic () =
  ...
  check int "after add_alpha channels" 4
    (GdkPixbuf.get_n_channels pb_with_alpha);
...
let test_add_alpha_with_transparent () =
  ...
  check bool "transparent add_alpha" true
    (GdkPixbuf.get_has_alpha pb_with_alpha);
  check int "transparent add_alpha channels" 4
    (GdkPixbuf.get_n_channels pb_with_alpha)

let test_copy_area () =
  let src = GdkPixbuf.create ~width:50 ~height:50 () in
  let dest = GdkPixbuf.create ~width:50 ~height:50 () in
  GdkPixbuf.fill src 0xFF0000FFl;
  (* Red *)
  GdkPixbuf.fill dest 0x0000FFFFl;

  (* Blue *)

  (* Copy 10x10 area from src to dest *)
  GdkPixbuf.copy_area ~dest ~dest_x:0 ~dest_y:0 ~width:10 ~height:10 ~src_x:0
    ~src_y:0 src;
...
let test_copy_area_out_of_bounds () =
  ...
  try
    ...
    GdkPixbuf.copy_area ~dest ~dest_x:0 ~dest_y:0 ~width:60 ~height:60 ~src_x:0
      ~src_y:0 src;
...
let test_multiple_pixbufs_gc () =
  ...
  for i = 1 to 10 do
    ...
    if i mod 2 = 0 then pixbufs := pb :: !pixbufs
  done;
  Gc.minor ();
...
let test_from_xpm_data () =
  ...
  let xpm_data = [| "2 2 1 1"; "r c #FF0000"; "rr"; "rr" |] in
...
let test_save_and_load () =
  ...
  GdkPixbuf.fill pb1 0x00FF00FFl;
  ...
  check int "loaded height" (GdkPixbuf.get_height pb1)
    (GdkPixbuf.get_height pb2);
...
let test_save_with_options () =
  ...
  GdkPixbuf.save ~filename:tmp_file ~typ:"png"
    ...
    [
      ...
      ( "Transform Operations",
        [
          ...
          test_case "Add alpha with transparent color" `Quick
            test_add_alpha_with_transparent;
      ...
      ( "Input Validation",
        [
          ...
          test_case "Invalid color in add_alpha" `Quick
            test_add_alpha_invalid_color;
          test_case "Copy area out of bounds" `Quick
            test_copy_area_out_of_bounds;
        ] );
      ( "Memory Safety",
        [
      ...
      ("XPM Data", [ test_case "Load from XPM data" `Quick test_from_xpm_data ]);
      ( "File I/O",
        [
