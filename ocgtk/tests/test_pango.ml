...
let test_font_variant () =
  ...
  check
    (testable
       Fmt.(
         using
           (function `NORMAL -> "normal" | `SMALL_CAPS -> "small-caps")
           string)
       ( = ))
...
let test_font_modify () =
  let desc = Pango.Font.from_string "Sans 10" in
  Pango.Font.modify desc ~family:"DejaVu Sans" ~style:`ITALIC
...
let test_scale_to_float () =
  ...
  check
    (testable Fmt.float (fun a b -> abs_float (a -. b) < 0.01))
...
let test_weight_enum () =
  ...
  let _weights : Pango.Tags.weight list =
    [ `ULTRALIGHT; `LIGHT; `NORMAL; `BOLD; `ULTRABOLD; `HEAVY; `CUSTOM 600 ]
  in
...
let integration_tests =
  [
    test_case "context operations (skipped)" `Quick
      test_context_operations_skipped;
    test_case "layout operations (skipped)" `Quick
      test_layout_operations_skipped;
  ]
