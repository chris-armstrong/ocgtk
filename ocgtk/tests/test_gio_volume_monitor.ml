(** Runtime integration tests for GList<interface> via L2 VolumeMonitor.
    ...
    In typical CI there are no real hardware drives, so Drive/Volume lists will
    be empty. Mount lists may also be empty. Each test is written to work
    correctly on empty lists while still exercising the wrapping machinery when
...
(** Test 1: L2 list return — get mounts and call get_name on each element. This
    verifies that the generated [List.map (fun ret -> new mount ret)] wrapper
    produces usable L2 objects and that calling a method on them does not crash.
*)
let test_l2_mounts_get_name () =
  ...
  List.iter
    (fun m ->
      ...
      check bool
        (Printf.sprintf "mount name '%s' is non-empty" name)
        true
...
let test_nested_drive_volumes () =
  ...
  List.iter
    (fun d ->
      ...
      check bool
        (Printf.sprintf "drive name '%s' is non-empty" drive_name)
        true
        (String.length drive_name > 0);
      let volumes = d#get_volumes () in
      List.iter
        (fun v ->
          let vol_name = v#get_name () in
          check bool
            (Printf.sprintf "volume name '%s' is non-empty" vol_name)
            true
            (String.length vol_name > 0))
        volumes)
    drives;
  check bool "get_connected_drives returned a list" true
    (List.length drives >= 0)

(** Test 3: GC stress — call get_mounts / get_volumes many times with GC
    compaction between iterations. Any ref-counting bug or use-after-free in the
    Val_GList_with / finaliser code will manifest here. *)
...
(** Test 4: Mount → optional drive → optional volumes traversal. Tests Option
    return of an interface type combined with a list from that interface:
    mount#get_drive () returns drive_t option, then drive#get_volumes () returns
    volume_t list. *)
  ...
  List.iter
    (fun m ->
      ...
      | Some d ->
          let drive_name = d#get_name () in
          check bool
            (Printf.sprintf "drive-from-mount name '%s' non-empty" drive_name)
            true
            (String.length drive_name > 0);
...
let () =
  Alcotest.run "GIO VolumeMonitor L2 GList interface tests"
    [
      ( "VolumeMonitor L2",
        [
          test_case "L2 mounts: get_name on each element" `Quick
            test_l2_mounts_get_name;
          test_case "nested drive#get_volumes then volume#get_name" `Quick
            test_nested_drive_volumes;
          test_case "GList GC stress (50 iterations + Gc.compact)" `Slow
            test_glist_gc_stress;
          test_case "mount -> optional drive -> volume list" `Quick
            test_mount_drive_volumes;
        ] );
    ]
