# Refactoring Plan: Multi-Library Support for ocgtk

## Goal
Restructure the ocgtk library to support multiple GObject-based libraries (Gtk, Gdk, Gio, etc.) by:
1. Separating hand-written common code from generated code
2. Organizing generated code by library namespace
3. Creating sub-libraries within the `ocgtk` package
...
- [x] Phase 5: Create src/gtk/dune (2025-12-18)
  ...
  - [x] Set library dependencies: ocgtk_common, ocgtk_generated_stubs, cairo2, threads
- [x] Phase 6: Update Main src/dune (2025-12-18)
  - [x] Converted to wrapper library with no modules
  - [x] Added dependencies: ocgtk_common, ocgtk_gtk, threads, cairo2
...
- [x] Phase 7: Update Generated dune-generated.inc (2025-12-18)
  ...
  - [x] Added ocgtk_common library dependency for header access
...
- [x] Phase 9: Testing and Validation (2025-12-18)
  ...
  - [x] Verified library artifacts created: ocgtk_common, ocgtk_generated_stubs, ocgtk_gtk, ocgtk
...
2. **Library Modularity**: Three distinct libraries working together:
   - `ocgtk_common`: Common utilities and GObject/GLib bindings
   - `ocgtk_generated_stubs`: Generated C stubs (depends on common)
   - `ocgtk_gtk`: Main GTK library combining core and generated code
   - `ocgtk`: Top-level wrapper exposing everything

### Technical Solutions
1. **Header Visibility**: Using `install_c_headers` in src/common/dune to expose wrappers.h
2. **Library Dependencies**: Generated stubs library explicitly depends on ocgtk_common (not just -I paths)
3. **Cross-Library Includes**: Common library includes -I../gtk/generated for generated_forward_decls.h
4. **Unified Build**: Single ocgtk_gtk library uses include_subdirs to find all modules
...
└── src/
    ├── dune                       # Main wrapper library (ocgtk)
    ├── common/                    # Hand-written common code (ocgtk_common)
    ...
    │
    ├── gtk/                       # Gtk library bindings (ocgtk_gtk)
    ...
    │   │   └── gtkSnapshot.ml/mli
    │   │
    │   └── generated/             # Generated Gtk code (ocgtk_generated_stubs + modules)
...
```
ocgtk (wrapper)
  ├─→ ocgtk_common (hand-written common utilities)
  ├─→ ocgtk_gtk (GTK bindings)
  │     ├─→ ocgtk_common
  │     ├─→ ocgtk_generated_stubs (generated C stubs)
  │     │     └─→ ocgtk_common (for headers via install_c_headers)
  │     ├─→ cairo2
  │     └─→ threads
  ├─→ cairo2
  └─→ threads
...
3. **Update dune file generation** in `generate/dune_file.ml`:
   ...
   - Add library dependency on `ocgtk.common` and `ocgtk.gtk.core`
...
(library
 ...
 (libraries ocgtk.common ocgtk.gtk.core ocgtk_generated_stubs))
```
...
```scheme
; Main ocgtk library - re-exports all sub-libraries
...
let generate_dune_library ~stub_names ~module_names =
  ...
  Buffer.add_string buf " (name ocgtk_generated_stubs)\n";
  Buffer.add_string buf " (public_name ocgtk.generated_stubs)\n";
...
1. **Gdk library**:
   ...
   - Update `src/gtk/dune` to depend on `ocgtk.gdk`
...
| Risk | Mitigation |
|------|-----------|
| Breaking existing code | Keep `ocgtk` library re-exporting all modules |
...
- All public APIs remain accessible through `ocgtk` library
