...
let () =
  ...
  vbox#append (new GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget.widget label_widget);

  (* Create button to increment *)
  let button_obj = Button.new_with_label "Click Me!" in
  let button = new GButton.button button_obj in
  vbox#append (button :> GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget.widget);
  ignore (button#on_clicked ~callback:(fun () ->
    count := !count + 1;
  ...
  vbox#append (reset_btn :> GEvent_controller_and__layout_child_and__layout_manager_and__root_and__widget.widget);
  ignore (reset_btn#on_clicked ~callback:(fun () ->
    count := 0;
    Label.set_label label_obj (Printf.sprintf "Count: %d" !count)
  ));
