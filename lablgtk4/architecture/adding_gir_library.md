# Adding a New GIR Library to lablgtk4

This document provides a step-by-step plan for adding a new GObject Introspection (GIR) library to lablgtk4, such as GIO, GtkSourceView, WebKit, or Libadwaita.
...
Edit `/workspaces/lablgtk/lablgtk4/src/tools/gir_gen/main.ml` around line 411-417:
...
```bash
cd /workspaces/lablgtk/lablgtk4
...
```bash
cd /workspaces/lablgtk/lablgtk4/src
...
```scheme
(library
 (name lablgtk4_<library>)
 (public_name lablgtk4.<library>)
 (synopsis "OCaml bindings for <Library>")
 (flags :standard -w -3-6-9-27-32-33-34-50)
 (libraries lablgtk4_common)
 (foreign_stubs
  (language c)
...
(library
 ...
 (libraries lablgtk4_common lablgtk4_gtk lablgtk4_<library>)
 (re_export lablgtk4_common lablgtk4_gtk lablgtk4_<library>))
```
...
│   ├── common/                       # lablgtk4_common
│   ├── gtk/                          # lablgtk4_gtk
│   └── <library>/                    # lablgtk4_<library> (NEW)
