...
class revealer (obj : Revealer.t) = object (self)
  ...
  method get_child_revealed : unit -> bool = fun () -> (Revealer.get_child_revealed obj )

  method get_reveal_child : unit -> bool = fun () -> (Revealer.get_reveal_child obj )

  method get_transition_duration : unit -> int = fun () -> (Revealer.get_transition_duration obj )
  ...
  method set_reveal_child : bool -> unit = fun reveal_child -> (Revealer.set_reveal_child obj reveal_child)

  method set_transition_duration : int -> unit = fun duration -> (Revealer.set_transition_duration obj duration)
