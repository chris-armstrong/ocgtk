(* GTK4 Widget Base Module - Manually Created *)
...
external get_parent : t -> Gtk.widget option = "ml_gtk_widget_get_parent"
external get_root : t -> Gtk.widget option = "ml_gtk_widget_get_root"
...
let align_to_int : Gtk.align -> int = function
  | `FILL -> 0
  | `START -> 1
  | `END -> 2
  | `CENTER -> 3
...
let int_to_align : int -> Gtk.align = function
  | 0 -> `FILL
  | 1 -> `START
  | 2 -> `END
  | 3 -> `CENTER
...
let get_halign widget : Gtk.align =
  int_to_align (get_halign_impl widget)

let set_halign widget (align : Gtk.align) =
  set_halign_impl widget (align_to_int align)

let get_valign widget : Gtk.align =
  int_to_align (get_valign_impl widget)

let set_valign widget (align : Gtk.align) =
