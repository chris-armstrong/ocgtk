...
let () =
  _init ();
  Callback.register_exception "gdk_pixbuf_error"
    (GdkPixbufError (ERROR_CORRUPT_IMAGE, ""))
...
external get_bits_per_sample : pixbuf -> int = "ml_gdk_pixbuf_get_bits_per_sample"
...
let create ~width ~height ?(bits=8) ?(colorspace=`RGB) ?(has_alpha=false) () =
  if width <= 0 || height <= 0 then
    invalid_arg "GdkPixbuf.create: width and height must be positive";
  if bits <> 8 then
    invalid_arg "GdkPixbuf.create: only 8 bits per sample is supported";
  _create ~colorspace ~has_alpha ~bits ~width ~height

let cast o : pixbuf = Gobject.unsafe_cast o
...
let add_alpha ?transparent pb =
  ...
  | None -> _add_alpha pb ~subst:false ~r:0 ~g:0 ~b:0
  | Some (r, g, b) ->
      if r < 0 || r > 255 || g < 0 || g > 255 || b < 0 || b > 255 then
        invalid_arg "GdkPixbuf.add_alpha: RGB values must be in range 0-255";
      _add_alpha pb ~subst:true ~r ~g ~b
...
let saturate_and_pixelate ~dest ~saturation ~pixelate src =
  if saturation < 0.0 then
    invalid_arg "GdkPixbuf.saturate_and_pixelate: saturation must be non-negative";
...
let composite ~dest ~alpha ?(dest_x=0) ?(dest_y=0) ?width ?height
  ...
  if alpha < 0 || alpha > 255 then
    invalid_arg "GdkPixbuf.composite: alpha must be in range 0-255";
  let width, scale_x =
    get_size width scale_x ~ssrc:(get_width src)
      ~sdest:(get_width dest) ~dest:dest_x ~ofs:ofs_x
  and height, scale_y =
