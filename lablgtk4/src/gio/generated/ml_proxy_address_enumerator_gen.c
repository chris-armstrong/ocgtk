...
if (pspec == NULL) caml_failwith("ml_gtk_proxy_address_enumerator_get_default_port: property 'default-port' not found");
...
CAMLexport CAMLprim value ml_gtk_proxy_address_enumerator_get_uri(value self)
{
CAMLparam1(self);
CAMLlocal1(result);
GProxyAddressEnumerator *obj = (GProxyAddressEnumerator *)GProxyAddressEnumerator_val(self);
...
if (pspec == NULL) caml_failwith("ml_gtk_proxy_address_enumerator_get_uri: property 'uri' not found");
GValue prop_gvalue = G_VALUE_INIT;
g_value_init(&prop_gvalue, pspec->value_type);
