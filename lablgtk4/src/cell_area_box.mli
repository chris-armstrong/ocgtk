(* GENERATED CODE - DO NOT EDIT *)
(* Widget: CellAreaBox *)

type t = Gtk.Widget.t

(** Create a new CellAreaBox *)
external new_ : unit -> t = "ml_gtk_cell_area_box_new"

(* Properties *)

(** Get property: spacing *)
external get_spacing : t -> int = "ml_gtk_cell_area_box_get_spacing"

(** Set property: spacing *)
external set_spacing : t -> int -> unit = "ml_gtk_cell_area_box_set_spacing"

external pack_start : t -> Gtk.Widget.t -> bool -> bool -> bool -> unit = "ml_gtk_cell_area_box_pack_start"

external pack_end : t -> Gtk.Widget.t -> bool -> bool -> bool -> unit = "ml_gtk_cell_area_box_pack_end"

