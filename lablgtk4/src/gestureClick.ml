...
external connect_pressed :
  ...
  Gobject.Signal.handler_id = "ml_gtk_gesture_click_connect_pressed"

external connect_released :
  t ->
  callback:(n_press:int -> x:float -> y:float -> unit) ->
  Gobject.Signal.handler_id = "ml_gtk_gesture_click_connect_released"

external connect_stopped :
  t ->
  callback:(unit -> unit) ->
  Gobject.Signal.handler_id = "ml_gtk_gesture_click_connect_stopped"

external connect_unpaired_release :
  t ->
  callback:(x:float -> y:float -> button:int -> unit) ->
  Gobject.Signal.handler_id = "ml_gtk_gesture_click_connect_unpaired_release"
