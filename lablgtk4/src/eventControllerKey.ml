...
external connect_key_pressed :
  ...
  Gobject.signal_id = "ml_gtk_event_controller_key_connect_key_pressed"

external connect_key_released :
  t ->
  callback:(keyval:int -> keycode:int -> state:Gdk.modifier_type list -> unit) ->
  Gobject.signal_id = "ml_gtk_event_controller_key_connect_key_released"

external connect_modifiers :
  t ->
  callback:(state:Gdk.modifier_type list -> bool) ->
  Gobject.signal_id = "ml_gtk_event_controller_key_connect_modifiers"

external forward : t -> Gdk.Event.t -> bool = "ml_gtk_event_controller_key_forward"
