...
external connect_key_pressed :
  ...
  Gobject.Signal.handler_id = "ml_gtk_event_controller_key_connect_key_pressed"

external connect_key_released :
  t ->
  callback:(keyval:int -> keycode:int -> state:Gdk.Tags.modifier_type list -> unit) ->
  Gobject.Signal.handler_id = "ml_gtk_event_controller_key_connect_key_released"

external connect_modifiers :
  t ->
  callback:(state:Gdk.Tags.modifier_type list -> bool) ->
  Gobject.Signal.handler_id = "ml_gtk_event_controller_key_connect_modifiers"

external forward : t -> [`widget] Gobject.obj -> bool = "ml_gtk_event_controller_key_forward"
