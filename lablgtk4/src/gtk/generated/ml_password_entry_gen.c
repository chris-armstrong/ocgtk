...
GtkPasswordEntry *obj = (GtkPasswordEntry *)GtkPasswordEntry_val(self);
    gboolean prop_value;
GParamSpec *pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(obj), "show-peek-icon");
if (pspec == NULL) caml_failwith("ml_gtk_password_entry_get_show_peek_icon: property 'show-peek-icon' not found");
GValue prop_gvalue = G_VALUE_INIT;
g_value_init(&prop_gvalue, pspec->value_type);
g_object_get_property(G_OBJECT(obj), "show-peek-icon", &prop_gvalue);
    prop_value = g_value_get_boolean(&prop_gvalue);
...
GtkPasswordEntry *obj = (GtkPasswordEntry *)GtkPasswordEntry_val(self);
    gboolean c_value = Bool_val(new_value);
GParamSpec *pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(obj), "show-peek-icon");
if (pspec == NULL) caml_failwith("ml_gtk_password_entry_set_show_peek_icon: property 'show-peek-icon' not found");
GValue prop_gvalue = G_VALUE_INIT;
g_value_init(&prop_gvalue, pspec->value_type);
    g_value_set_boolean(&prop_gvalue, c_value);
g_object_set_property(G_OBJECT(obj), "show-peek-icon", &prop_gvalue);
g_value_unset(&prop_gvalue);
CAMLreturn(Val_unit);
}
