...
if (pspec == NULL) caml_failwith("ml_gtk_filter_list_model_get_incremental: property 'incremental' not found");
...
GParamSpec *pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(obj), "incremental");
if (pspec == NULL) caml_failwith("ml_gtk_filter_list_model_set_incremental: property 'incremental' not found");
GValue prop_gvalue = G_VALUE_INIT;
g_value_init(&prop_gvalue, pspec->value_type);
...
if (pspec == NULL) caml_failwith("ml_gtk_filter_list_model_get_pending: property 'pending' not found");
GValue prop_gvalue = G_VALUE_INIT;
g_value_init(&prop_gvalue, pspec->value_type);
