...
class box_layout (obj : Box_layout.t) = object (self)
  ...
  method get_baseline_child : unit -> int = fun () -> (Box_layout.get_baseline_child obj )

  method get_baseline_position : unit -> Gtk_enums.baselineposition = fun () -> (Box_layout.get_baseline_position obj )

  method get_homogeneous : unit -> bool = fun () -> (Box_layout.get_homogeneous obj )

  method get_spacing : unit -> int = fun () -> (Box_layout.get_spacing obj )

  method set_baseline_child : int -> unit = fun child -> (Box_layout.set_baseline_child obj child)

  method set_baseline_position : Gtk_enums.baselineposition -> unit = fun position -> (Box_layout.set_baseline_position obj position)

  method set_homogeneous : bool -> unit = fun homogeneous -> (Box_layout.set_homogeneous obj homogeneous)

  method set_spacing : int -> unit = fun spacing -> (Box_layout.set_spacing obj spacing)
