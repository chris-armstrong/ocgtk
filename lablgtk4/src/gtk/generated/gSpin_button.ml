...
class spin_button (obj : Spin_button.t) = object (self)
  ...
  method set_activates_default : bool -> unit = fun activates_default -> (Spin_button.set_activates_default obj activates_default)
  ...
  method set_climb_rate : float -> unit = fun climb_rate -> (Spin_button.set_climb_rate obj climb_rate)

  method set_digits : int -> unit = fun digits -> (Spin_button.set_digits obj digits)

  method set_increments : float -> float -> unit = fun step page -> (Spin_button.set_increments obj step page)

  method set_numeric : bool -> unit = fun numeric -> (Spin_button.set_numeric obj numeric)

  method set_range : float -> float -> unit = fun min max -> (Spin_button.set_range obj min max)

  method set_snap_to_ticks : bool -> unit = fun snap_to_ticks -> (Spin_button.set_snap_to_ticks obj snap_to_ticks)

  method set_update_policy : Gtk_enums.spinbuttonupdatepolicy -> unit = fun policy -> (Spin_button.set_update_policy obj policy)

  method set_value : float -> unit = fun value -> (Spin_button.set_value obj value)
