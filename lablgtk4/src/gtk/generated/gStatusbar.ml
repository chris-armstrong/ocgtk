...
class statusbar (obj : Statusbar.t) = object (self)
  ...
  method get_context_id : string -> int = fun context_description -> (Statusbar.get_context_id obj context_description)

  method pop : int -> unit = fun context_id -> (Statusbar.pop obj context_id)

  method push : int -> string -> int = fun context_id text -> (Statusbar.push obj context_id text)

  method remove : int -> int -> unit = fun context_id message_id -> (Statusbar.remove obj context_id message_id)

  method remove_all : int -> unit = fun context_id -> (Statusbar.remove_all obj context_id)
