...
### 1. Signal Handling - Partially Implemented
**Issue:** Signal parameter marshalling exists (`signal_marshaller.ml`) but not all parameter types are supported.

**Working:**
- Parameterless void signals via `Gobject.Signal.connect_simple`
- Signals with typed parameters marshalled via `Gobject.Value` for supported types (bool, int, float, string, object types)

**Missing:**
- Signals with unsupported parameter types (e.g., `GdkEvent`, `GdkModifierType`, custom structs)
- Signals with return values (e.g., `delete-event` returning bool)
- Event marshalling for GdkEvent, GdkModifierType, etc.

**Impact:** Many GTK4 signals are usable, but event-related signals still require manual wrappers.
