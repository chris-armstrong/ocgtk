(** Signal code generation — pure-data API.
    ...
    [Signal_gen] classifies GIR signals into a [signal_emission] record
    that captures all information needed to emit L1 module bindings and
    L2 class method forwarders.  All functions are pure and produce
    strings; no file I/O is performed here.

    Invariants:
    - [classify] only returns [Ok] when every [In]-direction parameter
      has a supported marshaller and the return type is supported.
    - [emit_l1_val] / [emit_l1_let] / [emit_l2_method] are total
      functions over [signal_emission] values — they never fail. *)

open Types

(** All computed values for a single signal, derived from [classify].

    This record is the single source of truth for signal code generation:
    both L1 (module-level) and L2 (class-level) emission functions operate
    on it. *)
type signal_emission = {
  signal : gir_signal;
      (** The original GIR signal record. *)
  method_name : string;
      (** Sanitized OCaml method name, prefixed with ["on_"].
          Example: ["on_clicked"] for signal ["clicked"]. *)
  raw_signal_name : string;
      (** Original GIR signal name used as the GLib signal string.
          Example: ["key-pressed"]. *)
  param_marshallers : (gir_param * Signal_marshaller.marshaller) list;
      (** In-direction parameters paired with their marshalling specs.
          Non-In parameters are excluded (signals with Out/InOut params
          return [Error] from [classify]). *)
  ...
  ocaml_callback_type : string;
      (** Full OCaml function type for the callback parameter.
          Examples:
...
(** [sanitize_signal_name name] converts a GIR signal name to a valid
    OCaml identifier prefixed with ["on_"].

    Hyphens are replaced by underscores and the result is converted to
    snake_case.  If the result is an OCaml keyword, a trailing underscore
    is appended before prepending ["on_"].
...
(** [emit_l1_val e] returns a single [val] declaration line for insertion
    into a generated [.mli] file.

    Example output:
    {[
      val on_clicked : ?after:bool -> t -> callback:(unit -> unit) -> Gobject.handler_id
    ]}

    The [t] argument is the L1 module's own [t] (positional, not labelled). *)
val emit_l1_val : signal_emission -> string

(** [emit_l1_let e] returns a [let] binding for insertion into a generated
    [.ml] file.
...
(** [emit_l2_method e ~layer1_module_name ~class_snake] returns a class
    method definition for insertion into a generated L2 object body.
